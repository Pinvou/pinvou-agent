...
and reaped at the earlier of the scenario and global deadlines. Windows locks
the running image against write/delete and revalidates its final-path file ID
before every helper spawn; Linux executes `/proc/self/exe` so path replacement
cannot redirect the helper. Other Unix platforms fail closed until an equivalent
image-identity primitive is implemented.
