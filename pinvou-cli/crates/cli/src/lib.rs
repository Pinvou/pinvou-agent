    ...
    while index < values.len() {
        if values[index] == "--output" {
            ...
            match value.as_str() {
                ...
                // Not a global-mode value: leave the pair for the
                // subcommand. `sessions export --output FILE` legitimately
                // takes a path here (families without such a flag report it
                // themselves; a file literally named "json"/"human" needs a
                // ./ prefix), and `benchmark submission gaia` still accepts
                // `--output <file>` as a legacy alias of `--destination`
                // (consumed in parse_gaia_submission); everywhere else the
                // leftover token surfaces as the standard usage error.
                _ => index += 1,
            }
        } else {
            index += 1;
