...
bold "── 第 0 层：v0.9.5 r14 公开四主题基线拓扑 ──"
actual_head="$(git -C "$TUI" rev-parse HEAD 2>/dev/null || true)"
if [[ "$actual_head" == "$PUBLISHED_HEAD" ]]; then
  expected_commits="$PUBLISHED_COMMITS"
  green "  ✓ CodeWhale gitlink 指向 r14 四主题公开基线 $PUBLISHED_HEAD"
else
  expected_commits=""
  red "  ✗ CodeWhale HEAD 为 ${actual_head:-<unreadable>}，应为 r14 公开 head $PUBLISHED_HEAD"
  fail=1
fi

if git -C "$TUI" merge-base --is-ancestor "$EXPECTED_UPSTREAM" HEAD 2>/dev/null; then
  green "  ✓ r14 公开基线继承官方 v0.9.5"
else
  red "  ✗ r14 公开基线未继承官方 v0.9.5 $EXPECTED_UPSTREAM"
  fail=1
fi
...
fingerprints=(
  ...
  "T2|cmd.exe 单字母 / 旗标跳过           |CodeWhale/crates/execpolicy/src/lib.rs|fn is_single_letter_slash_flag"
  "T2|deny 中段通配符匹配零或多 token     |CodeWhale/crates/execpolicy/src/lib.rs|if rule_tokens[j] == \"*\" {"
  "T2|deny 命令词 .exe 后缀折叠           |CodeWhale/crates/execpolicy/src/lib.rs|basename.strip_suffix(\".exe\")"
  ...
  "T2|子代理工具调用过 execpolicy         |CodeWhale/crates/tui/src/core/engine.rs|pub(crate) fn exec_shell_ask_rule_decision_for_engine"
  "T2|子代理 execpolicy 拒绝回归          |CodeWhale/crates/tui/src/tools/subagent/tests.rs|async fn forkguard_subagent_execpolicy_deny_matches_main_line"
