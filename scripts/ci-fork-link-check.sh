#!/usr/bin/env bash
# CI 门:改了 submodule(DeepSeek-TUI)的 gitlink,就必须**同 PR**更新 docs/fork-modifications.md。
#
...
gl=no; fm=no
printf '%s\n' "$changed" | grep -qx 'DeepSeek-TUI'                  && gl=yes
printf '%s\n' "$changed" | grep -qx 'docs/fork-modifications.md'    && fm=yes

if [ "$gl" = yes ] && [ "$fm" = no ]; then
  echo "::error::改了 submodule DeepSeek-TUI 的 gitlink,但 docs/fork-modifications.md 未更新。"
