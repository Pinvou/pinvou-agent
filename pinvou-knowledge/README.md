...
服务器始终要求每台设备的独立令牌。分享链接默认 24 小时有效、可供多人提交加入申请；所有者通常需要在 Pinvou 中审批，也可在生成链接时仅对只读成员开启自动通过。Pinvou 只把设备令牌写入系统凭据库，不写入连接元数据。所有网络连接都使用由稳定服务身份保护的 HTTPS；首次加入必须使用携带服务身份和 CA 的分享链接，在发送设备凭据前完成校验。客户端不通过裸地址、局域网发现或 Tailscale 扫描建立首次信任。
