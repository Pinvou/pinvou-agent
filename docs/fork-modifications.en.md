...
- Guard: 7 new `forkguard_*` behavior tests (6 `forkguard_workspace_roots_*` + 1 source-label), default group 31→38. Multi-root sessions also gain an `Accessible folders:` line in per-turn turn_meta (next to `Current workspace:`, capped at 5 roots with truncation, byte-absent for single-root) — the channel through which the model learns about attached roots. Fingerprints: see `scripts/fork-guard.sh`.
