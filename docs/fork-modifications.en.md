...
CodeWhale PR #15 proposes `1eca6103a` plus security follow-ups `169c24cc5`, `21e5f661a`, and `a647ed866`. The candidate adds a process-local per-turn tool policy, complete trusted-path replacement, an exact final dispatch gate, read-only `File` action schema projection with a repeated read-only check before final execution, and denial of queued goal continuation, edit replay, and MCP reload while restricted. Idle child completions and background-shell wakes remain queued until an explicit new message installs replacement authority, and host read-only dispatch narrows Bash to the existing direct-argv read-only shell path. Restricted turns also block queued control-plane operations, hooks, MCP initialization, dynamic tools, and child agents, while tool logs and audits keep only non-private identity fields (event kind, tool name). The fast guard accepts only this registered candidate chain (current head `a647ed866`, 14 commits above v0.9.5) above the immutable `pinvou-v0.9.5-r7` baseline. Parent CI allows this registered candidate on `pull_request` events but enforces strict public-tag verification in the merge queue: before queueing for merge, the candidate chain must be merged, published as immutable tag `pinvou-v0.9.5-r8`, and the parent gitlink plus `PINVOU_CODEWHALE_TAG` aligned to r8.
...
- All 23 CodeWhale `forkguard_*` tests pass for the published r7 baseline; the registered PR #15 candidate passes 34.
