...
| Publication | Public `pinvou3-clean` has advanced to `92427bd8d` (including #58–#61); the immutable tag `pinvou-v0.9.12-r1` stays pinned at `1fafee7e2`; on this branch the parent gitlink points to the workspace_roots topic head `33b23a4a2` (CodeWhale PR #54 head, to be realigned after it merges) |
...
### Multi-root workspace_roots + relative instruction source label (2026-09-17, rebased; CodeWhale PR #54 head)

- CodeWhale branch `pinvou3/workspace-roots-v12` (`qiuYliangM/CodeWhale` fork, PR #54 head `33b23a4a2`; rebased onto `pinvou3-clean` @`92427bd8d` — 17 commits above v0.9.12 r1 + ae7e3fb36/#53 + #58–#61; the earlier 10-commit Chinese-message line with head `f2526196c` is superseded by this English rewrite): threads grow from a single `cwd` into **cwd (primary root) + workspace_roots (full accessible root set)** — the foundation prerequisite for the single-entry workspace (project = primary folder + a set of keys). Four layers: protocol/session model (`workspace_roots` on start/resume/fork params and the `Thread` DTO, serde-defaulted; SQLite `threads` v5 column; both TUI JSON stores additive), per-turn environment (runtime root replacement via `Op::SyncSession`/Runtime API with an active-turn fence, effective next turn; `normalize_workspace_roots` keeps cwd first, deduped; resume three-state semantics including the cwd fallback fix), permission sandbox (per-turn policy construction materializes the full root set directly — `workspace_write_policy` carries `writable_roots = normalize_workspace_roots(workspace, roots)` and an empty set is byte-identical to the legacy single root; the rewrite dropped the earlier `:workspace_roots` symbol-constant design, which no configuration surface consumed; carve-out and `ToolContext::resolve_path` span attached roots, and execpolicy rule matching spans roots while allow rules stay primary-root-scoped), and prompt/instructions (AGENTS.md discovery stays primary-root-only; the file-name-only `<project_instructions source="…">` label via the shared `project_instructions_source_label` helper was absorbed upstream into `pinvou3-clean` by #59, so this topic no longer carries that diff and keeps only the primary-root-only discovery semantics plus its regression tests — moving a directory or switching primary root with identical instructions no longer busts the KV prefix cache).
...
- Guard: 6 new `forkguard_workspace_roots_*` behavior names (the source-label relativization test was absorbed into `pinvou3-clean` by #59); repo-wide distinct forkguard names went 72→78, with the `codewhale-tui --lib forkguard_` default group at 72 plus 6 under `benchmark-eval-controls`, all passing. Drift: `pinvou3-clean` @`92427bd8d` → topic head `33b23a4a2` is `51 files, +2706/-168` (the old register's `40 files, +1635/-142` did not match reality: the old line r1→`f2526196c` actually measured `47 files, +1877/-158`, and the rebased rewrite carries six follow-up fix commits on top). Multi-root sessions also gain an `Accessible folders:` line in per-turn turn_meta (next to `Current workspace:`, capped at 5 roots with truncation, byte-absent for single-root) — the channel through which the model learns about attached roots. Fingerprints: see `scripts/fork-guard.sh`; note the former `WORKSPACE_ROOTS_SYMBOL` anchor in `sandbox/policy.rs` is replaced by the `writable_roots: codewhale_core::normalize_workspace_roots(workspace, workspace_roots),` anchor in `core/authority.rs`.
