...
### Turn-bound cancel slot: stale stops cannot kill engine self-started follow-up turns (candidate on top of r13, this PR)

- CodeWhale PR #38 (1 commit, head `2239d54a0`): the shared cancel slot upgrades from a bare `CancellationToken` to `TurnCancelSlot { turn_id, token }`; `handle_send_message` and the user `!` shell-command turn mint the turn id first and atomically install the fresh token under that identity. New host entry point `EngineHandle::cancel_turn(turn_id, reason, mode) -> bool` performs the identity check and token clone under the same slot lock the install uses — atomic against a concurrent turn start, and the cloned token can only ever fire the named turn. On mismatch nothing is cancelled and no steer disposition or cancel reason is published. `cancel_with_mode` keeps its semantics for single-user frontends. Closes Pinvou/pinvou-agent#254: the three runtime self-start paths (idle sub-agent completion, background shell wake, goal continuation) swapped the shared token before the host observed `TurnStarted`, so a stale generation-matched stop fired the follow-up turn's token.
- Parent-side adaptation (this PR): gitlink → `2239d54a0`; `EnginePool::cancel` snapshots the (epoch, turn_id) identity from one state lock (`TurnLifecycle::current_turn_identity`) and lets the engine slot arbitrate the cancel; the forwarder's `pending_cancel` replay becomes turn-bound; fork-guard layer 0 gains a candidate-head registration (`CANDIDATE_HEAD`) plus this section's fingerprints. `scripts/verify-public-submodule.sh` stays red for the candidate gitlink until the paired CodeWhale PR merges and the candidate is cut into a release (known candidate-period state).
