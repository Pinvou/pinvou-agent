...
### r14 candidate: adaptive swarm rate-limit scheduling (CodeWhale PR #43, pending release)

- CodeWhale PR #43 (head `6d81cfa83`, review fixes included; candidate base `8fd81f471`) adds rate-limit-aware adaptive scheduling to the sub-agent launch gate. It is the base-side counterpart of parent PR #444 ("swarm mode"): a swarm pins concurrency to the base hard ceilings (128 concurrent / 1024 admitted), which makes parallel 429s a steady state that needs sender-side feedback control.
- `launch_gate` changes from a fixed-capacity `tokio::sync::Semaphore` to a custom `DynamicGate` (`tools/subagent/governor.rs`, runtime-adjustable capacity; a `Semaphore` can only "shrink" by swapping the `Arc`, which silently fails while a permit is held). Waiters receive an *already-counted* permit through a oneshot channel, so a cancelled waiter cannot swallow the wakeup — the permit drops with the future and its `Drop` re-dispatches the slot to the next waiter.
- A new `RateLimitGovernor` (shared per engine, inherited through the `SubAgentRuntime` derive tree; the manager stamps it for every spawn path at the `spawn_background_with_assignment_options` chokepoint): a 60s sliding window tracks the 429 rate with AIMD — ≥2 events or >30% ratio halves capacity, ≥4 events pauses new launches (queued reason `queued: waiting for provider rate-limit recovery`); once limit events age out, capacity recovers at a quarter via consecutive successes or the **time-driven probe** (queued children periodically call `recover_if_window_drained`, so the queue cannot freeze until wall-time deadlines when the in-flight fleet drains). Runtime launch-concurrency changes go through the governor and cannot silently lift a pause.
- 429 retries honor `Retry-After`; without it, full-jitter exponential backoff (250ms start, 120s cap) de-synchronizes the fan-out. `QuotaExhausted` is not treated as a transient throttle and keeps the existing failure path.
- New `forkguard_rate_limit_governor_*` and `forkguard_dynamic_gate_*` behavior tests; the parent registers fingerprints as T5 (`scripts/fork-guard.sh`). After merging, publish tag `pinvou-v0.9.5-r14` and update the parent's `PUBLISHED_HEAD`/`PUBLISHED_COMMITS` (37→39, including the review-fix commit); until then `verify-public-submodule.sh` and fork-guard layer 0 stay red by design during the candidate window.
