...
> Updated: 2026-08-26. Public maintenance branch: upstream `v0.9.5` plus the r11 head merged through CodeWhale PR #29; immutable r11 tag publication is pending. Canonical Chinese register: [`docs/fork-modifications.md`](fork-modifications.md). This English page is a condensed summary; the Chinese version is the complete, authoritative register.
>
...
| Public maintenance branch | `Pinvou/CodeWhale:pinvou3-clean` at final r11 head `665b46cd9e67326459223aa662931bd36d726004`, merged through #29 |
| Merged fixes | `Pinvou/CodeWhale#9`, `#11`, `#12`, `#13`, `#15`, `#16`, `#17`, `#19`, `#27`, and `#29` are merged; #29 supersedes the closed #24/#28 publication attempts |
| Published status | `pinvou3-clean` resolves to the final r11 head; immutable tag `pinvou-v0.9.5-r11` is still pending, so the parent remains Draft and the public verifier remains fail-closed; `r1` through `r10` remain immutable historical tags |
...
| Drift | final r11 maintenance head: 71 files, `+6522/-816` (net 5,706) |
| Organization | Four current long-lived topics; PR #13 removes the product-specific orchestration topic |
| Guard inventory | final r11 maintenance head has 43 CodeWhale `forkguard_*` tests, plus generic tool-compatibility regressions and parent fingerprints/behavior tests |

### r11 Windows shell decoding (tag publication pending)

- `Pinvou/CodeWhale#29` merged the reviewed shell decoder into `pinvou3-clean` at final head `665b46cd9e67326459223aa662931bd36d726004`, superseding #24 and #28. Strict UTF-8 remains authoritative; incomplete sequences survive pipe boundaries; when an invalid sequence is found, the already-validated UTF-8 prefix is preserved and only the invalid suffix enters the Windows ACP decoder; synchronous output, bounded snapshots, raw deltas, tails, and detached readers share the same stateful semantics.
- The merged implementation retains bytes captured before a stable completion cutoff and keeps the Windows-only ACP mapping cross-platform tested without triggering `dead_code` in non-Windows library builds. It also incorporates the dependency-advisory baseline from #27, deterministic output tests, a focused Windows CI job, and the required contribution-gate synchronization. The generic fix is tracked upstream by `Hmbown/CodeWhale#5602`.
- The r10 parent integration from parent PR #322 is already merged; parent PR #348 now points to the final #29 head and waits only for immutable tag `pinvou-v0.9.5-r11`, exact gitlink/tag alignment, and a successful public-submodule verifier before leaving Draft.
...
- CodeWhale PR #29's required contribution gate, Windows shell regressions, metadata, format, advisory, and policy checks pass at final head `665b46cd9e67326459223aa662931bd36d726004`.
- The final head retains the focused output and shell-lifecycle coverage and contains 43 CodeWhale `forkguard_*` tests. The output coverage includes mixed valid UTF-8 plus CP1252 data and platform-independent final incomplete-byte expectations.
- Parent `./scripts/fork-guard.sh --fast`, architecture guard, CI-policy tests, and diff check are rerun against the final gitlink before publication.
- The public-submodule verifier remains fail-closed until immutable tag `pinvou-v0.9.5-r11` resolves to the final gitlink. The parent must remain Draft until that alignment is externally published and verified.
