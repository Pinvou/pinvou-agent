...
1. **Host embedding and routing boundary** — `331cb1594688c723d98499d9ca11f05af291b599`, `2eceab4e19cb0b15576c09d5b89e0d8bc42e11fd` (`Pinvou/CodeWhale#11`), `a36e6cd533024cfe5724bae21875aea42b2ed87a` (`Pinvou/CodeWhale#13`), `8aa5f77d35ac1d00d1f444193543307a7e9b391c` (`Pinvou/CodeWhale#16`), and `07d183e350ce4a1ed4f91bdfa1875c996e710d2b` (`Pinvou/CodeWhale#17`). Exposes only the library modules and narrow host seams needed for Fleet roster loading, live-worker projection, resolved routes, runtime snapshots/recovery, bulk cancellation, terminal facts, reliable steer ownership, and authoritative edit-target classification. Edit rejection cannot call the provider or mutate history, and the app reconciles an optimistic edit from the durable transcript. (Pending release, `Pinvou/CodeWhale#19`) Fixed-sampling routes strip an explicit non-1 `temperature` (otherwise a 400 "only 1 is allowed" breaks compaction): the Kimi Code membership route matched on the exact membership model IDs (`k3`, `k3-256k`, `kimi-for-coding`, `kimi-for-coding-highspeed`, Chat seam), plus a DeepSeek Responses-side compatibility shim limited to the exact `deepseek-v4-flash`; the Chat dialect preserves the documented 0..=2 contract (v4-pro rides the Chat dialect and is verified unrestricted), and gateways and other models keep their own wire contract. Also pending: `CompactionCompleted` carries `post_input_tokens`, the engine's canonical conservative estimate of the complete post-compaction request (system prompt and merged summary included), so hosts can refresh usage display immediately; the TUI and runtime-thread persistence paths do not consume it.
...
- All 37 CodeWhale `forkguard_*` tests pass for the published r9 baseline; the pending fixed-sampling and compaction-estimate changes (`Pinvou/CodeWhale#19`) add 4 more (41 total on the working branch).
