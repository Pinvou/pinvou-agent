...
| History | 36 DCO-signed-off commits above upstream in four long-lived topics (T1–T4) plus two append topics (T5 session archive export, T6 swarm rate-limit governor); all 21 post-r1 commits landed via squash-merged PRs through the five required gates |
...
| `2ab5e64b5` | T3 fix preserving the tool-round boundary across compaction handoff: chat wire role validity checks (compacted rounds keep a legal assistant/tool sequence), recompaction keeps the real user boundary, compacted rounds survive restores, generated compaction summaries are identified, restored-topology wire merging is scoped, pinned by seven forkguard tests (#62) |
| `ce783728c` | T2 fix in the computer-use plugin: the raster frame offset is rebound by the crop region at the parent scale after a zoom (child-raster coordinates no longer resolve against the stale full shot), element state is remembered host-side over ssh with a `state_computer_mismatch` check, and zoom/recording fail closed over ssh with named, actionable reasons (#57) |
| `7fc36e587` | T6 refactor rebuilding `DynamicGate` on a tokio `Semaphore` (cancelled grants are redispatched, stale queued waiters are skipped without leaking a slot, shrinking below outstanding re-admits later), extracting the `is_governor_reported_rate_limit` predicate with a forkguard pin that QuotaExhausted never feeds the governor window, and dropping dead branches with incorrect comments (#55, #43 review follow-up) |
