...
Origin scope: read-only `multi_tool_use.parallel` sub-calls share the wrapper call's origin; subagent jobs carry the launching Task-call origin plus `owner_agent_id`. Consumers must preserve that owner distinction rather than assume a unique origin for every child job.
