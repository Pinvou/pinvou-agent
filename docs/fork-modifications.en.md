...
CodeWhale PR #15 proposes `1eca6103a` plus security follow-ups `169c24cc5` and `21e5f661a`, followed by controlled Provider-profile injection in `214d08819`. The candidate adds a process-local per-turn tool policy, complete trusted-path replacement, an exact final dispatch gate, read-only `File` action schema projection with a repeated read-only check before final execution, and denial of queued goal continuation and MCP reload while restricted. Restricted turns also block queued control-plane operations, hooks, MCP initialization, dynamic tools, and child agents, while tool logs and audits keep only non-private identity fields (event kind, tool name). Trusted local hosts can inject Provider configuration and credentials through a transient stdio pipe. The fast guard accepts only this registered candidate chain (current head `214d08819`, 14 commits above v0.9.5) above the immutable `pinvou-v0.9.5-r7` baseline. Parent CI allows this registered candidate on `pull_request` events but enforces strict public-tag verification in the merge queue: before queueing for merge, the candidate chain must be merged, published as immutable tag `pinvou-v0.9.5-r8`, and the parent gitlink plus `PINVOU_CODEWHALE_TAG` aligned to r8.
...
1. **Host embedding and routing boundary** — `331cb1594688c723d98499d9ca11f05af291b599`, `2eceab4e19cb0b15576c09d5b89e0d8bc42e11fd` (`Pinvou/CodeWhale#11`), and `a36e6cd533024cfe5724bae21875aea42b2ed87a` (`Pinvou/CodeWhale#13`). Exposes only the library modules, narrow root-level Fleet roster API, read-only live-worker projection, opaque resolved-route interfaces, distinct runtime-snapshot versus process-resume session APIs, generic host bulk cancellation, terminal failure facts, and volatile app-server runtime-profile and controlled credential methods required by local hosts. The profile carries route facts and a secure-store reference; API keys cross only the transient stdio credential pipe and are excluded from argv, configuration persistence, and responses. Agent reasoning is projected as `thinking_delta` for host transcript rendering. The full `fleet` module remains private.
2. **Tool compatibility and command-execution safety** — `595adce47e2d1bcf895d7bfd6426c074eb969324`, `3bbf8421ebdb16bff71f83dac4d42c8fb65f0f02` (`Pinvou/CodeWhale#12`), and `a36e6cd533024cfe5724bae21875aea42b2ed87a` (`Pinvou/CodeWhale#13`). Adds host `extra_tools`, dynamic `SetDisallowedTools`, file-size enforcement, fail-closed multiline command safety, schema-bound JSON-container repair, provider-role-safe continuations, canonical registry instructions, and alias-aware Custom SubAgent allowlists while reusing upstream `allowed_tools`. The local PR #15 candidate layers an exact catalog/final-dispatch policy, trusted-root replacement, latched control-plane denial, hook opt-in, and restricted log/audit redaction while preserving the legacy `None` path. The GAIA follow-up explicitly requires read-only dispatch, projects the model-visible `File` schema to read/list/search actions, and repeats the read-only check before final execution. This generic seam is temporarily landing in the public Pinvou fork to unblock the GAIA release and parent merge queue, then will be proposed separately from the latest upstream main; Pinvou's GAIA profiles remain app-owned, and the fork copy will be removed if upstream accepts it.
