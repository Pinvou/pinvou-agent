...
- Integration status: [parent PR #443](https://github.com/Pinvou/pinvou-agent/pull/443) must remain draft. Independent source review accepted the scoped changes, but release integration is blocked: `scripts/verify-public-submodule.sh` requires the official immutable `pinvou-v0.9.12-r1` tag at `1fafee7e26b60a59457a43bce50c63aa2ad9dbaf`, whereas the candidate parent gitlink is `70eba057d4cbc1e13646272c56bb1d9ba0d487ae`. Passing source tests or fork fingerprints does not satisfy this public-release gate.
- Release dependency: an authorized `Pinvou/CodeWhale` maintainer must integrate the re-ported candidate (branch `codex/shell-guidance-v0912`, a re-port of [fork PR #42](https://github.com/Pinvou/CodeWhale/pull/42) onto v0.9.12 r1) and publish an actual new immutable release tag containing the fix. The original `zhuowp` authorship lacks push permission to that official repository. After publication, align the parent gitlink, public-submodule verifier, fork guard, and release inventory with the verified tag and SHA, then rerun the integration checks before marking the parent ready. Do not move the released v0.9.12 r1 tag, invent a future tag, or relax the verifier to accept a contributor ref.
...
- The parent gitlink pins candidate `70eba057d4cbc1e13646272c56bb1d9ba0d487ae` (branch `codex/shell-guidance-v0912`: the released v0.9.12 r1 head `1fafee7e26b60a59457a43bce50c63aa2ad9dbaf` plus one re-ported T2 commit, 16 commits above the official `v0.9.12` tag). Candidate drift above r1: 3 files, +207/-2. This is a reusable upstream contribution; the released v0.9.12 r1 maintenance branch and immutable tag below remain unchanged. Upstream main still contained the login-shell-only description when inspected on 2026-09-05.
