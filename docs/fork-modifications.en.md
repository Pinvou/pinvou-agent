...
| Guard | 72 distinct CodeWhale `forkguard_*` behavior names (registered floor 54, three of them pinned behind the `benchmark-eval-controls` gate) plus parent fingerprints/tests |
...
| `72e98fd89` | T3 fix stopping model-facing text from naming absent tools: the skills-index Usage line and the subagent `## Skills` header teach the two-step `tool_search` activation plus direct-call hydration fallback, the mcp-discovery skill self-conditions its registry commands on availability and drops the retired `exec_shell` alias, `best-of-n` gates `create_goal` on existence, the parent-context hint cites first-turn-active `read`/`bash`, bundled-skills retired/hidden-name sweep, and `MAX_REGISTRY_MATCHES` is compile-time pinned to the "eight" wording (#56) |
