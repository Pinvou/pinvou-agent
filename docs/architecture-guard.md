...
- Rust 超过 1500 行、前端超过 1000 行的现有文件记录精确行数基线：可以拆小，
  但不能继续增长；新文件一旦超过阈值会直接失败。控制台仍输出建议性警告，方便
  定位需要继续拆分的模块。
...
大文件行数同样遵循该棘轮。纯数据或测试文件如果确实需要超过阈值，应优先拆分；
不得通过提高行数基线绕过检查。

PR CI 还会通过 `--base-ref` 对比目标分支中的基线。当前分支不能增加目标分支已有的
额度、增加新条目或扩大依赖环，因此同时修改当前基线不能绕过门禁。首次引入基线时，
目标分支尚无该文件，脚本会明确报告初始化并允许通过。大文件行数规则也允许一次性
按当前实测值初始化；合入后即转为普通棘轮，不能新增文件或提高既有文件额度。
