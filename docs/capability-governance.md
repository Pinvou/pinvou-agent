...
安装/导入路径遵循同一方向的**事务边界**（#517 评审）：同意门同步在包落盘
之后执行，同步被拒即回滚本次刚落地的安装/导入（上传包落入回收站可恢复），
报错说明回滚结果，不允许「已安装且不在禁用集」的中间态暴露；CLI 连接器的
可用化（`*_apply_skills`）先登记 DenyAll 禁用集、后物化技能文件（deny-first），
被拒时连接在技能物化前中止。
