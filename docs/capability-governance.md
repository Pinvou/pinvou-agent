...
安装/导入路径遵循同一方向的**事务边界**（#517 评审）：DenyAll 禁用集登记先于
任何内容落盘或替换既有安装（deny-first）——登记被拒时整个安装/导入在落地前
中止，既不允许「已安装且不在禁用集」的中间态暴露，也绝不因回滚卸载破坏用户
既有的安装（重装/重导会原地覆盖旧副本，事后回滚无法复原）；CLI 连接器的
可用化（`*_apply_skills`）同为先登记 DenyAll 禁用集、后物化技能文件，
被拒时连接在技能物化前中止。deny-first 只作用于**新装**的包 id：已安装包的
重装/更新/重导跳过登记（`Ok`，不产生任何写）——已初始化 scope 的既有条目即
用户的授权记录，重登记既会在成功路径上重置用户的显式开启，也会在门后任何
一步失败（pip 依赖、磁盘满、内容冲突、远程校验）时把原本可用的安装静默禁用
且无恢复路径（与 `update_marketplace_skill`「更新保留用户启用状态」同契约）；
新装失败留下的惰性禁用条目方向仍 fail-closed，且随下次成功安装自然收敛。
