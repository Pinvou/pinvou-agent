...
| DMG compression | tauri emits UDZO without zlib-level (hdiutil default level 1); converted to **ULMO** (LZMA) after the build (`scripts/convert-dmg-ulmo.sh`; a failed conversion degrades to keeping the original dmg; mounting needs macOS 10.15+; our floor is 11.0) | `release-packages.yml` macOS job + `scripts/release-macos.sh` |
