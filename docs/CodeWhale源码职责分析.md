...
1. CodeWhale 在 pinvou3 中承担哪些底座职责。
2. pinvou3-app 如何从 Tauri/Rust 适配层接入 CodeWhale 的 Engine、Session、Tool、Skill、MCP、Hooks、Compaction 和 SubAgent 能力。
3. Windows 维护、合并、构建、打包和排查时，应优先看哪些源码区域和检查项。

本文不做这些事：

- 不替代 CodeWhale 官方架构文档，也不逐行翻译上游源码。
- 不修改 `CodeWhale/` 或 `pinvou3-app/` 业务代码。
- 不重新设计 agent runtime、工具注册、会话存储、工作流调度或打包方案。
- 不覆盖所有历史方案；分析以当前仓库检出的 `CodeWhale` 子模块和 `pinvou3-app/src-tauri/src/` 接入方式为准。

## 一句话定位

CodeWhale 是 pinvou3 的 agent 底座，负责模型调用、Engine 循环、工具注册与执行、流式事件、Session、Skill、Commands、MCP、Hooks、SubAgent、Compaction 等运行时能力。

pinvou3-app 是 Tauri UI、Rust wrapper、配置翻译和状态适配层：它把 `~/.pinvou3` 下的用户设置、会话目录、工作流目录、bundle prompt、搜索配置和 Windows 桌面事件翻译为 CodeWhale 可消费的 `EngineConfig`、`Op` 和事件回传。

## 源码全景

### CodeWhale 工作区

当前 `CodeWhale/Cargo.toml` 是 Rust workspace，版本为 `0.8.60`，edition 为 `2024`，`rust-version` 要求 `1.88`。workspace 成员如下：
...
| `crates/cli` | CodeWhale CLI 入口 | 当前未直接接入；pinvou3 走 Tauri app |
...
### 当前直接接入的 CodeWhale 能力

- `CodeWhale/crates/tui/src/core/engine.rs`：定义 `EngineConfig`、`EngineHandle`、`spawn_engine`，pinvou3-app 通过它创建每个 session 独立 Engine。
- `CodeWhale/crates/tui/src/core/ops.rs`：定义 `Op::SendMessage`、`Op::SyncSession`、`Op::SpawnSubAgent`、`Op::CompactContext`、`Op::Shutdown` 等操作。
- `CodeWhale/crates/tui/src/core/events.rs`：定义 `Event::MessageDelta`、`ToolCallStarted`、`ToolCallComplete`、`TurnComplete`、`AgentComplete`、`CompactionStarted` 等事件。
- `CodeWhale/crates/tui/src/session_manager.rs`：定义 `SessionManager`、`SavedSession`、`SessionMetadata`，pinvou3 包装后定向到 `~/.pinvou3/sessions/`。
- `CodeWhale/crates/tui/src/skills/mod.rs`：定义 `SkillRegistry`，pinvou3 用它发现 bundle skills。
- `CodeWhale/crates/tui/src/tools/registry.rs`：定义 `ToolRegistry` 和 `ToolRegistryBuilder`，底座负责统一注册、过滤、执行工具。
- `CodeWhale/crates/tui/src/compaction.rs`：定义 `CompactionConfig` 和压缩逻辑，pinvou3 只调整本地模型和阈值。
- `CodeWhale/crates/hooks/src/lib.rs`：定义 `HookEvent`、`HookDispatcher` 等 hook 机制，pinvou3 在 bridge 中生成 hook 配置。
- `CodeWhale/crates/mcp/src/lib.rs`：定义 `McpManager` 与 MCP 工具/资源结构，底座经工具体系统一接入。
- `CodeWhale/crates/tui/src/tools/pinvou3_blocklist.rs`：fork 中的 pinvou3 工具隐藏清单，`lib.rs` 有 guard 测试防止合并时误删。

## pinvou3 接入边界

### bridge：唯一配置翻译层

`pinvou3-app/src-tauri/src/bridge/mod.rs` 是 pinvou3-app 和 CodeWhale 的配置边界。它负责：
...
| 场景 | 入口 | pinvou3 适配层 | CodeWhale 能力 | 输出或副作用 |
|---|---|---|---|---|
...
| 底座能力 | CodeWhale 已做的事 | pinvou3 推荐扩展方式 |
|---|---|---|
...
| Commands | CodeWhale 有 slash command 和 TUI command 体系 | pinvou3 的 Tauri command 只做桌面 UI 入口；底座命令能力不要复制一份 |
...
| 证据点 | 路径或类型 | 说明 |
|---|---|---|
| 1 | `CodeWhale/Cargo.toml` | workspace、版本 `0.8.60`、Rust `1.88`、crate 成员 |
| 2 | `CodeWhale/crates/tui/src/core/engine.rs::EngineConfig` | Engine 配置主结构 |
| 3 | `CodeWhale/crates/tui/src/core/engine.rs::EngineHandle` | pinvou3 持有并发送 `Op` 的 runtime handle |
| 4 | `CodeWhale/crates/tui/src/core/engine.rs::spawn_engine` | pinvou3 创建 Engine 的底座工厂 |
| 5 | `CodeWhale/crates/tui/src/core/ops.rs::Op` | SendMessage、SyncSession、SpawnSubAgent、CompactContext、Shutdown |
| 6 | `CodeWhale/crates/tui/src/core/events.rs::Event` | delta、tool、turn、compaction、agent 事件源 |
| 7 | `CodeWhale/crates/tui/src/tools/registry.rs::ToolRegistry` | 工具注册和执行中心 |
| 8 | `CodeWhale/crates/tui/src/tools/registry.rs::ToolRegistryBuilder` | native/MCP/agent 工具组装入口 |
| 9 | `CodeWhale/crates/tui/src/skills/mod.rs::SkillRegistry` | `SKILL.md` 发现与解析 |
| 10 | `CodeWhale/crates/tui/src/session_manager.rs::SessionManager` | session 持久化和加载 |
| 11 | `CodeWhale/crates/tui/src/compaction.rs::CompactionConfig` | 上下文压缩配置 |
| 12 | `CodeWhale/crates/mcp/src/lib.rs::McpManager` | MCP server/tool/resource 管理 |
| 13 | `CodeWhale/crates/hooks/src/lib.rs::HookDispatcher` | hook 事件广播 |
...
| 16 | `pinvou3-app/src-tauri/src/engine.rs::spawn_event_forwarder` | CodeWhale 事件到 Tauri 事件的转译 |
...
| 检查项 | 识别方式 | 建议处理 |
|---|---|---|
| 子模块版本不匹配 | `git submodule status --recursive`；前缀 `-` 表示未初始化，`+` 表示检出提交与父仓记录不一致 | 先执行 `git submodule update --init --recursive CodeWhale`，再构建 |
| CodeWhale API 字段变化 | `bridge/mod.rs::build_engine_config` 编译失败，提示 `EngineConfig` 缺字段或多字段 | 不要用 `..Default::default()` 绕过；逐字段判断覆盖或透传 |
| `Cargo.lock` drift | 合并后 `pinvou3-app/src-tauri/Cargo.lock` 或 CodeWhale 依赖变化 | 用同一 Rust/Cargo 环境重新构建，确认 lock 变化是预期 |
| Rust 工具链 | `CodeWhale/Cargo.toml` 要求 `rust-version = "1.88"`；旧 rustc 会报版本或特性错误 | Windows 上可用最新 stable，但必须满足 `1.88+` |
...
| 搜索 provider/API key | `bridge/mod.rs` 将 pinvou3 prefs 翻译为 CodeWhale `SearchProvider` 和 `search_api_key` | 搜索不可用时先核对 provider 与 key 是否符合底座要求 |
| 工具隐藏清单 | `lib.rs` 中 `blocklist_contract` 测试，`CodeWhale/crates/tui/src/tools/pinvou3_blocklist.rs` | 合并 fork 后若 tool schema 膨胀，先看 blocklist 是否丢失 |
...
| 编译失败 | `pinvou3-app/src-tauri/Cargo.toml`、`Cargo.lock`、`CodeWhale/Cargo.toml`、`bridge/mod.rs::build_engine_config` | 先看子模块提交和 Rust 版本，再看上游字段变更 |
...
| 工具不可用 | `CodeWhale/crates/tui/src/tools/registry.rs`、`tool_setup.rs`、`pinvou3_blocklist.rs`、`bridge/mod.rs` features | 判断是工具未注册、被隐藏、feature 未开、还是 MCP/server 配置问题 |
...
| MCP 不可用 | `mcp_config_path`、`CodeWhale/crates/mcp/src/lib.rs`、`ToolRegistryBuilder` MCP adapter | 先看 MCP 配置路径，再看 server 启动状态和工具是否进入 registry |
| Hook 行为异常 | `bridge/mod.rs::build_hooks_config`、`CodeWhale/crates/hooks/src/lib.rs` | 检查 hook event 类型、sink、是否由 bridge 正确配置 |

## 验收清单

- [x] 中文主体：本文除技术名词、路径、类型和命令外均使用中文说明。
- [x] 定位清楚：已说明 CodeWhale 是 agent 底座，pinvou3-app 是 Tauri UI、Rust wrapper、配置/状态适配层。
- [x] 源码模块：已覆盖 `CodeWhale/Cargo.toml` workspace 和 16 个 crate。
