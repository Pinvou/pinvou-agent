...
> 最后更新：2026-08-21（公开维护基线：上游 `v0.9.5` r8；PR #15 已将逐轮工具安全扩展发布到现有 4 个 Pinvou 主题）
...
- `Pinvou/CodeWhale#15` 已 squash 合并为 `d127aed113529dc93754d044b9f352e9746f6b83`；`pinvou3-clean` 与固定标签 `pinvou-v0.9.5-r8` 均公开可达并指向该提交，`r1` 至 `r8` 保持不可变。
...
- r8 公开基线相对 `v0.9.5` 为 `+3222/-471，48 文件`，净增 2751 行。新增量主要来自逐轮工具权限、只读 schema/最终分发复检及其安全回归；既有保留量来自 Automation 持久化、会话恢复生命周期、工具兼容和嵌入上下文密封。后续优先上游化通用逐轮权限、宿主接口、会话快照/恢复 API 和 Automation 生命周期修复。
