...
- 不设题目级墙钟超时：官方 GAIA 协议未定义运行时长上限（据本仓库现状转述，非引用官方规程原文），运行由引擎自身边界收束（每轮 2,000 步模型步数预算为主轮与子代理共用的宿主默认值，而非对每次调用的硬性封顶：`agent` 调用显式传入 `max_steps` 时仍按底座语义处理——正值钳到同一 2,000 硬上限，0 为底座的无界哨兵；每轮 3,600 秒墙钟、取消机制；主轮步数上限可经 settings.json 的 `advanced.max_steps` 调大，3,600 秒墙钟在该路径为固定默认值，暂无配置入口)。注意该边界只覆盖 turn 本身：harness 侧的 `prepare`/`resolve_output`/`close` 阶段没有各自的截止时间，若这些本地阶段挂死，`pinvou benchmark run gaia` 会一直等待直到操作者手动终止（旧的 600 秒题目级 deadline 恰好覆盖过这类失败）。
