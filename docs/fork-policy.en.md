...
> Updated: 2026-09-20. Upstream `v0.9.12` r1; transition state: the parent gitlink advances along the maintenance branch ahead of the r1 tag and currently points at the T8 commits `9f1210d31`/`b5d9fa7c8`/`b11c12720` above the maintenance-branch head `7fc36e587`; the maintenance branch has since merged #66 (`c4e6caf9`), so upstream #65 must land commit-preserving and the parent re-pins to the new maintenance-branch head right after, until the r2 closure realigns them.
...
- Current fork baseline: `Pinvou/CodeWhale:pinvou3-clean` at head `7fc36e587a91bf400a38a452653933c347f699ca`, with thirty-six DCO-signed-off commits; the immutable tag `pinvou-v0.9.12-r1` stays pinned at the r1 closure `1fafee7e26b60a59457a43bce50c63aa2ad9dbaf` (fifteen commits formed through CodeWhale PR #44 and fast-follow PR #46), followed by twenty-one squash-merged PRs: thirteen from the 2026-09-10/11 backlog batch, five from the 2026-09-17 batch (#56/#58/#59/#60/#61), and three from the 2026-09-18 batch (#62/#57/#55). Three more T8 commits `9f1210d31`, `b5d9fa7c8`, and `b11c12720` (roster presentation of host profiles, the `profile_query` discovery filter, and review-pinned coverage gaps, currently the upstream #65 branch head) sit on top, landing through upstream CodeWhale #65 with the swarm phase-2 parent PR. #65 must land preserving its commits (no squash, or the pinned commits lose public reachability and the parent gitlink dangles); the maintenance branch has since merged #66 (`c4e6caf9`), so #65 can no longer land as a fast-forward onto the gitlink — once it merges, the maintenance-branch head advances past `b11c12720` to a merge commit, and the parent must follow with one re-pin commit aligning the gitlink and the `fork-guard.sh` constants (`EXPECTED_HEAD`/`EXPECTED_COMMITS`) to the new head (recount with `git rev-list --count dcd4c200..<new head>`); only then does the transition assertion (gitlink = maintenance-branch head) hold again.
