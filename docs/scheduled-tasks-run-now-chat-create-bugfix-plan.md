...
> 状态注记：本文部分路径写于 2026-07 目录重组前，已按 2026-09 现行布局更新；仓库现行布局见 `pinvou3-app/src/ARCHITECTURE.md`。

## 背景

写作时（2026-07 前）本文定位了两个问题，**二者均已修复落地**（模型绑定经
`model-bindings.json` + `ModelIdResolver` 落地、聊天创建经
`SCHEDULED_TASK_CHAT_PROMPT` + bridge 强制 `createScheduledTask` 落地），
以下保留方案与测试设计作为回归依据与设计说明：
...
run_scheduled_task_now
  ...
  -> AutomationManager::run_now（app 侧经 run_now_shared 进入）
...
```text
此任务绑定的 AI 模型配置不唯一，请重新选择 AI 模型并保存任务。模型：<model>
```
...
为了尽量减少 CodeWhale fork 改动，优先在 app 层新增 sidecar 存储，而不是直接扩展底座 `AutomationRecord`。
...
- `pinvou3-app/src-tauri/src/features/scheduled/stores.rs`：`ScheduledTaskModelBindingStore`（现行实现为 `VersionedJsonStore<ScheduledTaskModelBindingRegistry>` 的 type alias，非独立结构体）
- `pinvou3-app/src-tauri/src/features/scheduled/tasks.rs`：`ScheduledTaskState` 持有该 store（`model_bindings` 字段）
...
```text
~/.pinvou3/sessions/<session-id>.json
```
...
```text
~/.pinvou3/automations/automations/*.json
```
...
```text
pinvou3-app/src-tauri/src/features/scheduled/tasks.rs
```
...
- 支持时间规则仅限：
  ...
  - 每天指定时间
  - 每周指定星期和时间
  - 一次性定时：`FREQ=ONCE;AT=<本地时刻>`，`AT` 使用 `YYYY-MM-DDTHH:MM` 本地时刻（不带 Z 或时区偏移后缀），运行一次后任务自动结束；用户指定的时刻已过期时，必须先和用户确认改成未来的时刻，不要输出草稿
...
```text
pinvou3-app/src/platform/tauri/bridge/scheduled.js
```
...
（时点注：本小节工具名已按现行工具面更新为 `bash`/`write`/`edit`
（2026-07 前的旧拼写已彻底退役，见 `docs/tool-governance.md` §3）；
实际落地采取了更强的路线——创建会话的 prompt 直接禁止调用任何工具，
见已落地的 `SCHEDULED_TASK_CHAT_PROMPT`。）
...
3. 后端命令测试（`scheduled_session_metadata_dispatch_supports_rename_pin_archive`，`pinvou3-app/src-tauri/src/app/commands/tests.rs`，已实现）：
...
> **【已废弃】** 从本节起至“### 范围边界”止，是问题 3 定稿前的“行级任务
> 操作”（行级更多菜单/置顶分组）旧方案，已被上文问题 3 的结论取代——
> 行级不提供更多菜单、不按置顶/普通任务分组、详情底部操作组保留。以下
> 仅作决策留档，不要按此实施（例外：其中“后端数据设计”一节的
> `task-ui-metadata.json` sidecar 与 `set_scheduled_task_pinned` 后端命令
> 已落地，仍为现行实现；废弃仅指行级菜单/置顶分组的前端交互方案）。
> 块内已带【已废弃】小节标记；问题 3 结论中
> “侧边栏空间不足”仍是唯一未解决的开放项。
...
不建议为置顶直接改 CodeWhale 的 `AutomationRecord`，避免扩大 fork 差异。推荐新增 app 层 sidecar：
...
#### 3. 行级菜单组件【已废弃：行级菜单方案未采纳，见问题3结论与 §五.1 标注】
...
```text
pinvou3-app/tests/scheduled_tasks_unit.test.js
```
...
```text
pinvou3-app/src-tauri/src/features/scheduled/tasks.rs
pinvou3-app/src-tauri/src/features/scheduled/executor.rs
pinvou3-app/src-tauri/src/features/assistant/engine_pool.rs
```
...
2. `ScheduledTasksView.jsx` 包含：
   ...
   【已被问题3结论取代】行级菜单方案已废弃，现行断言为这些 testid 不存在（见上文问题3“自动化测试补充”与 `pinvou3-app/tests/scheduled_tasks_unit.test.js`）。
3. 【已废弃】行级操作按钮都调用 `event.stopPropagation()`，避免误打开编辑弹窗。（行级菜单方案已废弃，同上。）
4. 【已废弃】删除确认组件被行级菜单和详情菜单复用，而不是两套删除弹窗。（行级菜单方案已废弃，同上。）
5. `pinvou3-app/src/platform/tauri/bridge/scheduled.js` 暴露：
...
```js
// 前四条对应问题3结论：行级菜单方案已废弃，现行断言为 testid 不存在
// （见 tests/scheduled_tasks_unit.test.js）：
assert.ok(!/data-testid="scheduled-task-pin"/.test(indexHtml));
assert.ok(!/data-testid="scheduled-task-actions"/.test(indexHtml));
assert.ok(!/data-testid="scheduled-task-action-menu"/.test(indexHtml));
assert.ok(!/data-testid="scheduled-detail-actions"/.test(indexHtml));
...
> 【已废弃】场景 D–G 基于已废弃的行级菜单方案，已被问题3结论取代：主列表行级不显示置顶按钮和更多菜单，现行断言为相应 testid 不存在（见上文问题3“自动化测试补充”与 `pinvou3-app/tests/scheduled_tasks_unit.test.js`）。以下保留作为历史叙述。
...
> 【已废弃】本小节的行级置顶/菜单检查项随行级菜单方案废弃（见问题 3 结论），
> 现行 UI 无这些控件；可访问性要求以 `ScheduledTasksView.jsx` 现行实现为准。
...
> 注：清单 17–19 所述任务行 `...` 行级菜单交互已随行级菜单方案废弃（见问题3结论与 §五.1 的【已被问题3结论取代】标注），现行删除入口以 `ScheduledTasksView.jsx` 现行实现为准；任务置顶为后端已落地能力（sidecar + `set_scheduled_task_pinned`），当前 UI 暂无置顶入口。
...
14. （暂不可手动执行：置顶后端已落地，但当前 UI 无置顶入口，能力由
    `scheduled_session_metadata_dispatch` 单测覆盖。）
15. （待置顶入口落地后回归：重启 app，确认置顶仍保留。）
16. （待置顶入口落地后回归：取消置顶，确认任务回到普通分组。）
...
作用于某一次运行的会话，复用 `RecentItem` 与**普通 Session 命令**。后端 `SessionKind` 分发（`pinvou3-app/src-tauri/src/app/commands/sessions.rs`），前端不做命令分叉：
...
```text
pinvou3-app/src/app/main.jsx
pinvou3-app/src/components/layout/NavigationComponents.jsx
pinvou3-app/src/features/scheduled/ScheduledTasksView.jsx
pinvou3-app/src/platform/tauri/bridge.js
pinvou3-app/src/platform/tauri/bridge/scheduled.js
```
...
说明：`cargo check --locked` 不编译 `#[cfg(test)]`，要覆盖新增的后端命令测试需 `cargo check --locked --tests`。本机实际运行 Rust 测试二进制可能撞 `STATUS_ENTRYPOINT_NOT_FOUND`，根因与缓解步骤见 `docs/windows-rust-ci.md`（§What the job runs 第 6-9 步）。
...
4. 不要给侧边栏记录列表加二级索引或汇总存储。条数上限由后端既有 retention 兜底；前端加载走 `loadScheduledTaskRecentRuns()` 的单次 `list_scheduled_runs`（一次 reconcile + 过滤 + 倒序），见交接说明 §三.3。
