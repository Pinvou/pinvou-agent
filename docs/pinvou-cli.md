...
- Exit codes: `0` success, `1` host/runtime failure, `2` usage error. Exit
  ...
  - exit 2 (usage class) in the shared helpers: missing
    ...
    `scheduled`, and `voice --draft-file` (where every other input-content
    error in those same files is exit 1), plus state-dependent model-store
...
- Session eviction side effects: any store-opening command across the
  ...
  GUI chat sessions when the chat bucket is over cap, with the evicted
  ids reported on the headless runner's stderr. The `knowledge`
  family never boots it: `collections delete` does its job without the
  ...
  command cannot evict sessions on its way to saying so.
