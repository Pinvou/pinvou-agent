...
| `pinvou deps` | `check`, `install <NAME...> --yes` | External dependency detection/installation (apt/Homebrew/bundled). On Windows an already-installed LibreOffice still reports `Installed: libreoffice` (the lib returns a no-op success). |
...
- The five vendor-CLI spawn/drive sites (connectors `ensure`/`connect`/drain,
  voice ASR, code `login`) each carry their own drain/cap/kill plumbing;
  consolidating them behind a shared `run_vendor_cli` helper is a tracked
