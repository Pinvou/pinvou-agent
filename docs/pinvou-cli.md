...
- `--output human|json` (global flag): `human` prints short labeled lines;
  ...
  subcommand that takes its own `--output PATH` (e.g. `sessions export`)
  accepts any other value as that flag's argument, and a parser with no
...
- Building the `pinvou-cli` workspace with `--no-default-features` does not
  ...
  ["product-backend"]` is what gates `dep:anyhow`, and the `knowledge.rs`
  family module references `anyhow` with no `cfg` guard of its own (the
  other later family modules — `scheduled.rs`, `voice.rs`, `monitor.rs` —
  deliberately never name it for exactly this reason). The corollary is that
