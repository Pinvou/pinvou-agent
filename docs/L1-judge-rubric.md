...
- **工具使用合理性看 timeline**:
  ...
  - **白名单外工具出现** → 默认工具使用 ≤2 分（`rlm`/`tasks`/`automation`/`github`/`Run` 等未放出家族，以及 `git_*`/`apply_patch`/`exec_shell*` 等 v0.9.5 已退役旧名——模型可见面以 `pinvou3-app/src-tauri/src/features/assistant/tool_policy.rs` 的 `PINVOU3_ALLOWED_TOOLS` 白名单为准，完整机制见 `docs/tool-governance.md`）
