...
## 0. 当前状态（2026-09-22 · r2 已收口：tag `pinvou-v0.9.12-r2` 切在 `6f780290f`；gitlink 暂指其上 T8 head `4767c2816`（上游 #65 分支头），r1 基线之上 24 个登记提交 + 5 个 T8 待合并提交）
...
| `4767c2816` | T8 修复 | `load` 的 `[fleet.profiles]` 层在 `merge_member` 之前应用与 `from_host_config` 同方向的可混淆 id 去重（首中保留）：名册列表（引擎安装的 `load` 名册）与 spawn 刷新（`from_host_config`）对 `Expert`/`expert` 类同键变体不再可能分叉；接线级回归经 `FleetRoster::load` 装配工具并走真实 `refresh_spawn_route_sources`（`agent_roster_listing_and_spawn_agree_on_conflatable_config_ids` 钉定），`ROSTER_HOST_PROFILE_LIMIT=48` 字面钉定（随蜂群二期 PR 合入维护分支） |
