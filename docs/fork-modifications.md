...
- drift：`pinvou3-clean` @`92427bd8d` → 本主题 head `88405ffbc` 为 `54 files, +3278/-240`（第 6 轮登记 `51 files, +2697/-161`；更早登记 `40 files, +1635/-142` 与实际不符：旧线 r1→`f2526196c` 实为 `47 files, +1877/-158`）；新增 6 条 `forkguard_workspace_roots_*` 行为名（source 相对化那条已被 #59 收进 `pinvou3-clean`），全仓独立 forkguard 行为名 72→78，`codewhale-tui --lib forkguard_` 默认组 72 条 + benchmark 组 6 条运行通过。另含 turn_meta 列根：多根会话的每回合元数据新增 `Accessible folders:` 行（紧贴 `Current workspace:`，最多 5 个超出截断，单根逐字节无此行）——模型获知附加根的通道。
