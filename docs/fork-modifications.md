...
## 0. 当前状态（2026-09-20 · r1 基线 + workspace_roots 主题；73 个提交，r2 收口未切 tag）
...
| 发布状态 | 过渡期（fork-policy 第 0 节豁免）：不可变 tag `pinvou-v0.9.12-r1` 保持 r1 收口状态（15 个提交），父仓 gitlink 指向 workspace_roots 主题 head `c09b6c81a`、领先 tag 49 个提交（过渡期叠层：gitlink = `pinvou3-clean` @`7fc36e587` + 本主题 28 个提交；CodeWhale #54 squash 合入、`pinvou3-clean` 推进后即回收为分支头），直至下一次 r2 发布收口对齐 |
| 升级前回退点 | 公开不可变 tag `pinvou-v0.9.5-r13` → `f853f8f1566c57e6be40d5439a222a932aa79ef5`；同 SHA 的本地 `backup/pre-v0.9.12-sync` 仅作便利引用 |
| 历史组织 | 上游之上 64 个带 DCO sign-off 的提交（其中 28 个为 workspace_roots 主题、现居 fork 分支 `pinvou3/workspace-roots-v12`，待 CodeWhale #54 squash 合入），归属 4 个长期主题（T1–T4）+ 2 个追加减量主题（T5 会话归档导出、T6 蜂群限流治理）+ 1 个已合入维护分支的主题（T7 压缩检查点角色兼容）+ 1 个过渡期在册主题（workspace_roots，见下节）；r1 之后 49 个提交全部经 PR 审查，合入维护分支者均过五项必需门禁 |
| drift | `180 files, +13634/-1592`，净增 12042 行（维护分支对上游）；过渡期另加本主题 `61 files, +4971/-289`（`7fc36e587` → `c09b6c81a`）；r1 为 `94 files, +5022/-944`，旧 r13 为 `110 files, +10895/-1195` |
| 守护 | 96 条独立 CodeWhale `forkguard_*` 行为名（登记下限 96，其中 6 条钉在 `benchmark-eval-controls` 门控面、6 条为本主题新增的 `forkguard_workspace_roots_*`）+ 父仓指纹与行为测试（workspace_roots 指纹锚点已随本 PR 注册进 `scripts/fork-guard.sh`） |
...
### 多根工作区 workspace_roots + 指令 source 相对化（2026-09-19 已再 rebase，CodeWhale PR #54 head `c09b6c81a`）

- CodeWhale 分支 `pinvou3/workspace-roots-v12`（`qiuYliangM/CodeWhale` fork，PR #54 head `c09b6c81a`；已 rebase 到 `pinvou3-clean` @`7fc36e587` 之上——即官方维护线当前头 + 28 个主题提交（第 8 轮修复与登记对齐提交，历史无 force-push 残留；更早的 `88405ffbc`（rebase 到 `92427bd8d` 的 23 提交线）与 10 提交中文信息线 `f2526196c` 均已被本线取代））：线程从单根 `cwd` 扩展为 **cwd（主根）+ workspace_roots（全量可访问根集合）**，是"单入口工作区"（项目 = 主文件夹 + 一组钥匙）的底座前提。四层落地：
...
- 行为变化边界：未配置多根（空集合）时协议帧、策略值、路径判定逐字节等价单根现状；TUI 交互端无多根 UI，根集合只能经 Runtime API/headless 进入。第 7 轮收口：`ThreadForkParams.workspace_roots` 改为 `Option<Vec>`，裸 `thread/fork` 继承父线程集合（cwd 换主根槽位、附加根保留，`Some([])` 为显式清空），不再退化到 `[fallback_cwd]`；app-server 桥的 runtime 线程创建带全量根集合；repo law 逐根判定；`/save`、`/rename`、导出导入、`/fork <id>` 全部与主根成对落盘。
- drift：`pinvou3-clean` @`7fc36e587` → 本主题 head `c09b6c81a` 为 `61 files, +4971/-289`（第 8 轮在上一次登记 `54 files, +3278/-240` 之上补齐 ACP 会话/加载通道、`/title` 写路径、repo law 逐根判定与证据非空洞化等修复）；新增 6 条 `forkguard_workspace_roots_*` 行为名（source 相对化那条已被 #59 收进 `pinvou3-clean`），全仓独立 forkguard 行为名 90→96，登记下限随本 PR 的 `scripts/fork-guard.sh` 提到 96。另含 turn_meta 列根：多根会话的每回合元数据新增 `Accessible folders:` 行（紧贴 `Current workspace:`，最多 5 个超出截断，单根逐字节无此行）——模型获知附加根的通道。
...
- 候选登记已回收：#58 落地后父仓 gitlink 随登记批次重钉（回收时为 `92427bd8d`，`EXPECTED_COMMITS` 相应推进），`CANDIDATE_HEAD`/`CANDIDATE_COMMITS=29` 机制移除；其后 #552 批次已把登记头推进至 `7fc36e587`（`EXPECTED_COMMITS` 36，见第 0 节表格）；指纹层保留底座回显契约回归与父仓超越序回归两条锚点；`verify-public-submodule.sh` 过渡期口径为断言 gitlink = `pinvou3-clean` 分支头：本 PR 叠层期间（gitlink = 主题 head `c09b6c81a`）该门禁保持红并在 PR 描述中披露，待 CodeWhale #54 squash 合入、`pinvou3-clean` 推进后重钉恢复绿。
