...
| 发布状态 | 过渡期（fork-policy 第 0 节豁免）：不可变 tag `pinvou-v0.9.12-r1` 保持 r1 收口状态（15 个提交），父仓 gitlink 指向 workspace_roots 主题 head `c09b6c81a`、领先 tag 58 个提交（过渡期叠层：gitlink = `pinvou3-clean` @`7fc36e587` + 本主题 37 个提交；CodeWhale #54 squash 合入、`pinvou3-clean` 推进后即回收为分支头），直至下一次 r2 发布收口对齐 |
...
| 守护 | 106 条独立 CodeWhale `forkguard_*` 行为名（登记下限 96，其中 6 条钉在 `benchmark-eval-controls` 门控面、16 条为本主题新增：6 条 `forkguard_workspace_roots_*` 与 10 条 PowerShell 编码回退/外壳文本匹配）+ 父仓指纹与行为测试（workspace_roots 指纹锚点已随本 PR 注册进 `scripts/fork-guard.sh`） |
...
- drift：`pinvou3-clean` @`7fc36e587` → 本主题 head `c09b6c81a` 为 `61 files, +4971/-289`（第 8 轮在上一次登记 `54 files, +3278/-240` 之上补齐 ACP 会话/加载通道、`/title` 写路径、repo law 逐根判定与证据非空洞化等修复）；新增 16 条行为名：6 条 `forkguard_workspace_roots_*`（source 相对化那条已被 #59 收进 `pinvou3-clean`）与第 9 轮 PowerShell 编码回退/外壳文本匹配 10 条，全仓独立 forkguard 行为名 90→106，登记下限随本 PR 的 `scripts/fork-guard.sh` 提到 96。另含 turn_meta 列根：多根会话的每回合元数据新增 `Accessible folders:` 行（紧贴 `Current workspace:`，最多 5 个超出截断，单根逐字节无此行）——模型获知附加根的通道。
