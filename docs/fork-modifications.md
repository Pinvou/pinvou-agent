...
- 候选登记（本 PR 评审期）：父仓 gitlink 暂钉上述候选头 `064ac6e19`；`scripts/fork-guard.sh` 第 0 层按 `CANDIDATE_HEAD`/`CANDIDATE_COMMITS=29` 接受（登记头 28 + 1 个候选提交），指纹层新增底座回显契约回归与父仓超越序回归两条锚点；`verify-public-submodule.sh` 在候选期保持红（gitlink ≠ `pinvou3-clean` 公开维护分支头），为登记的候选期预期状态而非回归；候选并入 `pinvou3-clean` 后重钉 gitlink 并回收本登记。
