...
## 0. 当前状态（2026-09-18 · r1 基线 + 19 个登记提交，其中 1 个随本 PR 候选，r2 收口未切 tag）
...
| 候选 gitlink | 本 PR 候选 head `24f63bba46756f68b4d50b2793a988a431395a25`（维护分支 `92427bd8d` 之上 1 个提交：T2 Windows PowerShell 执行策略兼容；上游之上 34 个、r1 收口之上 19 个），合入维护分支后回归维护分支口径 |
| 发布状态 | 过渡期（fork-policy 第 0 节豁免）：不可变 tag `pinvou-v0.9.12-r1` 保持 r1 收口状态（15 个提交），维护分支在 `92427bd8d`（领先 tag 18 个提交），本 PR 的父仓 gitlink 指向候选 `24f63bba4`、领先 tag 19 个提交，直至下一次 r2 发布收口对齐 |
| 升级前回退点 | 公开不可变 tag `pinvou-v0.9.5-r13` → `f853f8f1566c57e6be40d5439a222a932aa79ef5`；同 SHA 的本地 `backup/pre-v0.9.12-sync` 仅作便利引用 |
| 历史组织 | 上游之上 34 个带 DCO sign-off 的提交，归属 4 个长期主题（T1–T4）+ 2 个追加减量主题（T5 会话归档导出、T6 蜂群限流治理）；r1 收口之后 19 个提交中，18 个已经 PR squash 合入维护分支并过五项必需门禁，1 个（T2 Windows PowerShell 执行策略兼容）随本 PR 候选待合入 |
| drift | `169 files, +11803/-1477`，净增 10326 行；r1 为 `94 files, +5022/-944`，旧 r13 为 `110 files, +10895/-1195` |
...
- `powershell_build_command_includes_no_profile_and_command_flags`（两条分支都带 `-ExecutionPolicy Bypass`）
- `powershell_encoded_fallback_avoids_the_script_file`（payload 可反解回原命令与 `$LASTEXITCODE` 尾巴）
...
- 不可变 `pinvou-v0.9.12-r1` 停在 r1 收口 `1fafee7e26b60a59457a43bce50c63aa2ad9dbaf`；当前处于过渡期：`pinvou3-clean` 维护分支在 `92427bd8d`（领先 tag 18 个 squash 提交），本 PR 的父仓 gitlink 指向候选 `24f63bba4`（维护分支之上 1 个提交，领先 tag 19 个），该提交合入维护分支后 gitlink 回归分支头。下一次 r2 发布收口时在合并头切不可变 tag 并对齐三方（分支/tag/gitlink），以 `scripts/verify-public-submodule.sh` 验证公开可达性（过渡期断言 tag 钉在 r1 收口、gitlink 沿维护/候选分支领先，r2 收口后恢复三方相等；依据 `docs/fork-policy.md` 第 0 节过渡期豁免）。
