...
| 发布状态 | r2 已收口（2026-09-21，main 经 #574 完成）：不可变 tag `pinvou-v0.9.12-r2` 切在 `6f780290f`，main 上维护分支/gitlink/tag 三方相等，但 r2 不含 workspace_roots 主题；本 PR 继续过渡期叠层（fork-policy 第 0 节豁免）：父仓 gitlink 钉主题 head `282eda740`（基于 r2 之前的 `7fc36e587`，38 个主题提交，领先 r1 tag 59 个提交、尚未包含 r2 收口内容），CodeWhale #54 squash 合入、`pinvou3-clean` 推进并重钉后回收过渡期叠层；不可变 tag `pinvou-v0.9.12-r1` 保持 r1 收口状态（15 个提交），仍为回退点 |
...
| drift | `190 files, +19135/-2628`，净增 16507 行（维护分支对上游，实测于 r2 收口 `6f780290f`）；过渡期另加本主题 `61 files, +4971/-288`（`7fc36e587` → `282eda740`，主题分支尚未 rebase 到 r2）；r1 为 `94 files, +5022/-944`，旧 r13 为 `110 files, +10895/-1195` |
...
### 多根工作区 workspace_roots + 指令 source 相对化（CodeWhale PR #54 head `282eda740`，基于 r2 之前的 `7fc36e587`，尚待 rebase 到 r2 `6f780290f`）

- CodeWhale 分支 `pinvou3/workspace-roots-v12`（`qiuYliangM/CodeWhale` fork，PR #54 head `282eda740`；仍基于 r2 之前的 `pinvou3-clean` @`7fc36e587`——维护线已随 r2 收口推进至 `6f780290f`，主题分支尚待 rebase 到 r2 之上——即 r2 前维护线头 + 38 个主题提交（第 8/9/10 轮修复提交；更早的 `88405ffbc`（rebase 到 `92427bd8d` 的 23 提交线）与 10 提交中文信息线 `f2526196c` 均已被本线取代））：线程从单根 `cwd` 扩展为 **cwd（主根）+ workspace_roots（全量可访问根集合）**，是"单入口工作区"（项目 = 主文件夹 + 一组钥匙）的底座前提。四层落地：
...
- drift：`pinvou3-clean` @`7fc36e587` → 本主题 head `282eda740` 为 `61 files, +4971/-288`（第 8 轮在上一次登记 `54 files, +3278/-240` 之上补齐 ACP 会话/加载通道、`/title` 写路径、repo law 逐根判定与证据非空洞化等修复，第 9/10 轮继续补齐评审修复）；本主题新增 6 条 `forkguard_workspace_roots_*` 行为名（source 相对化那条已被 #59 收进 `pinvou3-clean`）；叠加 #66 在底座新增的 10 条 PowerShell 相关行为名，全仓独立 forkguard 行为名 90→106，登记下限随本 PR 的 `scripts/fork-guard.sh` 提到 96。另含 turn_meta 列根：多根会话的每回合元数据新增 `Accessible folders:` 行（紧贴 `Current workspace:`，最多 5 个超出截断，单根逐字节无此行）——模型获知附加根的通道。
...
- 候选登记已回收：#58 落地后父仓 gitlink 随登记批次重钉（回收时为 `92427bd8d`，`EXPECTED_COMMITS` 相应推进），`CANDIDATE_HEAD`/`CANDIDATE_COMMITS=29` 机制移除；其后 #552 批次已把登记头推进至 `7fc36e587`（`EXPECTED_COMMITS` 36，见第 0 节表格）；指纹层保留底座回显契约回归与父仓超越序回归两条锚点；`verify-public-submodule.sh` 过渡期口径为断言 gitlink = `pinvou3-clean` 分支头：本 PR 叠层期间（gitlink = 主题 head `282eda740`）该门禁保持红并在 PR 描述中披露，待 CodeWhale #54 squash 合入、`pinvou3-clean` 推进后重钉恢复绿。
...
- 不可变 `pinvou-v0.9.12-r1` 停在 r1 收口 `1fafee7e26b60a59457a43bce50c63aa2ad9dbaf`；r2 已收口（2026-09-21，main 经 #574 `1de2a60f0` 完成）：不可变 tag `pinvou-v0.9.12-r2` 切在 `6f780290f`，main 上维护分支/gitlink/tag 三方相等；本 PR 仍处于过渡期叠层：父仓 gitlink 指向 workspace_roots 主题 head `282eda740`（基于 r2 之前的 `7fc36e587`），以 `scripts/verify-public-submodule.sh` 验证公开可达性（过渡期断言 gitlink=分支头，本 PR 叠层期间该门禁保持红并在 PR 描述披露；待 #54 squash 合入、`pinvou3-clean` 推进后将主题 rebase 到 r2 之上并重钉 gitlink，恢复三方相等；依据 `docs/fork-policy.md` 第 0 节过渡期豁免）。
