...
## 0. 当前状态（2026-09-17 · r1 基线 + 18 个登记提交，r2 收口未切 tag）
...
| 发布状态 | 过渡期（fork-policy 第 0 节豁免）：不可变 tag `pinvou-v0.9.12-r1` 保持 r1 收口状态（15 个提交），父仓 gitlink 指向 `92427bd8d`、领先 tag 18 个提交，直至下一次 r2 发布收口对齐 |
...
- CodeWhale 分支 `pinvou3/workspace-roots-v12`（`qiuYliangM/CodeWhale` fork，PR #54 head `88405ffbc`；已 rebase 到 `pinvou3-clean` @`92427bd8d`——即 v0.9.12 r1 + ae7e3fb36/#53 + #58–#61 之上 23 个提交（第 7 轮修复 5 个提交，历史 4 条不实 commit message 已改写并 force-push，树内容与第 6 轮审阅树逐字节一致）；更早的 10 提交中文信息线 head `f2526196c` 已被该英文改写线取代）：线程从单根 `cwd` 扩展为 **cwd（主根）+ workspace_roots（全量可访问根集合）**，是"单入口工作区"（项目 = 主文件夹 + 一组钥匙）的底座前提。四层落地：
...
- drift：`pinvou3-clean` @`7fc36e587` → 本主题 head `98b87d527` 仍为 `54 files, +3278/-240`（与上一版逐字节相同：第 7 轮后 #54 只做了一次纯 rebase，父仓消费面无变化）（第 6 轮登记 `51 files, +2697/-161`；更早登记 `40 files, +1635/-142` 与实际不符：旧线 r1→`f2526196c` 实为 `47 files, +1877/-158`）；新增 6 条 `forkguard_workspace_roots_*` 行为名（source 相对化那条已被 #59 收进 `pinvou3-clean`），全仓独立 forkguard 行为名 72→78，`codewhale-tui --lib forkguard_` 默认组 72 条 + benchmark 组 6 条运行通过。另含 turn_meta 列根：多根会话的每回合元数据新增 `Accessible folders:` 行（紧贴 `Current workspace:`，最多 5 个超出截断，单根逐字节无此行）——模型获知附加根的通道。
