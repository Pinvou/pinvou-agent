...
- 这是可复用的 CodeWhale 修复，已在 `Pinvou/CodeWhale#62` 通过审核（候选 head `abadae45d` 已把维护分支 `92427bd8d` 并入），待 squash 合入 `pinvou3-clean`。修复前持久化的会话由该修复在会话载入/恢复时自动修正，不再要求新建会话，也没有独立的迁移工具。当前父仓 gitlink 的公开可达性门禁仍需合入 `pinvou3-clean` 后验证。
...
所有非合并提交都含 DCO `Signed-off-by`（本表唯一的合并提交 `abadae45d` 不带该 trailer）。`b4c02616b` 包含大部分跨主题收口，历史粒度确实不利于 bisect；分支公开进入评审后没有为历史美化 force-push，而是追加带 sign-off 的提交修复评审和发布门禁问题，并用本表、指纹和行为测试弥补审计粒度。不可变 tag 已创建，后续不得重写。
