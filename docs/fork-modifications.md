...
- bridge 保留 v0.9.12 的有限 `max_steps` 轮次预算、read denylist、bubblewrap、MCP OAuth、goal loop 与 telemetry 安全默认值；产品构建不再叠加基准专用的每轮工具上限（见 §2 的 2026-09-11 行），runaway 防护由底座的步数/墙钟/取消边界承担。
