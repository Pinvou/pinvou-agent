...
| 发布状态 | r2 已收口（2026-09-21，main 经 #574 完成）：不可变 tag `pinvou-v0.9.12-r2` 切在 `6f780290f`，main 上维护分支/gitlink/tag 三方相等，但 r2 不含 workspace_roots 主题；本 PR 继续过渡期叠层（fork-policy 第 0 节豁免）：父仓 gitlink 钉主题 head `28956fdeb`（已 rebase 到 r2 收口 `6f780290f` 之上，39 个主题提交，领先 r1 tag 63 个提交），CodeWhale #54 squash 合入、`pinvou3-clean` 推进并重钉后回收过渡期叠层；不可变 tag `pinvou-v0.9.12-r1` 保持 r1 收口状态（15 个提交），仍为回退点 |
...
| drift | `190 files, +19135/-2628`，净增 16507 行（维护分支对上游，实测于 r2 收口 `6f780290f`）；过渡期另加本主题 `57 files, +4261/-268`（r2 收口 `6f780290f` → `28956fdeb`，主题分支已 rebase 到 r2 之上）；r1 为 `94 files, +5022/-944`，旧 r13 为 `110 files, +10895/-1195` |
...
### 多根工作区 workspace_roots + 指令 source 相对化（CodeWhale PR #54 head `28956fdeb`，已 rebase 到 r2 收口 `6f780290f` 之上）

- CodeWhale 分支 `pinvou3/workspace-roots-v12`（`qiuYliangM/CodeWhale` fork，PR #54 head `28956fdeb`；已 rebase 到 r2 收口 `pinvou3-clean` @`6f780290f` 之上——即 r2 收口维护线头 + 39 个主题提交（第 8/9/10 轮修复提交与 r2 语义合并修复；更早的 `88405ffbc`（rebase 到 `92427bd8d` 的 23 提交线）与 10 提交中文信息线 `f2526196c` 均已被本线取代））：线程从单根 `cwd` 扩展为 **cwd（主根）+ workspace_roots（全量可访问根集合）**，是"单入口工作区"（项目 = 主文件夹 + 一组钥匙）的底座前提。四层落地：
...
- 候选登记已回收：#58 落地后父仓 gitlink 随登记批次重钉（回收时为 `92427bd8d`，`EXPECTED_COMMITS` 相应推进），`CANDIDATE_HEAD`/`CANDIDATE_COMMITS=29` 机制移除；其后 #552 批次已把登记头推进至 `7fc36e587`（`EXPECTED_COMMITS` 36，见第 0 节表格）；指纹层保留底座回显契约回归与父仓超越序回归两条锚点；`verify-public-submodule.sh` 过渡期口径为断言 gitlink = `pinvou3-clean` 分支头：本 PR 叠层期间（gitlink = 主题 head `28956fdeb`）该门禁保持红并在 PR 描述中披露，待 CodeWhale #54 squash 合入、`pinvou3-clean` 推进后重钉恢复绿。
