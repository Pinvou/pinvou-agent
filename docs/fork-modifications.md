...
## 0. 当前状态（2026-09-23 · r3 已收口（tag `pinvou-v0.9.12-r3` @ `61cb769be`，不含 workspace_roots 主题）+ workspace_roots 主题过渡叠层；上游起 84 个提交）
...
| 发布状态 | r3 已收口（2026-09-23）：不可变 tag `pinvou-v0.9.12-r3` 切在合并头 `61cb769be`，main 上维护分支/gitlink/tag 三方相等；r3 tag 曾短暂钉在 `ba2a07768f`，在父仓引用前因 #77 Windows 编译修复重切至 `61cb769be`（同日完成，未被任何父仓提交引用）。但 r3 不含 workspace_roots 主题；本 PR 继续过渡期叠层（fork-policy 第 0 节豁免）：父仓 gitlink 钉主题 head `9f5f4d34a`（已 rebase 到 r2 收口 `6f780290f` 之上、尚未 rebase 到 r3，45 个主题提交，领先 r1 tag 69 个提交），CodeWhale #54 squash 合入、`pinvou3-clean` 推进并重钉后回收过渡期叠层；不可变 tag `pinvou-v0.9.12-r2` 保持 r2 收口状态（24 个提交）与 `pinvou-v0.9.12-r1`（15 个提交）同为回退点 |
| 升级前回退点 | 公开不可变 tag `pinvou-v0.9.5-r13` → `f853f8f1566c57e6be40d5439a222a932aa79ef5`；同 SHA 的本地 `backup/pre-v0.9.12-sync` 仅作便利引用 |
| 历史组织 | 上游之上 84 个带 DCO sign-off 的提交（gitlink `9f5f4d34a` = r2 收口维护线 39 个 + workspace_roots 主题 45 个，主题现居 fork 分支 `pinvou3/workspace-roots-v12`，待 CodeWhale #54 squash 合入），归属 4 个长期主题（T1–T4）+ 2 个追加减量主题（T5 会话归档导出、T6 蜂群限流治理）+ 1 个已合入维护分支的主题（T7 压缩检查点角色兼容）+ 1 个过渡期在册主题（workspace_roots，见下节）；r1 之后的维护线提交全部经 PR squash 合入并过五项必需门禁（r3 收口线 34 个） |
| drift | `229 files, +29272/-5352`，净增 23920 行（维护分支对上游，实测于 r3 收口 `61cb769be`）；过渡期另加本主题 `78 files, +5882/-373`（r2 收口 `6f780290f` → `9f5f4d34a`，主题分支已 rebase 到 r2 之上、尚未 rebase 到 r3）；r2 为 `190 files, +19135/-2628`，r1 为 `94 files, +5022/-944`，旧 r13 为 `110 files, +10895/-1195` |
| 守护 | CodeWhale `forkguard_*` 独立行为名实测 **142**（于登记的 gitlink `9f5f4d34a`，即 `scripts/fork-guard.sh` 实际执行的计数）：r2 收口基线 133 + 本主题新增 9 条 `forkguard_workspace_roots_*`；r3 收口维护线实测 141（主题分支尚未 rebase 到 r3，#54 合并时按合并树重测）；其中 6 条钉在 `benchmark-eval-controls` 门控面，9 条为 #66 随维护分支在底座新增的 PowerShell 相关行为名，其余为 T1–T7 等存量。登记下限 96 + 父仓指纹与行为测试（workspace_roots 指纹锚点已随本 PR 注册进 `scripts/fork-guard.sh`） |
...
### 多根工作区 workspace_roots + 指令 source 相对化（CodeWhale PR #54 head `9f5f4d34a`，已 rebase 到 r2 收口 `6f780290f` 之上）

- CodeWhale 分支 `pinvou3/workspace-roots-v12`（`qiuYliangM/CodeWhale` fork，PR #54 head `9f5f4d34a`；已 rebase 到 r2 收口 `pinvou3-clean` @`6f780290f` 之上——即 r2 收口维护线头 + 41 个主题提交（第 8–14 轮修复提交与 r2 语义合并修复；更早的 `88405ffbc`（rebase 到 `92427bd8d` 的 23 提交线）与 10 提交中文信息线 `f2526196c` 均已被本线取代））：线程从单根 `cwd` 扩展为 **cwd（主根）+ workspace_roots（全量可访问根集合）**，是"单入口工作区"（项目 = 主文件夹 + 一组钥匙）的底座前提。四层落地：
...
- drift：`pinvou3-clean` @`6f780290f`（r2 收口）→ 本主题 head `9f5f4d34a` 为 `78 files, +5882/-373`（第 8 轮在上一次登记 `54 files, +3278/-240` 之上补齐 ACP 会话/加载通道、`/title` 写路径、repo law 逐根判定与证据非空洞化等修复，第 9–14 轮继续补齐评审修复（hint 记录全域 missing 防护、根集 PATCH 驱逐测试、第 14 轮的只读 Scout shell 多根信任边界与 /cd 回执/孤儿会话修复），rebase 到 r2 后底座已含的 #66 cherry-pick 等内容退出 diff）；本主题新增 9 条 `forkguard_workspace_roots_*` 行为名（source 相对化那条已被 #59 收进 `pinvou3-clean`）；全仓独立 forkguard 行为名实测 142（r2 线 #55–#67 在底座亦有新增），登记下限随本 PR 的 `scripts/fork-guard.sh` 提到 96。另含 turn_meta 列根：多根会话的每回合元数据新增 `Accessible folders:` 行（紧贴 `Current workspace:`，最多 5 个超出截断，单根逐字节无此行）——模型获知附加根的通道。
...
- 候选登记已回收：#58 落地后父仓 gitlink 随登记批次重钉（回收时为 `92427bd8d`，`EXPECTED_COMMITS` 相应推进），`CANDIDATE_HEAD`/`CANDIDATE_COMMITS=29` 机制移除；其后 #552 批次已把登记头推进至 `7fc36e587`（`EXPECTED_COMMITS` 36，见第 0 节表格）；指纹层保留底座回显契约回归与父仓超越序回归两条锚点；`verify-public-submodule.sh` 过渡期口径为断言 gitlink = `pinvou3-clean` 分支头：本 PR 叠层期间（gitlink = 主题 head `9f5f4d34a`）该门禁保持红并在 PR 描述中披露，待 CodeWhale #54 squash 合入、`pinvou3-clean` 推进后重钉恢复绿。
...
- 本 PR 仍处于过渡期叠层：父仓 gitlink 指向 workspace_roots 主题 head `9f5f4d34a`（已 rebase 到 r2 收口 `6f780290f` 之上、尚未 rebase 到 r3），以 `scripts/verify-public-submodule.sh` 验证公开可达性（过渡期断言 gitlink=分支头，本 PR 叠层期间该门禁保持红并在 PR 描述披露；待 #54 squash 合入、`pinvou3-clean` 推进后重钉 gitlink，恢复三方相等；依据 `docs/fork-policy.md` 第 0 节过渡期豁免）。
