...
> **历史快照注（2026-08-22）**：本报告是 0.9.5 升级时点的留档；§9 的交付状态与"下一步"描述当时的 PR #285，该 PR 已合并，main 已继续推进。fork 现行状态以 `docs/fork-modifications.md` §0 为准。§6 只记录了当时实际执行的 L0/部分 L1 验证；`docs/底座升级验收清单.md` 的 L1.4/L1.5/L2/L3 未在本次升级中逐项执行签收（其中 L1.5 需真实 vLLM 端点，本次未运行），验收清单已按 0.9.5 机制修订。
