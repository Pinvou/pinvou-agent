...
| C-6 | 首个 include 放出：code 档案 `["git_status", "git_diff"]`（只读 git 工具，事件走前端通用 tool_call 渲染） | 功能 |
...
- 语义 = **基础集 + 差量**（方案 §3.5）：上游新增工具仍被基础集（底座隐藏常量）挡住，模式只表达差异。v1 档案：plain 全空差量（零影响），code `tools.include = ["git_status", "git_diff"]`（步骤 C-6，已评估事件渲染的只读工具）。
...
- code 档案放出 `git_status` / `git_diff`（PINVOU3_HIDDEN_TOOLS 原含二者，2026-07-03 因纯办公定位全隐藏；code 模式为编码场景，按方案步骤 5 的"逐个评估、小步放出"恢复这两个只读查询工具）。
- **事件渲染已确认（U-9）**：二者是标准 tool_call 事件（前端 `acp-state.js` 通用处理 tool_call/tool_call_update），无 agent_spawn 类特殊事件流，不产生裸 JSON。`apply_patch` 属"定位性隐藏"按方案默认不放。
...
| U-9 | 前端渲染（include 工具事件有 UI 渲染，无裸 JSON） | git_status/git_diff 走通用 tool_call 渲染（C-6 确认） |
| U-10 | 自动检查 | 见 §五 |
...
- 前端：无代码改动（C-6 走通用渲染）；`lint:ui` / `test:ui-language` 基线不变。
...
- **行为变化面（刻意）**：code 会话放出 `git_status` / `git_diff`（C-6）；`SessionPolicy` 内部重构（resolve 统一产出，外部行为等价）。
- **遗留**：fork ② 依据 `.luzeyang/capability-unified/01-fork改动申请-领导评审.md` 实施；若评审未过可整体回退——app 停止注入（include 清空）即回退现状，revert 为纯代码回退、无数据迁移。档案 v1 编译内嵌不可运行期编辑（未来可编辑化需重议 respawn，方案 §5.3 已记录）。
