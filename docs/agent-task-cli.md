...
- `attachments`: files staged into the session's `attachments/` directory, ingested through the GUI attachment pipeline, and rendered with the same attachment text (errors `agent_attachment_not_found` / `agent_attachment_too_many` / `agent_attachment_too_large`); `remove_after_ingest` removes the caller-side source after the turn is admitted (refused under falsy `KEEP_SESSION`, see above).
