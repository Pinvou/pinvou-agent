...
## Concurrency and session-store sharing

The session store has no cross-process lock. A desktop app and a headless `agent run` sharing one `PINVOU3_HOME` write the main `<session_id>.json` records last-writer-wins per file: an interleaved run can lose the losing writer's transcript tail, and the GUI's session list is an in-process cache, so a headless-created session appears only after a GUI rescan or restart. The pinned/hidden and multi-agent sidecar files fare better — writes merge id-level mutations against the current file content — but even those read-modify-write without a cross-process lock, so two simultaneous writers can still lose an update. Marketplace governance state (`disabled_bundles.json`) is the exception: both processes serialize it behind a cross-process bundle lock. Extending that lock pattern to the sessions root is a deliberate follow-up product decision, not an oversight; until it lands, point harnesses at a dedicated `PINVOU3_HOME` sandbox instead of the desktop's live store.
