...
- Session lifecycle: sessions persist after the run (GUI parity — engine resources are reclaimed, and the transcript/artifacts/timeline stay under `$PINVOU3_HOME/sessions/<session_id>`, subject to the same 50-session retention cap as the desktop app). Set `PINVOU3_AGENT_TASK_KEEP_SESSION=0|false|no|off` to restore the legacy one-shot cleanup that deletes the temporary session once the report has been produced; the legacy truthy values (`1`/`true`/`yes`/`on`) are still accepted and keep the default. A caller-provided `--session` is never deleted by the run regardless of this setting (it still counts toward retention like any chat session). Retention counts these kept sessions against the same 50-per-kind cap as GUI chat sessions: a batch harness looping this command against the desktop's default `PINVOU3_HOME` will evict the user's oldest GUI chat sessions (pinned ones included), the CLI prints a stderr warning when it detects the store is at the cap, so watch for it — and point `PINVOU3_HOME` at a sandbox for bulk runs. There is no cross-process lock between `--session` and a GUI session open in the app; concurrent writes are last-writer-wins.

Prerequisites: the `settings.json` of the sandbox `PINVOU3_HOME` needs an active model (any OpenAI-compatible endpoint works, `preset = "openai_compatible"`); `PINVOU3_ALLOW_SHELL=1` pins shell authorization without relying on prefs. Tool-call rounds are **not limited anywhere**: no host-level cap and no environment knob exists (the former `PINVOU3_MAX_TOOL_CALLS` knob was removed) — runaway protection comes from the engine's own bounds (2,000 model steps per turn by default, a 3,600 s per-turn wall clock, bounded retries, the cancel boundary) plus this harness's 600 s per-task deadline and the FINAL ANSWER output contract. The 2,000-step figure is the host default for the main turn and the sub-agent default alike, not a hard cap on every call: an `agent` call that explicitly passes `max_steps` keeps the foundation's spawn semantics — positive values clamp to the same 2,000 ceiling, and an explicit 0 remains the foundation's unbounded sentinel.
