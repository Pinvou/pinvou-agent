...
- Session lifecycle: sessions persist after the run (GUI parity — engine resources are reclaimed, and the transcript/artifacts/timeline stay under `$PINVOU3_HOME/sessions/<session_id>`, subject to the same 50-session retention cap as the desktop app). A run that fails before producing a report (exit 1) deletes the session it just created, best-effort. Set `PINVOU3_AGENT_TASK_KEEP_SESSION=0|false|no|off` to restore the legacy one-shot cleanup that deletes the temporary session once the report has been produced; the legacy truthy values (`1`/`true`/`yes`/`on`) are still accepted and keep the default. A caller-provided `--session` is never deleted by the run regardless of this setting (it still counts toward retention like any chat session). Retention counts these kept sessions against the same 50-per-kind cap as GUI chat sessions: a batch harness looping this command against the desktop's default `PINVOU3_HOME` will evict the user's oldest GUI chat sessions (pinned ones included). Before a fresh run (not a `--session` continuation) that samples the store at the cap, the CLI prints a stderr warning — before the run, regardless of its outcome — so watch for it, and point `PINVOU3_HOME` at a sandbox for bulk runs. There is no cross-process lock between `--session` and a GUI session open in the app; concurrent writes are last-writer-wins.
