...
- **SKILL.md 说明同步（双向）**：仅当包内有且仅有一个技能（`bundles/<id>/skills/`
  ...
  - 清空显示说明 → 从备份恢复 frontmatter 原值（原缺失则删掉 description 行），
    ...
    （原文件块状写法）或含引号——恢复按原值无损回写（多行写成块状字面量），
    不因原值形态而失败；恢复以备份值为准，会覆盖此期间对 `SKILL.md`
    description 的手工编辑。
  值无法单行互洽表达（含双引号、反斜杠）、或 frontmatter 存在结构性多重
  description 定义时整体报错、不落盘（备份 key 也不写）。多技能包与纯 MCP 包
  跳过同步（说明覆盖仅写入 extra，不备份）。重导入整体替换包内容时，说明备份
  随内容重基线（丢弃，防止把旧包描述恢复进新包）；展示名/说明覆盖本身按上文
  语义跨重导入保留。
- **编辑与重导的交互**：说明回写改变了包内容，同一 zip 之后再导入会按 §11
  第 9 条的同 id 内容比对被判「内容不同」拒收——更新包内容需更换包 id，或先
  删除 `~/.pinvou3/bundles/<id>/` 目录再导入（Upload 包卸载保留目录作为用户
  唯一副本，「先卸载再导入」清不掉同 id 冲突）。
