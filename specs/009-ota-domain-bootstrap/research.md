...
## 决策 3：域名引导 SN 可兜底，OTA 更新 SN 始终使用 BIOS SN

**Decision**：域名引导前读取 Windows BIOS SN，trim 后如果以 `2198` 或 `2199` 开头，则作为 `/v2/bootstrap` 请求 `device_id`；否则域名引导请求使用固定 SN `219904A17T4257W00018`。后续 OTA 更新查询、下载信息查询和升级结果反馈请求中的 `sn` 始终使用真实 BIOS SN，不使用固定 SN 兜底。

**Rationale**：用户明确指定固定 SN 只适用于域名引导请求。将域名引导 `device_id` 与 OTA 更新 `sn` 拆开，可以覆盖非目标设备仍通过引导获取 OTA host，同时保证更新后台收到真实设备 BIOS SN。

**Alternatives considered**：
- 继续使用 `COMPUTERNAME` 或 `PINVOU3_OTA_SN`：不满足 BIOS SN 的业务要求。
- OTA 更新接口继续复用域名引导 `effective_sn`：会让非目标设备的更新查询和反馈错误使用固定 SN。
