...
**能调用工具、操作文件、沉淀知识并交付产物的桌面 AI Agent 工作台。**
...
Pinvou Agent 支持本地 vLLM 和 OpenAI-compatible API。应用内可保存多个模型配置，并在不同会话间快速切换；当前提供本地 vLLM、DeepSeek、Kimi、通义千问、豆包、MiniMax、智谱、MiMo 等配置模板，也可以填写自定义兼容端点。
...
| 修复底座通用问题 | CodeWhale fork，并按 fork policy 维护 |
...
> **macOS 用户**：Pinvou Agent 支持 Apple Silicon (arm64) + macOS 14.0+。语音识别引擎可按构建配置打包；文件解析（PDF/Office/OCR/压缩包等）依赖可选外部工具，可通过 Homebrew 或各工具官网安装。
...
完整的提交规范、CI 门控和 fork 修改规则见 [CONTRIBUTING.zh-CN.md](CONTRIBUTING.zh-CN.md)。
