...
pinvou3 的 5 mode 架构面向 **单次会话 / 半小时内 / 中等复杂度** 任务：
...
| 5-7 天行程 | 🟡 勉强 | 拆解可能挤、单步会漏细节 |

### 1.4 非目标场景

以下场景不在当前覆盖范围（需要 P2 新能力，强行用 5 mode 处理会有体验问题）：

| 场景 | 缺什么能力 | 路线 |
|---|---|---|
| **超长规划**（10+ 天行程、月度项目） | 嵌套 milestone、变更传播 | P2: `SubAgentRouter` |
| **跨会话持续任务**（每天追踪进度） | 对话状态持久化 | P2: `CheckpointStore` |
| **多文档输出**（行程 + 装备 + 预算分别成册） | 结构化产出形态 | P2 |
| **长上下文累积** | context 自动压缩 / 摘要 | P2 |
