...
/// Existing engine/file-watcher call sites use this narrow hook. Events are
/// forwarded only when a connected WebUI subscribed to the allowlisted name.
...
pub async fn web_access_create_session(
    ...
    let metadata = crate::commands::create_session(Some(false), store, pool).await?;
    let transcript_revision = crate::bridge::sessions::transcript_revision(&[])
...
pub async fn web_access_load_session_chunk(
    ...
    crate::bridge::sessions::validate_session_id(&id)
    ...
    let (download_id, transcript_revision) = match download_id {
        ...
        _ => {
            ...
            let revision = tokio::task::spawn_blocking(move || -> Result<String, String> {
                ...
                let revision = crate::bridge::sessions::transcript_revision(&saved.messages)
                    .map_err(|error| {
...
pub async fn web_access_ingest_file(
    ...
    let result = crate::commands::ingest_file(path).await?;
...
pub async fn web_access_chat(
    ...
    crate::bridge::sessions::validate_session_id(&session_id)
        .map_err(|error| format!("invalid Session id: {error:#}"))?;
    store
        .load(&session_id)
        .map_err(|error| format!("load Session {session_id}: {error:#}"))?;
    ...
    let result = crate::commands::chat_with_reservation(
        message,
        Some(attachments),
        session_id,
        restrict_tools,
...
pub async fn web_access_transcribe_voice_audio(
    ...
    session_id: String,
    store: State<'_, SessionStore>,
) -> Result<crate::commands::VoiceTranscriptionResponse, crate::commands::VoiceCommandError> {
    crate::bridge::sessions::validate_session_id(&session_id).map_err(|error| {
...
pub async fn web_access_start_skill_session(
    ...
    store: State<'_, SessionStore>,
    pool: State<'_, EnginePool>,
) -> Result<crate::commands::StartSkillSessionResult, String> {
    crate::commands::start_skill_session(name, Some(false), store, pool).await
}
...
pub async fn web_access_update_settings(prefs: UserPrefs) -> Result<(), String> {
    ...
    crate::commands::update_settings(current).await
}
...
pub async fn web_access_list_deliverables(
    ...
    crate::commands::list_deliverables(scoped_workflow_project(&store, &session_id)?).await
}
...
pub async fn web_access_get_role_prompt(
    ...
    session_id: String,
    store: State<'_, SessionStore>,
) -> Result<crate::commands::RolePromptPayload, String> {
    let project = scoped_workflow_project(&store, &session_id)?;
    crate::commands::get_role_prompt(role_id, Some(project)).await
}
...
pub async fn web_access_get_role_outputs(
    ...
    session_id: String,
    store: State<'_, SessionStore>,
) -> Result<Vec<crate::commands::OutputFile>, String> {
    crate::commands::get_role_outputs(role_id, scoped_workflow_project(&store, &session_id)?).await
}
...
pub async fn web_access_get_role_logs(
    ...
    crate::commands::get_role_logs(role_id, scoped_workflow_project(&store, &session_id)?, tail)
        .await
}
...
pub async fn web_access_get_gate_report(
    ...
    crate::commands::get_gate_report(role_id, scoped_workflow_project(&store, &session_id)?).await
}
...
pub fn web_access_artifact_info(
    ...
    session_id: String,
    store: State<'_, SessionStore>,
) -> Result<crate::commands::ArtifactInfo, String> {
    let resolved = scoped_artifact_path(&store, &session_id, &path)?;
    crate::commands::artifact_info_impl(&resolved)
}
...
pub fn web_access_read_artifact_text(
    ...
    ensure_web_artifact_response(crate::commands::read_artifact_text_impl(&resolved)?)
}
...
pub async fn web_access_read_artifact_image_b64(
    ...
    ensure_web_artifact_response(crate::commands::read_artifact_image_b64(resolved).await?)
}
...
pub async fn web_access_read_artifact_thumbnail(
    ...
    let resolved = scoped_artifact_path(&store, &session_id, &path)?;
    ensure_web_artifact_response(crate::commands::read_artifact_thumbnail(resolved).await?)
}
...
pub async fn web_access_render_artifact_visual(
    ...
    session_id: String,
    store: State<'_, SessionStore>,
) -> Result<crate::commands::VisualResult, String> {
    let resolved = scoped_artifact_path(&store, &session_id, &path)?;
    ensure_web_artifact_response(crate::commands::render_artifact_visual(resolved).await?)
}
