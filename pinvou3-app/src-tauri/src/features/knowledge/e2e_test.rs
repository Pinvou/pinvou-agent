...
fn full_l0_l1_e2e() {
    ...
    std::env::set_var("PINVOU3_KB_EMBED_MODEL_DIR", format!("{home}/models/bge-m3"));

    let root = std::env::temp_dir().join(format!("pinvou3_e2e_{}", std::process::id()));
    let _ = fs::remove_dir_all(&root);
    fs::create_dir_all(root.join("docs")).unwrap();
    // L0 多类型 + 一对内容相同的重复文件
    fs::write(root.join("docs/季度报告.md"), "# 季度销售报告\n华东区增长 20%，华南区持平。重点客户续约率 85%。").unwrap();
    fs::write(root.join("docs/访谈纪要.md"), "# 用户访谈\n受访者认为保险报价流程过于繁琐，希望一键比价。竞品在交强险环节体验更顺畅。").unwrap();
    fs::write(root.join("docs/副本.md"), "# 季度销售报告\n华东区增长 20%，华南区持平。重点客户续约率 85%。").unwrap();
    ...
    assert!(stats.total_files >= 5, "应扫到 ≥5 常用文件，实际 {}", stats.total_files);

    // ───── L0：类型白名单(源码/编译产物不入库) ─────
    let src = svc
        .store
        .search(&SearchQuery { text: Some("main".into()), limit: 50, ..Default::default() })
        .expect("search src");
    assert!(!src.iter().any(|h| h.name == "main.c"), "源码 main.c 应被白名单排除");
    let cls = svc
        .store
        .search(&SearchQuery { text: Some("App".into()), limit: 50, ..Default::default() })
        .expect("search class");
    assert!(!cls.iter().any(|h| h.name == "App.class"), "编译产物 App.class 应被白名单排除");

    // ───── L0：按名搜索 ─────
    let by_name = svc
        .store
        .search(&SearchQuery { text: Some("报告".into()), limit: 50, ..Default::default() })
        .expect("search name");
    assert!(by_name.iter().any(|h| h.name.contains("季度报告")), "应搜到 季度报告.md");

    // ───── L0：按类型过滤 ─────
    let by_ext = svc
        .store
        .search(&SearchQuery { exts: vec!["md".into()], limit: 50, ..Default::default() })
        .expect("search ext");
    assert!(by_ext.len() >= 3, "应有 ≥3 个 .md，实际 {}", by_ext.len());
    assert!(by_ext.iter().all(|h| h.ext.as_deref() == Some("md")), "过滤后应全为 md");

    // ───── L0：类型计数 ─────
    let tc = svc.store.type_counts().expect("type_counts");
    let md = tc.iter().find(|t| t.ext == "md").map(|t| t.count).unwrap_or(0);
    assert!(md >= 3, "md 计数应 ≥3，实际 {}", md);

    // ───── L1：知识集 + 真实解析 + embedding ─────
    assert!(svc.l1().has_embedder(), "L1 embedding 应已启用(env 配了 bge-m3)");
    let cid = svc.l1().create_collection("调研集", Some("调研"), Some("访谈与报告")).expect("create coll");
    assert_eq!(svc.l1().ingest_file(cid, &root.join("docs/访谈纪要.md")), "parsed");
    assert_eq!(svc.l1().ingest_file(cid, &root.join("docs/季度报告.md")), "parsed");

    // ───── L1：关键词检索(命中原词) ─────
    let kw = svc.l1().retrieve_for_chat(cid, "交强险", 5, 0).expect("kw search");
    assert!(!kw.is_empty(), "应检索到含'交强险'的块");
    assert!(kw.iter().any(|h| h.text.contains("交强险")), "命中块应含'交强险'");
    assert!(kw[0].doc_name.contains("访谈"), "溯源应指向访谈纪要");

    // ───── L1：语义检索(同义不同词，考验向量) ─────
    let sem = svc.l1().retrieve_for_chat(cid, "车险价格怎么比较", 5, 0).expect("semantic search");
    assert!(!sem.is_empty(), "语义查询应有召回(向量路径)");
    assert!(
        sem.iter().any(|h| h.text.contains("报价") || h.text.contains("比价") || h.text.contains("交强险")),
    ...
    let coll = svc.l1().list_collections().expect("colls").into_iter().find(|c| c.id == cid).unwrap();
