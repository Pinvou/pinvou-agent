...
mod tests {
    ...
    fn apply_output_passes_runtime_config_ready() {
        ...
        // 产物必须通过官方校验函数（真实解析路径）
        assert!(crate::features::codex_acp::auth_probe::kimi_runtime_config_ready(&raw, false));
    ...
    fn official_default_model_roundtrip() {
        ...
        assert!(crate::features::codex_acp::auth_probe::kimi_runtime_config_ready(&raw, true));
        let _ = fs::remove_dir_all(&dir);
    }
