...
use agent_probe::{CliProbeCache, CliProbeGates};
use anyhow::{bail, Context, Result};
use auth_probe::{cli_status_success, AgentAuthProbeState, CachedAuthStatus};
use serde::Serialize;
...
use runtime::{probe_codex_runtime, version_at_least, ResolvedCodex, MIN_CODEX_VERSION};
pub use store::{
...
impl AcpPool {
    ...
    async fn refresh_runtime_probe(&self, force: bool) {
        ...
        let detected = tokio::task::spawn_blocking(move || {
            ...
            // CLI 正常可用时安装来源不会影响任何 UI 或行为，只做纯路径判断；
            // 仅版本过旧、服务端要求升级或用户主动重查时执行 npm/brew 子进程。
            ...
            RuntimeProbeCache {
                ...
                system_codex_incompatible: candidates.system_codex_incompatible,
            }
        })
        .await;
...
pub(super) fn probe_cli_version(command: &Path) -> CliVersionProbe {
    command_version_probe(command)
}
...
fn brew_package_installed(backend: AgentBackend) -> bool {
    ...
    cli_status_success(Path::new(platform::brew_bin()), args)
}
...
/// `npm ls -g <pkg> --depth=0` 退出码 0 即视为 npm 全局安装；这是本地元数据
/// 查询，5 秒仍无响应即放弃，不能让包管理器异常拖住状态界面。
...
/// `npm prefix -g` 输出的全局根目录；npm 不可用或 5 秒超时返回 None。
...
#[cfg(test)]
mod tests {
    use super::auth_probe::{kimi_credentials_valid, kimi_runtime_config_ready};
    use super::*;
