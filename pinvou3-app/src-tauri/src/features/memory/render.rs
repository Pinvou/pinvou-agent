...
pub fn render_memory_block() -> stdio::Result<(String, Vec<InjectedMemoryItem>)> {
    ...
    let preferences = io::load_preferences().unwrap_or_default();
    let work_context = io::load_work_context().unwrap_or_default();
    let current_focus = io::load_current_focus().unwrap_or_default();
    let recent_activity = io::load_recent_activity().unwrap_or_default();
    let legacy_recent_work = io::load_recent_work().unwrap_or_default();
