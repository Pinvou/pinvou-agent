...
/// Engine idle-reclaim threshold: an engine is an in-process task (not a
/// subprocess), but each holds a channel, a toolset, and the hydrated
/// conversation context; the pool is unbounded and memory grows linearly with
/// session count. Reclaim when idle beyond this duration with no in-flight
/// turn and not the active session (reusing `reclaim_engine_entry`'s reclaim
/// sequence). Reclaiming returns to lazy-spawn semantics — the next message
/// rebuilds + SyncSession-hydrates, losslessly; 30 minutes is on the
/// conservative side: rather reclaim too little than hurt a session about to
/// be used.
const IDLE_EVICT_AFTER_SECS: u64 = 30 * 60;
/// Idle-reclaim sweep interval: one round every 5 minutes, a compromise
/// between timeliness and sweep overhead (kept in step with the AcpPool idle
/// reclaim and the embedder idle unload cadence).
const REAP_INTERVAL_SECS: u64 = 5 * 60;

/// Idle-reclaim decision (pure function, easy to unit test): never reclaim
/// when a turn is active (a reservation is held or a terminal state is being
/// closed), when a scheduled turn is in flight (in run_scheduled_turn's
/// spawn→submit window the lifecycle is not yet active), or when it is the
...
/// EnginePool preparatory API (test coverage included; pending Tauri command layer integration); dead code from the lib production perspective.
#[allow(dead_code)]
...
/// Whether two epoch snapshots still point at the same turn, used by cancel
/// for guarding across the turn_lock boundary.
///
...
/// (idempotent, executed in both phase one and two; the implementation may
/// also try_send the cascade cancel, best-effort). `cascade_cancel`
/// asynchronously sends the cascade cancel (CancelSubAgents) and **is only
/// awaited in phase two while holding `turn_lock`**: this guarantees the
/// cascade cancel finishes enqueueing before the turn gate is released — the
/// next turn's `SendMessage` must wait for the same `turn_lock`
/// (`send_reserved_user_message`), so the cascade cancel is always enqueued
/// before the new turn's message; FIFO guarantees the engine cancels the old
/// turn's sub-agents before starting the new turn, and a late cascade cancel
/// cannot kill the new turn's just-started sub-agents by mistake (reviewer
/// point 4: spawning the send asynchronously loses the enqueue-order
/// guarantee relative to the next SendMessage).
///
        ...
        if let Some(engine) = get_engine().await {
            if generation_matches(
                ...
                if let Some(lifecycle) = lifecycle.as_ref() {
                    // arm uses the initiation-time snapshot target (already
                    // validated as still the target turn); the forwarder
                    // re-validates the epoch when it takes pending_cancel
                    // after TurnStarted, and a cross-turn stale pending is
                    // dropped. arm + cancel_current complete atomically
                    // inside the state lock (same as phase one, reviewer
                    // point 8): reserve_turn needs the same state lock, so no
                    // turn switch can be inserted between "check/arm" and
                    // "cancel". Returning false = the turn switched after the
                    // recheck passed: skip cancel and cascade side effects,
                    // avoiding hitting the new turn's live token (reviewer
                    // point 6).
                    ...
                    cancel_current(&engine);
                    // cascade is meaningless when the lifecycle is missing
                    // (no active turn): the cascade cancel CancelSubAgents
                    // targets the engine's current sub-agents, and an idle
                    // engine has none, so not sending it is lossless (keeps
...
/// A session's resident entry in the pool: the engine + its dedicated event
/// forwarder task.
struct EngineEntry {
    engine: AppEngine,
    ...
    /// Engine epoch (UNIX ms): a "still running" record on the worker ledger
    /// counts as truly alive only if it had activity within this epoch. The
    /// foundation's restart load only replays in-memory state and does not
    /// write the on-disk running entries back (subagent/mod.rs's load path);
    /// without this vetting, a previous process's zombie workers would show
    /// as "working" again after the parent session rebuilds its engine and
...
pub struct EnginePool {
    ...
    /// All sessions share the one booted bridge (boot writes to disk / sets
    /// env and can run only once). commands read model / workspace through it
    /// as well.
...
/// Background sweep task handle: Drop cancels first and aborts second,
/// double-insuring the sweep stops (same pattern as scheduled/tasks.rs
/// ScheduledTaskState's Drop cleanup).
...
impl EnginePool {
    ...
    /// genuinely idle — double-insured on the conservative side.
    ...
    pub fn start_idle_reaper(&self) {
        ...
        let handle = tauri::async_runtime::spawn(async move {
            ...
            // tokio::interval's first tick fires immediately: skip it so
            // everything follows the periodic cadence.
            interval.tick().await;
            loop {
                tokio::select! {
                ...
                // Per-round panic isolation: a reclaim-logic panic would take
                // down the whole sweep async task (silently stalling it, with
                // engines staying resident forever). Each round spawns
                // independently; a panic kills only that round and the outer
    ...
    /// Called after a model configuration or a user-managed credential is
    /// saved successfully. Only bumps a non-sensitive in-memory revision
    /// number; engines already generating are not interrupted immediately —
    /// they are safely reclaimed and rebuilt before the next turn is sent.
    ...
    /// Synchronous version of the online-session composed-directory rewrite
    /// (**only for synchronous callers not on a tokio runtime**:
    /// `blocking_lock` panics on a runtime thread; async commands must use
    /// [`Self::refresh_live_sessions_skills`] instead). The composed
    /// directory is small and the diff rewrite is extremely fast.
    ...
    /// Build the session's bridge for a standalone call. Shares the same
    /// runtime provider as EnginePool's lazy spawn, so side entrances such as
    /// review do not bypass runtime credential preparation either.
    ...
    async fn finalize_runtime_bridge(
        ...
        // Local endpoint (an OpenAI-compatible preset pointing at a
        // local/intranet service): probe the server kind (Ollama / vLLM /
        // LM Studio / generic) so thinking control uses the matching
    ...
    /// Create and load a one-shot eval session. The eval runner predetermines
    /// the session ID so reports and cleanup correlate precisely; ordinary
    /// GUI sessions keep using SessionStore's auto-generated IDs.
    #[cfg(any(feature = "benchmark-hooks", test))]
    ...
    /// Send a user message to the specified session's engine (lazy spawn if
    /// not running).
    #[allow(dead_code)]
    ...
    pub async fn cancel(&self, session_id: &str, keep_inbox: bool) -> CancelOutcome {
        ...
        let (target, claimed_unsubmitted) = cancel_turn_with_gates(
            ...
            // cancel_current: synchronously cancel the in-pool engine
            // (idempotent), and best-effort try_send the cascade cancel of
            // that engine's background sub-agents (multiagent, ADR-0006): the
            // "stop" button is a sub-agent's only deterministic stop entry
            // (cards have no cancel button and a natural-language instruction
            // is only a suggestion); canceling only the host turn would leave
            // background sub-agents burning money. try_send does not block
            // and enqueues immediately when the channel has room (earlier
            // than the next SendMessage); when the channel is full (capacity
            // 32) it gives up, and phase two plus the mismatch resend paths
            // guarantee delivery with a locked await (reviewer point 9 + G1).
            |engine| {
    ...
    /// pinvou3 tool toggles (globally persisted): broadcast the "disabled
    /// tool full names" (model-visible full names, lowercase) to **every
    /// running session engine** → written into each config.disallowed_tools
    /// and hidden from the next turn. Sessions not running read the initial
    /// value from the persisted list at their next spawn
    /// (build_engine_config), so new windows/new conversations inherit the
    /// same disabled state.
...
mod scheduled_model_tests {
    ...
    async fn stale_cancel_after_turn_change_leaves_new_turn_intact() {
        ...
        let lifecycle = lifecycles.for_session(sid);
        // turn1: on_submitted activates (active+submitted+epoch increments),
        // so phase one's cancel_engine can match the generation, and
        // finish_once can claim (requires submitted).
        ...
        let cancel_task = tokio::spawn(async move {
            cancel_turn_with_gates(
                ...
                || async { Some(()) },
                // cancel_current: both phase one and the phase-two recheck
                // go through here — told apart by a counter.
                // phase one (turn1) takes probe1 0→1; a mistaken phase-two
                // execution sets probe2.
                move |_engine: &()| {
    ...
    async fn fresh_cancel_after_turn_change_still_cancels_new_turn() {
        ...
        cancel_turn_with_gates(
            ...
            || async { Some(()) },
            // cancel_current: record the trigger.
            move |_engine: &()| {
    ...
    async fn stale_cancel_retries_cascade_when_phase_one_try_send_failed() {
        // Deterministic regression of reviewer point 9: when phase 1's
        // best-effort try_send fails because the ops channel is full
        // (capacity 32), CancelSubAgents is never enqueued; if the old turn
        // then ends and a new turn completes reserve before phase 2 acquires
        // the turn gate, phase two returning directly on the generation
        // mismatch would lose the cascade cancel forever, and the old turn's
        // detached sub-agents keep running. After the fix, the mismatch
        // branch must resend the cascade once under the lock when the new
        // turn has not been submitted yet (only reserved, not sent —
        // SendMessage needs the same turn_lock, held by this function, so the
        // engine still has only the old turn's leftover sub-agents) (after
        // simplification ③ there is no longer a distinction of whether
        // try_send failed: resend uniformly per should_retry_cascade —
        ...
        let lifecycle = lifecycles.for_session(sid);
        // turn1: on_submitted activates (active+submitted+epoch=1).
        assert!(lifecycle.on_submitted());
        ...
        // phase 1's try_send failed (ops channel full): the resend path does
        // not rely on the failure record — it is decided uniformly by
        // should_retry_cascade (new turn not submitted).
        ...
        let cancel_task = tokio::spawn(async move {
            cancel_turn_with_gates(
                ...
                sid,
                deepseek_tui::core::engine::CancelMode::StopDropInbox,
                // get_engine: engine present (phase one and the resend
                // recheck both return Some).
                || async { Some(()) },
                // cancel_current: phase one runs it once (canceling the old
                // turn).
                move |_engine: &()| {
                    probe_cancel.store(true, Ordering::Release);
                },
                // cascade_cancel: the new turn is not submitted → the
                // mismatch branch must resend; the cascade cancel must not be
                // dropped on the generation mismatch.
                move |_engine: &()| {
                    probe_cascade.store(true, Ordering::Release);
                    async {}
                },
        ...
        // turn1 terminal (submitted → claim path) → turn2 reserve (epoch=2,
        // not submitted — SendMessage is blocked by the turn_lock phase two
        // holds).
        assert!(lifecycle.finish_once(|| {}).is_some());
        let reservation2 = lifecycle.reserve().expect("turn2 reserve");

        // Release turn_lock: phase two resumes, current=Some(2) ≠
        // target=Some(1) → mismatch → should_retry_cascade (new turn not
        // submitted) → resend the cascade cancel.
        drop(blocker);
        drop(gate);
        cancel_task.await.expect("cancel task joins");
    ...
    async fn stale_cancel_skips_cascade_retry_when_new_turn_already_submitted() {
        ...
        let lifecycle = lifecycles.for_session(sid);
        // turn1: on_submitted activates (epoch=1).
        assert!(lifecycle.on_submitted());
    ...
    async fn stale_cancel_retries_cascade_when_mismatch_surfaces_after_phase_two_engine_lookup() {
        ...
        let cancel_task = tokio::spawn(async move {
            cancel_turn_with_gates(
                ...
                // cancel_current: phase one fires once (canceling T1); phase
                // two must skip it on the later mismatch.
                move |_engine: &()| {
    ...
    async fn stale_cancel_skips_unsubmitted_claim_and_leaves_new_turn_intact() {
        ...
        let cancel_task = tokio::spawn(async move {
            cancel_turn_with_gates(
                ...
                || async { None::<()> },
                // cancel_current: must not be called when the engine is
                // absent.
                |_engine: &()| {},
                // cascade_cancel: engine absent, not called in phase two.
                |_engine: &()| async {},
                move |lc, _target| {
                    probe.store(true, Ordering::Release);
    ...
    async fn stale_cancel_claim_with_epoch_guard_rejects_new_turn() {
        ...
        cancel_turn_with_gates(
            ...
            // claim_unsubmitted.
            || async { None::<()> },
            // cancel_current: engine absent, must not be called.
            |_engine: &()| {},
            // cascade_cancel: engine absent, not called in phase two.
            |_engine: &()| async {},
    ...
    async fn stale_cancel_during_phase_one_engine_lookup_leaves_new_turn_intact() {
        ...
        let cancel_task = tokio::spawn(async move {
            cancel_turn_with_gates(
                ...
                // cancel_current: must not be called (epoch mismatch after
                // phase one's await).
                move |_engine: &()| {
                    probe.store(true, Ordering::Release);
                },
    ...
    async fn pending_cancel_is_armed_before_cancel_current_in_phase_two() {
        ...
        let lifecycle = lifecycles.for_session(sid);
        // turn: on_submitted activates (submitted but not started, turn_id
        // still None, epoch=1) — arm_pending_cancel's precondition holds.
        assert!(lifecycle.on_submitted());
        ...
        cancel_turn_with_gates(
            ...
            // get_engine: engine present.
            || async { Some(()) },
            // cancel_current probe: counts only. The cancel executes inside
            // the state lock, so the probe must not take the lifecycle lock
            // again (std Mutex is not reentrant — would deadlock); instead it
            // verifies after the call that the pending can still be consumed
            // by the forwarder.
            move |_engine: &()| {
        ...
        // arm before cancel: after the cancel runs, the pending can still be
        // consumed by the forwarder (simulating a take-and-replay when
        // TurnStarted arrives).
