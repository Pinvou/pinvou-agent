...
async fn serve_probe_connection(
    ...
    child_prompt_sentinel: Arc<String>,
    ...
    probe
        ...
        .push(body.clone());
    let (content_type, response_body) = if body.contains(child_prompt_sentinel.as_str()) {
        ...
        probe.parent_followups.fetch_add(1, Ordering::SeqCst);
...
async fn start_spawn_probe(
    ...
    let child_prompt_sentinel = Arc::new(child_prompt_sentinel);
    ...
    let task = tokio::spawn(async move {
        while let Ok((stream, _)) = listener.accept().await {
            tokio::spawn(serve_probe_connection(
                ...
                Arc::clone(&child_prompt_sentinel),
...
/// 真实穿过普通 PinvouOS Front 的 turn route：系统 Orchestrator 既要出现在
/// Engine 初始 roster，也要在 SendMessage 覆盖 route 后仍能被 spawn refresh
/// 解析，并把完整后台契约送进子 Agent prompt。
#[tokio::test(flavor = "current_thread")]
#[allow(clippy::await_holding_lock)]
async fn pinvou_os_front_route_real_spawn_resolves_system_orchestrator() {
    let _env_lock = crate::platform::paths::tests::ENV_LOCK
        .lock()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let env_names = [
    ...
    let root = unique_temp_root("pinvou-os-orchestrator-spawn");
    ...
    let child_prompt_sentinel = "# PinvouOS Orchestrator Agent";
    let (base_url, probe, server_task) = start_spawn_probe(
        crate::features::pinvou_os::ORCHESTRATOR_PROFILE_ID.to_string(),
        child_prompt_sentinel.to_string(),
    )
    .await;
    std::env::set_var("DEEPSEEK_BASE_URL", base_url);

    let bridge = Pinvou3Bridge::boot_with_workspace(workspace).expect("boot bridge");
    let session_id = "pinvou-os-front-a";
    let roots = bridge.session_roots(session_id);
    let fleet_config = crate::features::pinvou_os::pinvou_os_fleet_config(None);
    let mut engine_config = bridge.build_engine_config_for_session_roots(session_id, roots);
    ...
    assert!(
        engine_config
            .fleet_roster
            .get(crate::features::pinvou_os::ORCHESTRATOR_PROFILE_ID)
            .is_some(),
        "system Orchestrator must be visible before the first turn"
    );
    engine_config.snapshots_enabled = false;
    engine_config.terminal_chrome_enabled = false;
    let mut dt_config = bridge.build_dt_config();
    dt_config.fleet = Some(fleet_config);
    let (engine, handle) = Engine::new(engine_config, &dt_config);
    let run_task = tokio::spawn(engine.run());
    let op = bridge
        .build_pinvou_os_send_message_op(
            session_id,
            "Research, implement, and verify this complex request.".to_string(),
            AppMode::Yolo,
            None,
            false,
        )
        .expect("build PinvouOS Front turn");
    handle.send(op).await.expect("send Front turn");
    ...
    while !(saw_agent_complete
        && saw_parent_complete
        && probe.parent_followups.load(Ordering::SeqCst) > 0)
    {
        let event = tokio::time::timeout_at(deadline, events.recv())
            .await
            .expect("timed out waiting for system Orchestrator spawn")
    ...
    assert!(saw_agent_tool_success, "agent tool did not finish");
    assert!(saw_agent_spawned, "no AgentSpawned event was emitted");
    assert!(
        errors
            .iter()
            .all(|message| !message.contains("Unknown fleet role/profile")),
        "turn-time route lost the system Orchestrator profile: {errors:?}"
    );
    assert_eq!(probe.child_requests.load(Ordering::SeqCst), 1);
    assert_eq!(
        probe.parent_followups.load(Ordering::SeqCst),
        1,
        "Front must resume after the Orchestrator result and own the final turn"
    );
    assert!(
        probe
            .request_bodies
            ...
            .iter()
            .any(|body| body.contains(child_prompt_sentinel)
                && body.contains("STATUS: NO_OP")
                && body.contains("Front 始终持有用户关系")),
        "child request did not receive the Orchestrator authority contract"
    );
...
async fn code_session_real_spawn_refresh_resolves_config_expert_without_project_writes() {
    ...
    let (base_url, probe, server_task) =
        start_spawn_probe(profile_id.clone(), EXPERT_PROMPT_SENTINEL.to_string()).await;
    std::env::set_var("DEEPSEEK_BASE_URL", base_url);
