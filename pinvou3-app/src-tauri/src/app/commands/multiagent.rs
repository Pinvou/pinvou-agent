...
/// - 资源护栏：父模型是唯一调度者，每次 `agent` 委派使用 `max_depth=0`，
///   子智能体不再委派；最多 4 个同时执行、8 个排队 + 执行。复杂任务由父模型
///   分批派发和接力，不递归裂变。
...
pub(crate) fn delegation_reminder() -> String {
    ...
    format!(
        "本会话已开启多智能体模式：请按任务形态**主动委派**，工具面与普通\
         ...
         3. 资源边界：你是唯一调度者，每次调用 `agent` 都设 `max_depth=0`，\
         子智能体不得再创建子智能体。同时执行最多 4 个，排队与执行合计\
         最多 8 个；复杂任务由你等待、复用结果后分批派发，不要递归裂变；\n\
    ...
    fn delegation_reminder_teaches_delegation() {
        ...
        assert!(
            msg.contains("`max_depth=0`")
                && msg.contains("子智能体不得再创建子智能体")
                && msg.contains("同时执行最多 4 个")
                && msg.contains("最多 8 个"),
            "多智能体必须明确直属委派和并发/准入资源边界"
        );
        assert!(
