...
/// 用户在前端明确「拒绝」一个被拦截的 T3 动作：清除 pending，并把该动作
/// 摘要记入循环抑制表（同会话同摘要在确认 TTL 内的重试直接被拒且**不再
/// 重发确认事件重开对话框**——consent fatigue 防护，round-6 评审）。没有
/// 这条命令时，拒绝只关前端对话框，模型在 TTL 内重试会不断重开阻塞式
/// modal，唯一应用内出口是批准。
#[tauri::command]
