...
/// 删除项目:会话只被解绑(回落自动/隐式分组),永不删除。返回受影响会话 id,
/// 但当前前端直接丢弃返回值(handleDeleteProject 无消费),该字段是协议面预留,
/// 供后续提示功能使用。
#[tauri::command]
...
pub async fn move_session_to_project(
    ...
    // and the store re-validates overlap, so committing mid-rebind can both
