...
/// 自动探测回填(「自动探测」档保存时,一律探测,无表内加速):
/// 1. 连接探测不通 → 未知 → 回填 `Pinvou`(pinvou 决策,不做识图探测);
/// 2. 连接通 → 识图探测(探测链路与真实链路对齐:always-thinking 模型注入 thinking):
///    - 识别出测试色 → `Enabled`(支持图片);
///    - 未能正确响应(unsupported/unverified/error)→ `Disabled`(不支持图片);
///    - 意外状态 → 保持 `Auto`,不冒充结论。
    ...
    match probe.status.as_str() {
        ...
        "unsupported" | "unverified" | "error" => {
            ...
            ImageProbeOutcome {
                ...
                applied_override: Some("disabled".to_string()),
                summary: format!(
                    "未能正确响应识图探测，已设为「不支持图片」：{}",
                    probe.summary
                ),
            }
        }
    ...
    if status.is_success() {
        return match extract_chat_reply_summary(body) {
            Some(reply) if !reply.is_empty() => {
                ...
                // ① 识别出测试色 → 支持;
                // ② 模型明确说看不到图片 → 不支持图像识别;
                // ③ 其余未识别出测试色(描述图片/模板话术/给错颜色)→
                //    未能正确识别图像,原因未知——可能是请求层(图片未送达/
                //    网关剥离)、模型行为(拒绝回答)或能力,不冒充结论。
                if recognized {
...
pub async fn run_image_capability_probe(
    ...
    if let Err(e) = reqwest::Url::parse(&url) {
        return image_capability_result("error", false, format!("服务地址格式不正确: {e}"), None);
    }
    let client = match reqwest::Client::builder()
