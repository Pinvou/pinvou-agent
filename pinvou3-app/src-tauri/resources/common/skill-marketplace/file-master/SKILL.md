...
7. 仍无结果 → **按盘符组成逐盘定向补搜**：结合 `disk_layout` 的盘符列表，按用户线索优先级对每个盘根 `dir` 定向搜（如 `dir="D:\\"` 搜 D 盘根、`dir="D:\\myWork"` 定向项目目录），每次定向都有完整预算（8 秒），比默认主目录搜索更集中；目录名像项目/工作目录时优先试常见盘符根。所有盘都搜过仍无 → 如实说明"实时搜索未找到"，列出已尝试的关键词与已覆盖目录（`searched_dirs`）。向用户追问大致位置（拿到位置 → 回第 2 步定向搜）或更多线索（大致日期、文件格式）。**要按内容找 → `grep_files`**。
