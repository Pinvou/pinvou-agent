...
| # | 规范 |
|---|------|
| 1 | **串行查详情**：拿到多个 ID 后，逐条串行执行查询、由 AI 在内存合并结果；**严禁** `&` 并行 + `wait`（Windows cmd 下 `&` 是分隔符不是后台符，`wait` 非内置命令） |
| 2 | **翻页**：分页接口须拉全直至无更多 |
| 3 | **优先批量 API**：有批量接口则用批量；无则按 #1 串行 |
| 4 | **群消息**：必须先 `chat search --query` 得 `openConversationId`，再 `chat message list --group`；多群逐条串行查询 |
| 5 | **列表少轮次**：带条件搜索/列表 → 一次采全详情；**禁止**无新参数时重复同一 `list` / `search` |

## 多源信息采集（公共模式）

> recipe 引用方式：`按「多源信息采集」执行（关键词=<X>，时间=<Y>至<Z>）`。

- 逐条串行执行 dws 命令，由 AI 在内存合并结果（**禁止 `&` 并行 + `wait`**）；分页须采全。
- 只保留与主题相关的数据，无关丢弃。
- 有批量详情接口优先；否则逐条串行拉详情（见上表 #1）。
