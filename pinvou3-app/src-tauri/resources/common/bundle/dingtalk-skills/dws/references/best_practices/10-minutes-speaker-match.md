...
> 产品命令见 [minutes.md](../products/minutes.md)、[aisearch.md](../products/aisearch.md)、[chat.md](../products/chat.md)、[doc.md](../products/doc.md)。

| Recipe                    | 行动指南（固定路线）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| minutes-speaker-summarize | 1. **读取转写**：有 URL → 提取 `taskUuid` → `minutes get transcription --id <uuid>`；有时间/关键词 → `minutes list all` 筛选 → 获取转写；无信息 → 必须询问 2. **声纹标注检查**：已标注真实姓名 → 直接跳 Step 6；仅匿名编号（发言人1/2/3）→ 继续 3. **转写原文推断**：利用称呼、自我介绍、上下文指代、发言顺序推断，高置信度 → 跳 Step 6；无法确定 → 继续 4. **多路身份推断**（逐条串行执行、由 AI 在内存合并结果，禁止 `&` 并行 / `wait`）：`aisearch person --query "<姓名>" --dimension name` → `chat message list` 取近期 IM → `drive search --query "<姓名>"` 前 3 篇；两路以上一致才下结论 5. **置信度判断**：置信度分支与用户确认语义以 [minutes.md](../products/minutes.md)「发言人识别与总结执行链路」Step 5 为准 6. **结构化总结输出**：核心观点 + 问题/关注点 + Action Item + 立场态度；输出后追问是否替换发言人标注 |
...
- 用户想纠错发言人标注 → 转 speaker-correct 流程（见 [11-minutes-speaker-correct.md](./11-minutes-speaker-correct.md)）
...
逐条串行执行以下查询、由 AI 在内存合并结果（**禁止 `&` 并行 + `wait`**，见 minutes.md「错误处理」）：
...
> **置信度分支与用户确认语义以 [minutes.md](../products/minutes.md)「发言人识别与总结执行链路」Step 5 为准**，本文件不另设阈值。

需要用户确认/辨认时，列出 TOP3 候选（转写文本片段，必须来自不同发言人，不重复），每人附一段代表性转写片段：
...
- 用户选 1/2/3 → 进入 Step 6
- 用户说"都不是" → 告知无法确认，建议转 speaker-correct 流程（见 [11-minutes-speaker-correct.md](./11-minutes-speaker-correct.md)），**不再重复**
...
- **声纹库命中时直接输出**：不做任何额外确认
- **置信度分支以 minutes.md 为准**：阈值与用户确认语义不另设（见 [minutes.md](../products/minutes.md)「发言人识别与总结执行链路」Step 5）
