...
> **前置条件（MUST READ）：** 执行本命令前，必须先用 `read` 读取以下文件：
...
> **关键原则**：CLI 内置自动分片。超长内容（>10000 字符）自动按 markdown 结构切分后逐片写入，对调用方透明。CLI 不自动回读；写入完成后必须主动回读确认（硬约束，见下文「内容完整性验证」）。
...
某片写入超时后 CLI **不会**自动重试（也无减半逻辑）：直接返回「文档写入响应超时，服务端提交状态未知」错误（`status: unknown`，不可自动重试），避免重复创建或重复追加。处置：先按上文回读确认实际写入状态，仅在确认服务端未提交后重新执行或从断点续写。
...
[INFO] 文档已创建 (nodeId=abc123)，开始分片写入...
[INFO] 写入分片 (1/5)，10000 字符 (create)...
[INFO] 写入分片 (2/5)，10000 字符 (append)...
[INFO] 写入分片 (3/5)，10000 字符 (append)...
[INFO] 写入分片 (4/5)，10000 字符 (append)...
[INFO] 写入分片 (5/5)，5000 字符 (append)...
[INFO] 全部 5 个分片写入完成
{"success": true, "nodeId": "abc123", "chunksWritten": 5}
```

### 写入超时（提交状态未知）

某片写入响应超时时，CLI 不会自动重试，直接返回「文档写入响应超时，服务端提交状态未知」错误（避免重复创建或重复追加）。应对策略：

1. 先用 `dws doc read --node <NODE_ID>` 读取目标文档，确认实际已写入的内容
2. 仅在确认服务端未提交后，重新执行写入或从断点处用 `dws doc update --mode append` 续写
...
| 写入超时错误（提交状态未知） | 先 [`./doc-read.md`](./doc-read.md) 回读确认实际写入状态，仅在确认未提交后再次 `update --mode append` 续写 |
