...
- 禁止 Agent 直接调用 `report query-data` / `report columns` / `check result` / `check record`，这些由脚本内部自动完成（指报表导出工作流内必须走脚本；单次点查场景按 attendance.md 意图表路由）
...
> 预设报表**不会改变报表类型的判断逻辑**，报表类型始终按下方「报表类型（五选一）」规则判断。
...
| 11 | 打卡图片1 | `check record` |
| 12 | 打卡图片2 | `check record` |
| 13 | 打卡设备 | `check record` |
| 14 | 管理员修改备注 | `check record` |
| 15 | 管理员修改备注图片1 | `check record` |
| 16 | 管理员修改备注图片2 | `check record` |
| 17 | 管理员修改备注图片3 | `check record` |

### 签到报表（3 个基础信息列 + 10 个签到字段列 + 9 个图片列）
...
| 序号 | 字段名称 | 数据来源 |
|------|---------|---------|
| 4 | 日期 | `timestamp`（毫秒时间戳转日期） |
| 5 | 时间 | `timestamp`（毫秒时间戳转时间） |
| 6 | 经度 | `longitude` |
| 7 | 纬度 | `latitude` |
| 8 | 地点 | `place` |
| 9 | 详细地址 | `detailPlace` |
| 10 | 拜访客户 | `customers` |
| 11 | 客户部门名称 | 预留（签到接口暂无此字段） |
| 12 | 工作内容 | `remark` |
| 13 | 手机标识 | `mobileId` |
