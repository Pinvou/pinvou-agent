...
> **MUST READ**：动手写 JSONML 前，必须先用 `read` 读取 [doc-jsonml-cookbook.md](../format/doc-jsonml-cookbook.md) — 其中 §决策型文档骨架范例 有可直接复制修改的完整模板。
...
- 返回「文档写入响应超时，服务端提交状态未知」（写入超时，CLI 不自动重试）
- 命令超时或只写入部分分片
- 回读发现后半段缺失、章节乱序或表格损坏
