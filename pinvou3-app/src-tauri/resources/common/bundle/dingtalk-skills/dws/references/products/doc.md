...
> **关键原则**：CLI 内置自动分片。超长内容（>30000 字符）自动按 markdown 结构切分后逐片写入，每一片都是完整自包含的顶层 block 序列（表格切分会重发表头行）。任何改变渲染结构的切分点都会在 `degradations` 里上报，不会静默降级。CLI 不自动回读；写入完成后必须主动回读确认（硬约束，见下文「内容完整性验证」）。
...
- 新增权限 → `permission add`（需 `--node` + `--users` + `--role`，或 `--node` + `--members`）
