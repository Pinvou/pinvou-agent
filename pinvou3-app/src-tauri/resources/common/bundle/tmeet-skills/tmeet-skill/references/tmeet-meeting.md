...
>
> 完整规则（含“不得在 list 上硬塞关键词”、“不得把关键词当时间使用”、“歧义时先澄清”等）见 [SKILL.md 「查询命令选择准则」](../SKILL.md#查询命令选择准则)。

---

## create — 创建会议

> ⚠️ **写操作，执行前请确认用户意图。携带 `--invitees` 时为高风险写操作（最多 100 人，受邀者会收到会议通知）：执行前必须展示会议主题、时间与完整受邀成员名单并获得明确确认（见 [SKILL.md 「安全规则」](../SKILL.md#安全规则)「以下命令操作必须二次确认」表）。**
...
| `--invitees <ids>` | 否 | — | 邀请成员的 openid 列表，逗号分隔或重复传参，最多 100 人，可通过 `contact search` 命令查询获得（属「会议邀请」场景）              |
...
> ⚠️ **`--meeting-type 1` 的使用条件**：仅当需要修改**周期规则**（`--recurring-type` / `--until-type` / `--until-count` / `--until-date`）或修改**某场子会议**（`--sub-meeting-id`）时，必须传 `--meeting-type 1`。**只修改主题、时间、密码、入会限制、水印、录制、邀请列表等与周期规则无关的字段时，不需要传 `--meeting-type`**，服务端不会因此把周期性会议改成普通会议，周期规则不会丢失。
...
| `--timezone <tz>` | 否 | — | 新时区                                              |
| `--meeting-type <n>` | 修改周期规则或子会议时必填 | `0` | 会议类型：`0`-普通，`1`-周期性；仅修改主题/时间等非周期字段时不需要传 |
| `--join-type <n>` | 否 | `0` | 入会限制：`1`-所有成员，`2`-仅受邀，`3`-仅企业内部                  |
| `--waiting-room` | 否 | `false` | 开启等候室                                            |
| `--recurring-type <n>` | 周期性时使用 | `0` | 重复类型：`0`-每天，`1`-每周一至五，`2`-每周，`3`-每两周，`4`-每月      |
| `--until-type <n>` | 周期性时使用 | `0` | 结束类型：`0`-按日期，`1`-按次数                             |
...
| `500273 会议已开始，无法修改受邀人` | 会议已开始，无法修改受邀人 | 会议已开始，无法修改受邀人 |
| `500274 会议已取消，无法修改受邀人` | 会议已取消，无法修改受邀人 | 会议已取消，无法修改受邀人 |
| `500275 会议不存在，无法修改受邀人` | 会议不存在，无法修改受邀人 | 会议不存在，无法修改受邀人 |
