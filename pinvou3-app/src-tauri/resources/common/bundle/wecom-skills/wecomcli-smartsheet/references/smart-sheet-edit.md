...
>
> **警惕数据破坏行为**：
> 1. 批量删除记录、清空字段、删除子表等不可逆操作，执行前必须确保用户给出了**明确、具体的保留策略**（如"删除 2026 年 3 月之前的所有记录"、"只保留状态为已完成的行"）。若用户描述模棱两可（如"删除全部"、"删掉就好了"、"清一下"），**必须先向用户确认**具体的删除范围与保留条件，不得直接执行；单次删除影响超过 100 条记录时，无论指令是否明确，均须先向用户说明影响范围并取得确认后方可执行
...
5. **更新记录按批完整提交**——`wecom-cli smartsheet records update` 单次请求内必须完整提交该批记录，**严禁**拆分同批内的记录；待处理总量超过 2000 条时，按每批最多 2000 条分段提交（与下方「记录操作」的批量上限一致）。
