...
> **模糊查询前置 [REQUIRED]**：若本次是"会 / xx会 / xx会议 / 有什么会 / 最近有哪些会"等模糊查询（见 [SKILL.md 查询消歧](../SKILL.md)），除按下面拉会议 `list` 外，必须同时 `读取 wecomcli-calendar 技能` 用相同时间范围拉日程 `list`，把两边结果合并、分「（会议）」「（日程）」两部分汇总展示（同一场会议按主题 + 时间去重）——不论会议是否查到都要查日程。仅当用户明确指向在线会议（入会链接 / 会议号 / 视频会议 / 腾讯会议等）时才只查会议。
