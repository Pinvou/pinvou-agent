...
**`+dropdown-update` 的选项模式（`--options` / `--source-range` 二选一）+ 配色规则**（配色默认即生效；`--highlight=false` 时 `--colors` 被忽略（同 write-cells 规则）；更新会重写完整验证规则，需要保留已有配色时先回读并透传 `--colors`）见 [`lark-sheets-write-cells`](./lark-sheets-write-cells.md) 的「Dropdown 选项 + 配色」节，本文不重复。`+dropdown-delete` 不涉及这些 flag。
