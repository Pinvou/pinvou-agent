...
| 查找 / 替换文本 | `+cells-search`（找，关键字用 `--find`）、`+cells-replace`（替换） | `lark-sheets-search-replace` | `+cells-find`、`--query`（`+find` 是 `+cells-search` 的隐藏别名，不在 `sheets --help` 清单，正式名直给） |
...
- **读表理解优先用 `scripts/lark_*.py`（若可用）**：`lark_inspect_workbook.py` / `lark_detect_subtables.py` / `lark_profile_table.py` 是只读脚本，用来把在线表格整理成结构摘要。**可选增强，不是必经步骤**——脚本随品悟应用内置分发；脚本运行失败或环境缺少 Python 时直接用 CLI 等价路径（对照表见 `lark-sheets-read-data`：`+workbook-info` / `+sheet-info` / 小窗口 `+csv-get`）。它们不替代写入类 shortcut；确认目标区域后，写入仍按对应 reference 执行。
...
2. **sheet 定位（公共四件套 shortcut 必填）**：`--sheet-id` 与 `--sheet-name` 二选一。
   ...
   - **例外**：徽章标 `_公共：URL/token（无 sheet 定位）…_` 的 shortcut（`+workbook-info` / `+workbook-export` / `+batch-update` / `+styles-put` / `+dropdown-update|delete` / `+cells-batch-clear` / `+sheet-create`）不接受公共 sheet 定位——不在此穷举，以各 reference 的徽章为准。`+pivot-create` 用 `--target-sheet-id/name`（XOR，可都不传）。注意 `+table-get` 的 `--sheet-id` / `--sheet-name` 是**可选过滤**（限定只读某一个子表，省略则读全部子表），不是公共四件套那种必填定位。
...
> ⚠️ **high-risk-write 命令清单（exit 10 强确认门禁）**：`+batch-update`、`+cells-clear`、`+cells-batch-clear`、`+sheet-delete`、`+dim-delete`、`+dropdown-delete`、`+history-revert`，以及各对象删除 `+chart-delete` / `+pivot-delete` / `+cond-format-delete` / `+filter-delete` / `+filter-view-delete` / `+sparkline-delete` / `+float-image-delete`。
>
...
推荐写法：payload 写到 cwd 下 `tmp/` 子目录的临时文件（见 `../lark-shared/SKILL.md` 安全规则），再用 stdin 喂进去：

```bash
# TMPFILE 指向 payload 文件（写在 cwd 下 tmp/ 子目录）
...
- **stdin 每次调用只能给一个 flag**：`+table-put` 同时传 `--sheets` 与 `--styles` 两个大 JSON 时，一个走 `-`、另一个走 `@tmp/styles.json`（`@file` 只接受 cwd 下相对路径，**绝对路径会被拒**；正解是 stdin，别 cd、临时文件写 cwd 下 `tmp/` 子目录，别落用户项目目录根）。
