...
| 用户输入 | 动作 |
|----------|------|
| 只给了「会议标题」 | 不确定是日程标题还是即时会议标题，**同时**查 `calendar +search-event --query <标题>` 与 `lark-cli vc +search --query <标题>`（lark-meeting 技能未随包收录，直接用 CLI 命令），交叉后按上述意图分流 |
| 直接给了 `meeting_id` | 直接进入 `lark-cli vc +detail --meeting-ids <meeting_id>`（lark-meeting 技能未随包收录，直接用 CLI 命令），跳过日程 |
