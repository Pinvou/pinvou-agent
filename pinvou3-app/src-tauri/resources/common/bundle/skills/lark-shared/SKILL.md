...
- **临时文件一律写 `tmp/` 子目录,别落工作目录根**：CLI 入参、API 响应、分步数据等中间文件,统一写到 cwd 下的 `tmp/` 子目录(如 `--data @tmp/params.json`、`lark-cli ... > tmp/resp.json`,相对路径仍合法)。原因:工作目录根的文件会被当作「产出物」展示给用户,过程文件会污染产物列表。能用 stdin 不落文件就别落;必须落就放 `tmp/`(必要时先 `mkdir -p tmp`)。
