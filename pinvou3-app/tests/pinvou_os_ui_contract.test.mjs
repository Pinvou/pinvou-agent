...
test('Agent Runtime dock is backed by native runtime truth and has no legacy terminology', () => {
  ...
  assert.doesNotMatch(shell, /getTurnFeedback|turnFeedbackNow|pinvou-os-turn-feedback/);
  assert.doesNotMatch(i18n, /voiceTurnFeedbackReady|voiceTurnFeedbackLong|voiceTurnFeedbackExtended/);
  ...
  assert.match(shell, /className="pinvou-os-thinking"/);
  assert.match(shellCss, /\.pinvou-os-interaction-state\.is-running > span::after[\s\S]*animation: pinvouOsStateSignal/);
  assert.match(shellCss, /@keyframes pinvouOsStateSignal[\s\S]*transform: scale\(1\.65\)/);
  ...
  assert.match(shellCss, /@media \(prefers-reduced-motion: reduce\)[\s\S]*\.pinvou-os-interaction-state\.is-running > span::after,[\s\S]*\.pinvou-os-queued-cancel \{ transition: none !important; \}/);
  ...
  assert.match(shellCss, /\.pinvou-os-voice-shell\.is-voice-transcribing \*::after[\s\S]*animation: none !important;/);
...
test('PinvouOS canvas consumes a read-only namespaced A2UI v0.9 projection', () => {
  ...
  assert.match(surface, /className=\{`pinvou-os-interaction-state is-\$\{interactionState\}`\}/);
