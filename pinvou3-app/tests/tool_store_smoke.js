  ...
  return `(function(){
    ...
    function invoke(cmd,args){
      ...
      switch(cmd){
        ...
        case 'eip_status': return Promise.resolve({connected:state.connected.eip});
        case 'zhidao_status': return Promise.resolve({connected:state.connected.zhidao});
        case 'feishu_ensure_cli': case 'wecom_ensure_cli': case 'dingtalk_ensure_cli': case 'feishu_connect_begin': case 'wecom_connect_begin': case 'dingtalk_connect_begin': case 'eip_connect_begin': case 'zhidao_connect_begin': return Promise.resolve(null);
...
(async () => {
  ...
  const connectors=[
    ...
    ['H3C 员工门户（EIP）','eip','eip:connected',['eip_connect_begin']],
