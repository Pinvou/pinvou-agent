      ...
      switch(cmd){
        ...
        case 'detect_local_vllm_setup': {
          ...
          return Promise.resolve(window.__VLLM_ELIGIBLE__ && engineState !== 'starting'
            ? {eligible:true,is_megacube:true,has_packages:true,vllm_online:false,engine_state:engineState,already_bootstrapped:false}
            : {eligible:false,is_megacube:engineState==='starting',has_packages:engineState==='starting',vllm_online:false,engine_state:engineState,already_bootstrapped:false});
        }
...
(async () => {
  ...
  await page.evaluate(() => { window.__VLLM_STATE__ = 'stopped'; });
