...
(async () => {
  ...
  rec('⑥ 预设模型目录不展示已下线或弃用推荐项', Object.values(freshCatalog).every(Boolean), JSON.stringify(freshCatalog));
  ...
  rec('⑥ 添加预设云模型表单精简且 API Key 前禁用保存', modelPickerWidth >= 430 && modelPickerWidth <= 455 && Object.values(addModelBeforeKey).every(Boolean), JSON.stringify({ modelPickerWidth, ...addModelBeforeKey }));
