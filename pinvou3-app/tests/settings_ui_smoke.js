...
(async () => {
  ...
  const freshCatalog = await page.evaluate(() => {
    const text = document.body.innerText;
    const stale = ['deepseek-chat', 'kimi-k2.5', 'glm-4-plus', 'minimax-m1', 'abab6.5s-chat', 'mimo-v2-flash', 'qwen-max', 'qwen-plus', 'qwen-turbo', 'doubao-pro-256k', 'gpt-4o', 'MiniMax-M2.5', 'MiniMax-M2.5-highspeed', 'qwen3.6-flash', 'qwen3.8-max-preview'];
    return {
  ...
  const tokenPlanModels = await page.evaluate(() => {
    ...
    return {
      ...
      hasFlash: lines.includes('qwen3.7-flash'),
      noPreview: !lines.includes('qwen3.8-max-preview'),
    };
  });
  rec('⑥.1d Token Plan 条目列正式旗舰与 Flash，且不展示已下线预览版', Object.values(tokenPlanModels).every(Boolean), JSON.stringify(tokenPlanModels));
  await clickExact(page, '取消');
  await sleep(300);
