...
const codexZh = {
  ...
  setupHints:{ kimi_cli_missing:'请先安装 Kimi Code CLI', kimi_auth_required:'使用 Kimi 账号完成设备码授权', claude_cli_missing:'请先安装 Claude Code CLI（如 brew install --cask claude-code），并确保 claude 在 PATH 中', claude_auth_required:'使用 Claude 账号完成浏览器授权，或设置 ANTHROPIC_API_KEY' },
...
const codexEn = {
  ...
  setupHints:{ kimi_cli_missing:'Install the Kimi Code CLI first', kimi_auth_required:'Authorize with your Kimi account using the device code', claude_cli_missing:'Install the Claude Code CLI first (e.g. brew install --cask claude-code) and make sure claude is on PATH', claude_auth_required:'Authorize with your Claude account in the browser, or set ANTHROPIC_API_KEY' },
...
const codexJa = {
  ...
  setupHints:{ kimi_cli_missing:'先に Kimi Code CLI をインストールしてください', kimi_auth_required:'Kimi アカウントでデバイスコード認証を完了してください', claude_cli_missing:'先に Claude Code CLI をインストールし（例: brew install --cask claude-code）、claude が PATH にあることを確認してください', claude_auth_required:'Claude アカウントでブラウザー認証を完了するか、ANTHROPIC_API_KEY を設定してください' },
