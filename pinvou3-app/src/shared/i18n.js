...
const dict = {
    ...
    Object.assign(dict.zh.uiToolDetails.tools, {
      ...
      'file-master':{ title:'文件管理大师', subtitle:'本机文件查找、磁盘扫描与回收站安全清理', latency:'本地', desc:'说“帮我找一下文件 / C 盘怎么满了”，AI 秒搜常用目录定位文件（支持扩展名、时间、大小过滤），只读扫描磁盘给出分组风险画像（🟢🟡🔴），按你的点名把文件移入系统回收站（可恢复），支持后台进度轮询、清空回收站与一键误删还原；全程本地、零依赖、数据不出机。', welcomeQueries:['帮我找一下那个报表文件在哪','C 盘空间被什么占了','把下载文件夹里旧的安装包移入回收站'] },
    });
    Object.assign(dict.en.uiToolDetails.tools, {
      ...
      'file-master':{ title:'File Master', subtitle:'Local file search, disk scanning, and safe recycle-bin cleanup', latency:'Local', desc:'Say \u201chelp me find a file / why is my C drive full\u201d and the AI instantly searches common folders to locate the file (with extension, time, and size filters), read-only scans the disk for a grouped risk profile (\ud83d\udfe2\ud83d\udfe1\ud83d\udd34), and once you pick the items, moves them to the system recycle bin (recoverable), with background progress polling, empty-recycle, and one-click restore for accidental deletions. Fully local, zero dependencies, your data never leaves the device.', welcomeQueries:['Find that report file for me','What is taking up my C drive space','Move old installers in Downloads to the recycle bin'] },
    });
    Object.assign(dict.ja.uiToolDetails.tools, {
      ...
      'file-master':{ title:'ファイル管理マスター', subtitle:'ローカルファイル検索、ディスクスキャン、安全なゴミ箱整理', latency:'ローカル', desc:'「ファイルを探して / C ドライブの空きが少ない」と言うと、AI がよく使うフォルダーを即座に検索し（拡張子・時間・サイズで絞り込み可能）、ディスクを読み取り専用でスキャンしてリスク別の概観（🟢🟡🔴）を提示。指定したファイルをシステムのゴミ箱（復元可能）に移し、バックグラウンド進行のポーリング、ゴミ箱の空化、誤削除のワンクリック復元にも対応します。すべてローカル処理・ゼロ依存で、データは端末の外に出ません。', welcomeQueries:['あのレポートファイルを探して','C ドライブの空きを占めているものは？','ダウンロード内の古いインストーラーをゴミ箱へ移動'] },
    });
