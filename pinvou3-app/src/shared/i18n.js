...
const dict = {
      zh: {
        ...
        uiMultiAgent: {
          toggleLabel:'多智能体模式',
          toggleHint:'开启后本会话可主动委派子智能体（专家协作），关闭不影响在跑的子智能体；开启会把本会话的私有工作区初始化为本地 git 仓库并按轮快照（仅本地留痕，不上传、不改全局 git 配置）',
        ...
        depInstalling: '安装中…', depInstallBtn: '一键安装', depGoInstall: '去安装', depInstallNote: '将提权安装缺失依赖（需联网或可用 apt 源）', depInstallNoteWindows: '将下载缺失模型；LibreOffice 通过 winget 提权安装（需联网）',
        ...
        dep_voice_asr_model: '语音识别模型（SenseVoice q8）', dep_knowledge_embedding_model: '知识库向量模型（bge-m3）', dep_git: '多智能体并行隔离（git）',
      ...
      en: {
        ...
        uiMultiAgent: {
          ...
          toggleHint:'Let this conversation delegate to subagents (expert collaboration); turning it off never interrupts running subagents. Enabling also turns this conversation\'s private workspace into a local git repo with per-turn snapshots (local only; nothing is uploaded and your global git config is untouched)',
        ...
        depInstalling: 'Installing…', depInstallBtn: 'Install', depGoInstall: 'Install', depInstallNote: 'Installs missing deps via elevated apt (needs network or an apt source)', depInstallNoteWindows: 'Downloads missing models; installs LibreOffice through elevated winget (requires network)',
        ...
        dep_voice_asr_model: 'Speech recognition model (SenseVoice q8)', dep_knowledge_embedding_model: 'Knowledge embedding model (bge-m3)', dep_git: 'Multi-agent parallel isolation (git)',
      ...
      ja: {
        ...
        uiMultiAgent: {
          ...
          toggleHint:'この会話からサブエージェントへ委任できるようにする（専門家連携）。オフにしても実行中のサブエージェントは中断されない。オンにすると、この会話専用の作業フォルダをローカル git リポジトリ化し、ターンごとにスナップショットします（ローカルのみ。アップロードやグローバル git 設定の変更はありません）',
        ...
        depInstalling: 'インストール中…', depInstallBtn: 'インストール', depGoInstall: 'インストール', depInstallNote: '権限昇格して不足依存をインストール（ネットワークまたは apt ソースが必要）', depInstallNoteWindows: '不足モデルをダウンロードし、LibreOffice は winget で権限昇格してインストール（ネットワークが必要）',
        ...
        dep_voice_asr_model: '音声認識モデル（SenseVoice q8）', dep_knowledge_embedding_model: 'ナレッジ埋め込みモデル（bge-m3）', dep_git: 'マルチエージェント並列分離（git）',
