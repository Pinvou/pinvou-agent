...
Object.assign(dict.zh.uiToolStore, {
  ...
  uploadSkillPackage:'上传插件包', dropSkillZipHere:'松开以导入插件包 (zip)', invalidSkillZipDrop:'仅支持 .zip 插件包', zipTooLarge:mb=>`文件超过 ${mb}MiB 限制，无法导入`, visibilityLoadFailed:'读取可见性配置失败，请重试（未修改已保存的配置）',
...
Object.assign(dict.en.uiToolStore, {
  ...
  uploadSkillPackage:'Upload Plugin Package', dropSkillZipHere:'Drop to import a plugin package (zip)', invalidSkillZipDrop:'Only .zip plugin packages are supported', zipTooLarge:mb=>`File exceeds the ${mb}MiB limit`, visibilityLoadFailed:'Failed to load visibility settings. Please retry (saved settings are unchanged).',
...
Object.assign(dict.ja.uiToolStore, {
  ...
  uploadSkillPackage:'プラグインパッケージをアップロード', dropSkillZipHere:'離してプラグインパッケージ (zip) をインポート', invalidSkillZipDrop:'対応するのは .zip プラグインパッケージのみです', zipTooLarge:mb=>`ファイルが ${mb}MiB の上限を超えています`, visibilityLoadFailed:'表示設定の読み込みに失敗しました。再試行してください（保存済みの設定は変更されません）。',
