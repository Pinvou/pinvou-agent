...
const acpProvidersZh = {
  ...
  switch:'切换', switchOk:'切换成功', switchFail:'切换失败，请重试', useNow:'使用', restoredOfficial:'已恢复官方登录', restoreOfficial:'切回官方', deleted:'已删除', uninstallDone:'卸载完成', currentOverriddenByEnv:'已切换 · 被环境变量覆盖', effectiveTitle:'生效中配置（来自 CLI 配置文件）', noAnthropicEndpointWarning:'该厂商未提供 Anthropic 兼容端点。Claude Code 仅支持 Anthropic 协议，请确认地址可用或改用其他中转。', wireResponses:'Responses',
  ...
  keyAction:'密钥处理', keepKey:'保留已保存的密钥', replaceKey:'替换为新密钥', deleteKey:'删除已保存的密钥', showKey:'显示', hideKey:'隐藏',
  ...
  uninstallBusy:'正在卸载…', uninstallBlocked:'有运行中的会话，请先关闭后再卸载',
  loading:'加载中…', loadFailed:'加载失败', retry:'重试', configUnreadable:'配置文件不可解析', configUnreadableDesc:'该 Agent 的 CLI 配置文件无法解析。为避免覆盖你的配置，当前禁止切换；请手动修复或删除该文件后重试。',
  sessionProvider:'会话 Provider', sessionProviderDesc:'本会话固定使用的 Provider；默认跟随全局设置', sessionOfficial:'跟随全局',
  faultSwitch:'切换 Provider', faultManage:'管理 Provider',
};
const codexEn = {
...
const acpProvidersEn = {
  ...
  keyAction:'Key handling', keepKey:'Keep the saved key', replaceKey:'Replace with a new key', deleteKey:'Delete the saved key', showKey:'Show', hideKey:'Hide',
  ...
  uninstallBusy:'Uninstalling…', uninstallBlocked:'Sessions are still running. Close them before uninstalling.',
  loading:'Loading…', loadFailed:'Failed to load', retry:'Retry', configUnreadable:'Config file unreadable', configUnreadableDesc:'The CLI config file for this agent cannot be parsed. Switching is blocked to avoid overwriting your config; fix or remove the file and try again.',
  ...
  faultSwitch:'Switch provider', faultManage:'Manage providers',
};
const codexJa = {
...
const acpProvidersJa = {
  ...
  keyAction:'キーの処理', keepKey:'保存済みのキーを保持', replaceKey:'新しいキーに置き換え', deleteKey:'保存済みのキーを削除', showKey:'表示', hideKey:'非表示',
  ...
  uninstallBusy:'アンインストール中…', uninstallBlocked:'実行中のセッションがあります。先に閉じてください。',
  loading:'読み込み中…', loadFailed:'読み込みに失敗しました', retry:'再試行', configUnreadable:'設定ファイルを解析できません', configUnreadableDesc:'この Agent の CLI 設定ファイルを解析できません。設定の上書きを避けるため切り替えをブロックしています。ファイルを修正または削除してから再試行してください。',
  sessionProvider:'セッションのプロバイダー', sessionProviderDesc:'このセッションで固定するプロバイダー。デフォルトはグローバル設定に従います', sessionOfficial:'グローバルに従う',
  faultSwitch:'プロバイダーを切り替え', faultManage:'プロバイダーを管理',
};
...
const dict = {
      zh: {
        ...
        composerProjectSkills: '项目技能', composerProjectSkillsDesc: '扫描项目内 .claude/skills 等目录', composerProjectSkillsWarning: '项目内技能文本可能包含恶意指令（提示注入），仅对可信项目开启',
      ...
      en: {
        ...
        composerProjectSkills: 'Project skills', composerProjectSkillsDesc: 'Scan project .claude/skills and similar folders', composerProjectSkillsWarning: 'Project skill files may contain malicious instructions (prompt injection) — enable only for trusted projects',
      ...
      ja: {
        ...
        composerProjectSkills: 'プロジェクトスキル', composerProjectSkillsDesc: 'プロジェクト内の .claude/skills 等をスキャン', composerProjectSkillsWarning: 'プロジェクト内のスキルには悪意のある指示（プロンプトインジェクション）が含まれる可能性があります。信頼できるプロジェクトでのみ有効にしてください',
    ...
    Object.assign(dict.zh.uiToolDetails.tools, {
      ...
      'file-master':{ title:'文件管理大师', subtitle:'本机找文件 / C 盘占用分析 / 回收站安全删除与还原', latency:'本地', desc:'说“帮我找一下那个文件 / C 盘满了”：找文件按文件名/目录名秒搜常用目录并兜底主目录（AppData 下应用目录也能命中）；磁盘清理只读扫描系统盘与非系统盘并逐层下钻，按 🟢🟡🔴 三级风险说清“这是什么数据”，你点名的文件后台移入系统回收站（内置系统目录白名单，绝不物理删除）并自动记录删除日志，删错了可按日志一键还原，还能一键清空回收站释放空间。纯本地零依赖、数据不出机。安装后同名「文件管理大师」技能卡一并生效。⚠️ 危险操作提示：清空回收站与物理清除不可恢复；清理前请逐项核对。本工具通过白名单与双重确认保障安全，但不对误删或清理造成的损失承担责任。', welcomeQueries:['帮我找一下上周的报表文件','C盘满了，看看什么占空间','清理一下浏览器缓存'] },
    });
    Object.assign(dict.en.uiToolDetails.tools, {
      ...
      'file-master':{ title:'File Master', subtitle:'Find local files / C: drive usage analysis / safe Recycle Bin deletion and restore', latency:'Local', desc:'Say \\u201cfind that file for me / my C: drive is full\\u201d: file search matches file and folder names across common directories with a home-directory fallback (including app directories under AppData); disk cleanup read-only scans system and non-system drives and drills down level by level, explaining \\u201cwhat this data is\\u201d with 🟢🟡🔴 three-level risk labels. Files you explicitly pick are moved to the system Recycle Bin in the background (built-in system-directory whitelist, never physically deleted) with an automatic deletion log, so mistakes can be restored from the log, and you can empty the Recycle Bin to free space. Fully local, zero dependencies, your data never leaves the device. Installing it also enables the \\u201cFile Master\\u201d skill card of the same name. ⚠️ Danger: emptying the Recycle Bin and permanent erasure are irreversible; review each item before cleaning. This tool uses whitelists and double confirmation to reduce risk, but is not liable for accidental deletion or data loss caused by cleaning.', welcomeQueries:['Find last week\\u2019s report file','My C: drive is full\\u2014see what is taking up space','Clean up the browser cache'] },
    });
    Object.assign(dict.ja.uiToolDetails.tools, {
      ...
      'file-master':{ title:'ファイルマスター', subtitle:'ローカルファイル検索 / C ドライブ使用状況分析 / ごみ箱への安全な削除と復元', latency:'ローカル', desc:'「あのファイルを探して / C ドライブがいっぱい」と言うと：ファイル検索はファイル名・フォルダー名でよく使うディレクトリを即座に検索し、ホームディレクトリ全体にフォールバックします（AppData 配下のアプリディレクトリも対象）。ディスククリーンアップはシステムドライブと他ドライブを読み取り専用でスキャンして階層をたどり、🟢🟡🔴 の3段階リスクで「これは何のデータか」を説明します。指定したファイルはバックグラウンドでシステムのごみ箱に移動します（システムディレクトリのホワイトリスト内蔵、物理削除は一切なし）。削除ログを自動記録するため、誤削除はログからワンクリックで復元できます。ごみ箱を空にして容量を解放することもできます。完全ローカル・依存ゼロで、データは端末の外に出ません。インストールすると同名の「ファイルマスター」スキルカードも有効になります。 ⚠️ 危険な操作の注意：ごみ箱を空にする操作と物理削除は取り消せません。削除前に各項目を確認してください。本ツールはホワイトリストと二重確認でリスクを軽減しますが、誤削除や削除によるデータ損失について責任を負いません。', welcomeQueries:['先週のレポートファイルを探して','C ドライブがいっぱい。何が容量を使っているか見て','ブラウザーキャッシュをクリーンアップ'] },
    });
...
Object.assign(dict.zh.uiToolStore, { storeData: {
  skills: {
    ...
    'file-master': { title:'文件管理大师', subtitle:'本机找文件 / C 盘占用三级风险分析 / 回收站安全删除与还原', desc:'一个技能管好本机文件：找文件按文件名/目录名秒搜常用目录并兜底主目录（AppData 下应用目录也能命中），配合搜索阶梯与中文 Windows 路径先验提高一次命中率；磁盘清理只读扫描系统盘与非系统盘并逐层下钻，按 🟢🟡🔴 三级风险说清“这是什么数据”，你点名的项后台移入系统回收站（可恢复，绝不物理删除），删错了可按日志一键还原，还能一键清空回收站释放空间。随工具商店的「文件管理大师」工具挂载生效。⚠️ 危险操作提示：清空回收站与物理清除不可恢复；清理前请逐项核对。本工具通过白名单与双重确认保障安全，但不对误删或清理造成的损失承担责任。', latency:'本地' },
  },
...
Object.assign(dict.en.uiToolStore, { storeData: {
  skills: {
    ...
    'file-master': { title:'File Master', subtitle:'Find local files / C: drive three-level risk analysis / safe Recycle Bin deletion and restore', desc:'One skill to manage local files: file search matches file and folder names across common directories with a home-directory fallback (including app directories under AppData), using a search ladder and Chinese Windows path priors for first-try hits; disk cleanup read-only scans system and non-system drives and drills down level by level, explaining "what this data is" with 🟢🟡🔴 risk labels. Items you pick are moved to the system Recycle Bin in the background (recoverable, never physically deleted), mistakes can be restored from the log, and you can empty the Recycle Bin to free space. Takes effect with the "File Master" tool from the Tool Store. ⚠️ Danger: emptying the Recycle Bin and permanent erasure are irreversible; review each item before cleaning. This tool uses whitelists and double confirmation to reduce risk, but is not liable for accidental deletion or data loss caused by cleaning.', latency:'Local' },
  },
...
Object.assign(dict.ja.uiToolStore, { storeData: {
  skills: {
    ...
    'file-master': { title:'ファイルマスター', subtitle:'ローカルファイル検索 / C ドライブ3段階リスク分析 / ごみ箱への安全な削除と復元', desc:'1つのスキルでローカルファイルを管理：ファイル検索はファイル名・フォルダー名でよく使うディレクトリを即座に検索し、ホームディレクトリ全体にフォールバックします（AppData 配下のアプリディレクトリも対象）。検索ラダーと中国語 Windows のパス知識で初回ヒット率を高めます。ディスククリーンアップはシステムドライブと他ドライブを読み取り専用でスキャンして階層をたどり、🟢🟡🔴 の3段階リスクで「これは何のデータか」を説明します。指定した項目はシステムのごみ箱に移動します（復元可能、物理削除は一切なし）。誤削除はログからワンクリックで復元できます。ツールストアの「ファイルマスター」ツールと一緒に有効になります。 ⚠️ 危険な操作の注意：ごみ箱を空にする操作と物理削除は取り消せません。削除前に各項目を確認してください。本ツールはホワイトリストと二重確認でリスクを軽減しますが、誤削除や削除によるデータ損失について責任を負いません。', latency:'ローカル' },
  },
