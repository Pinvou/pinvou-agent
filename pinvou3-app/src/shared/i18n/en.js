...
Object.assign(dictEn.uiAttachments, {
  ...
  localEngineInstallPrompt:'The local image engine is not installed yet. Install it now? You can also install or uninstall it in Settings → Models → Local Vision.',
  localEngineInstallConfirm:'Install and continue', localEngineInstallCancel:'Not now',
  localEngineNotRunningPrompt:'The local image engine is not running, and auto-start is set to “Never”.',
  localEngineGoSettings:'Start from settings', localEngineSendFallback:'Send via cloud vision model this time',
...
dictEn.uiSettingsDetail.llamaEngine = {
  title:'Local Multimodal Engine',
  introWhatLabel:'What', introWhat:'Runs the open-source vision model (Qwen3-VL) on this device. Images are analyzed locally and never uploaded.',
  introWhenLabel:'When', introWhen:'When your chat model cannot see images, or you prefer not to send images to a cloud provider.',
  introHowLabel:'Enable', introHow:'Download the engine and a model below, then start the service. Keep “Auto fallback for image recognition” on so images are analyzed locally by default.',
  introPerfLabel:'Speed', introPerf:'Recognition speed depends on hardware. A high-performance GPU or CPU is recommended; the first model load takes a while.',
  setupSection:'Setup & model', behaviorSection:'Behavior', engineReady:'Engine installed', engineMissing:'Engine not installed',
  modelReady:'Model ready', modelMissing:'Model not downloaded', running:'Running', starting:'Starting…', stopped:'Stopped',
  runningWithPort:port=>`Running (127.0.0.1:${port})`,
  downloadingEngine:'Downloading engine', downloadingModel:'Downloading model', cancelDownload:'Cancel',
  engineLabel:'Engine', modelLabel:'Vision model', deviceLabel:'Device', serviceLabel:'Service',
  installEngine:'Download engine (~40MB)', installModel:'Download', gpu:'GPU (Vulkan / Metal)', cpu:'CPU',
  deviceAuto:'Auto', deviceAutoGpu:'Usable GPU detected', deviceAutoCpu:'No usable GPU detected; CPU will be used',
  recommended:'Recommended', default:'Default', start:'Start engine', stop:'Stop engine', deleteModel:'Delete', deleteEngine:'Uninstall',
  deleteRequiresStop:'Stop the engine before deleting',
  deleteEngineConfirm:tag=>`Uninstall the local engine${tag ? ` (v${tag})` : ''}? Model files are kept and work again after reinstalling the engine.`,
  restartApply:'Restart & apply', restartNeeded:'The model change takes effect after an engine restart',
  deleteModelConfirm:name=>`Delete the local model files for “${name}”? You can download them again later.`,
  viewLogs:'View logs', privacyNote:'Listens on 127.0.0.1 only. Models and images never leave your computer.',
  ...
  visionFallbackDesc:'When enabled, text-only models without a configured vision model use the local engine to describe images. Models with an explicit vision model (or “Local vision engine” selected) keep their own configuration and are never taken over.',
  localEngineOption:'Local image engine', autoStartLabel:'Auto-start engine', autoStartFirstImage:'On first image send',
  autoStartLaunch:'When pinvou launches', autoStartNever:'Never auto-start',
  autoStartFirstImageDesc:'Zero footprint until your first image; the first recognition waits for the model to load',
  autoStartLaunchDesc:'Pre-starts the engine at app launch for instant recognition; the engine stays resident in memory',
  autoStartNeverDesc:'No automatic start; start manually on this page or choose when sending an image',
