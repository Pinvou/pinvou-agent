...
const jaBase = {
  ...browserJa,
        ...
        uiProjects: { convertToProject:'プロジェクトに変換', projectNamePlaceholder:'プロジェクト名', renameProject:'プロジェクトの名前を変更', deleteProject:'プロジェクトを削除', deleteConfirmLabel:'このプロジェクトを削除しますか？', deleteProjectHint:'会話は削除されず、未分類に戻ります', moveToProject:'プロジェクトへ移動…', moveOut:'プロジェクトから外す', moveToUngrouped:'未分類へ移動', temporaryBadge:'一時ワークスペース', folderUnavailable:'フォルダー利用不可', rebindFolder:'再バインド', addFolderTitle:'フォルダーをプロジェクトに追加しますか？', addFolderBody:folder=>`プロジェクトにこの会話の現在の作業ディレクトリ（${folder}）が含まれ、そこで新規作成した会話は自動的にこのプロジェクトに入ります。`, searchPlaceholder:'プロジェクトを検索', noProjects:'プロジェクトはまだありません', ungrouped:'未分類', currentProject:'現在のプロジェクト', movedNotice:'会話を移動しました', movedNoticeWithFolder:folder=>`会話を移動し、フォルダー ${folder} を追加しました`, addFolderConfirm:'追加して移動', moveOnly:'移動のみ', noMatchProject:'一致するプロジェクトがありません', opFailed:'プロジェクト操作に失敗しました', rebindTitle:'プロジェクトフォルダーを再バインド', rebindConfirm:'移動して再バインド', rebindSessionsHint:n=>`${n} 件の会話が新しいフォルダーを使用します`, rebindSuccess:n=>`${n} 件の会話を再バインドしました`, rebindOldExistsWarn:'元のフォルダーがまだ存在します。切り替えると 2 か所が別々のディレクトリを指します。続行しますか？' },
