...
const jaBase = {
  ...browserJa,
        ...
        uiKeychain: { accessibleFolders:n=>`アクセス可能な ${n} 個のフォルダー`, primaryBadge:'主', unknownDirectory:'不明なディレクトリ', alignAction:'プロジェクトに合わせる', alignActionDesc:'この会話のアクセス可能フォルダーをプロジェクトの現在のセットに更新します（次のターンから有効）', alignDone:'プロジェクトに合わせました', alignNoChange:'すでに最新です', alignBusy:'現在のターン終了後に再試行してください', alignFailed:'プロジェクトに合わせる操作に失敗しました' },
        ...
        uiWorkspacePicker: { title:'ワークスペースを選択', searchPlaceholder:'プロジェクトやパスを検索', empty:'プロジェクトはまだありません。フォルダーを参照するか、一時チャットを開始できます', noMatch:'一致するプロジェクトがありません', multiRootSummary:(n, primary)=>`${n} 個のフォルダー · 既定: ${primary}`, primaryRootBadge:'既定', noticeRestricted:n=>`この会話はこのプロジェクトの ${n} 個のフォルダーにアクセスできます`, noticeVisibility:n=>`この ${n} 個のフォルダーがこの会話に関連していることがモデルに伝わります`, temporary:'一時チャット', temporaryDesc:'ディレクトリに紐付けません', browse:'他のフォルダーを参照…', browseDesc:'選択したフォルダーを起点に新しい会話を開始', excludedTitle:'このフォルダーの自動プロジェクト化はオフです', excludedBody:name=>`「${name}」は除外リストに入っているため、プロジェクトに自動整理されません。このフォルダーで通常の会話を始めることはできます。`, excludedProceed:'それでもこのフォルダーで開始', unknownDirectory:'不明なディレクトリ' },
