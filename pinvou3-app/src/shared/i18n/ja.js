...
dictJa.uiToolDetails.actions = { connectedTmeet:'Tencent Meeting に接続しました', enabled:'接続しました。ツールスイッチはデフォルトでオフです。入力欄のツールリストでオンにしてください', disconnectedTmeet:'Tencent Meeting から切断しました', connectingIma:'Tencent ima に接続中', validatingIma:'OpenAPI 認証情報を確認しています…', connectedIma:'Tencent ima に接続しました', imaEnabled:'IMA に接続しました。ツールスイッチはデフォルトでオフです。入力欄のツールリストでオンにしてください。', imaFailed:'IMA の接続に失敗しました', disconnectedIma:'Tencent ima から切断しました', operationFailed:'操作に失敗しました。再試行してください' };
...
failure: 'PPT 生成機能の準備に失敗しました。しばらくしてからもう一度お試しください。',
  ...
  missingCapabilities: (ids) => `不足している機能：${ids}`, switchedOffPacks: (ids) => `次のツールは現在オフになっています。入力欄のツールリストでオンにしてから再試行してください：${ids}`, notAppliedPacks: (ids) => `次のパックはまだ有効になっていません（インストール中の可能性があります）。しばらくしてから再試行してください：${ids}`,
...
desktopHint:'先にデスクトップで Obsidian をインストールして保管庫を作成し、ここで再検出してください。',
  ...
  connectedBanner:name=>`${name} に接続済み · ツールスイッチはデフォルトでオフです。入力欄のツールリストでオンにできます`,
  connectedBannerIma:'Tencent ima に接続済み · ツールスイッチはデフォルトでオフです。入力欄のツールリストでオンにできます',
