...
const jaBase = {
  ...browserJa,
        ...
        uiWorkspacePicker: { title:'ワークスペースを選択', searchPlaceholder:'プロジェクトやパスを検索', empty:'プロジェクトはまだありません。フォルダーを参照するか、一時チャットを開始できます', noMatch:'一致するプロジェクトがありません', multiRootSummary:(n, primary)=>`${n} 個のフォルダー · 既定: ${primary}`, primaryRootBadge:'既定', noticeRestricted:n=>`この会話は ${n} 個のフォルダーにアクセスできます`, noticeVisibility:n=>`この ${n} 個のフォルダーがこの会話に関連していることがモデルに伝わります`, temporary:'一時チャット', temporaryDesc:'ディレクトリに紐付けません', browse:'他のフォルダーを参照…', browseDesc:'選択したフォルダーを起点に新しい会話を開始', excludedTitle:'このフォルダーの自動プロジェクト化はオフです', excludedBody:name=>`「${name}」は除外リストに入っているため、プロジェクトに自動整理されません。このフォルダーで通常の会話を始めることはできます。`, excludedProceed:'それでもこのフォルダーで開始', unknownDirectory:'不明なディレクトリ' },
