...
const codexJa = {
  ...
  activeHint:'ツール呼び出し、思考、計画、権限リクエストは ACP 本来の意味で表示され、鲜小助 の作業セッションメッセージとは分離されます。',
  nativeActiveHint:'ツール呼び出し、思考、選択確認は 鲜小助 Engine の意味で表示され、作業セッションと同じエンジン経路を共有します。',
  draftHint:'入力すると一時セッションを作成できます。入力欄の左下からプロジェクトディレクトリを選ぶこともできます。',
  nativeDraftHint:'入力すると一時セッションを作成できます。入力欄の左下からプロジェクトディレクトリを選ぶと、鲜小助 がその中で直接作業します。',
  ...
  temporarySessionDesc:'鲜小助 が管理する隔離ディレクトリを使用', recentProjects:'最近のプロジェクト',
  ...
  bridgeRepair:'鲜小助 を修復または再インストールしてください。開発環境では npm run prepare:codex-bridge を実行できます。',
  ...
  finishAgentAuth:agent=>`ブラウザーで ${agent} の認証を完了してください。完了後 鲜小助 が自動接続します`,
  ...
  finishBrowserAuth:'ブラウザーで ChatGPT の認証を完了してください。完了後 鲜小助 が自動接続します',
...
const acpProvidersJa = {
  ...
  thirdPartyWarning:'鲜小助 はお客様の API Key をアップロードしません。',
...
const jaBase = {
  ...browserJa,
        ...
        kbBuildDups: '重複をスキャン', kbDupHint: '重複データがありません。「重複をスキャン」で必要なファイルだけを読み込み、フィンガープリントを算出します。',
        ...
        kbOutSub: '鲜小助 と一緒に作ったものをここにまとめます。',
        ...
        kbOutCount: (n) => `${n} 件`, kbOutSort: '最近の更新',
        ...
        kbOutEmpty: '成果物はまだありません', kbOutEmptyHint: '鲜小助 とチャットして作ったものがここに表示されます。',
        ...
        kbModelPkgName: '鲜小助 Embedding モデル', kbModelPkgSub: '多言語ベクトルモデル · int8 量子化 · CPU のみでローカル動作',
        ...
        vllm: '推論エンジン', ctx: 'コンテキスト長', ctxWarn: '⚠️ コンテキスト長が小さすぎます（現在 %s、128k 未満）。長い会話は頻繁に圧縮され履歴が減ります。128k 以上のモデルを使うか --max-model-len を上げてください', queue: 'タスク待ち', kv: 'KV ヒット率', ttft: 'TTFT', tps: 'TPS（生成速度）', tokTotal: '累計トークン・生成/入力', clearHold: '長押しで消去', clearHolding: '押し続けて…', clearDone: '消去完了', clearHint: '「消去」をゲージがいっぱいになるまで長押しするとリセットされます · 離すとキャンセル', statsSince: '%t から集計', statsAge: ' · %m 分経過', statsJustReset: ' · 今リセット', statsLifetime: 'エンジン起動からの累計', app: 'アプリサービス', appRunning: 'コアデーモン稼働中',
        ...
        communityQrAlt: '鲜小助 QQ グループの QR コード', communityQrPlaceholder: 'QR コード準備中', communityChannelTag: '中国語 QQ コミュニティ',
        ...
        chatGreeting: 'こんにちは、今日は何をしましょうか？', artifacts: '成果物とコード', placeholder: '鲜小助 に質問またはコマンドを入力', disclaimer: '鲜小助 は不正確な情報を生成する可能性があります。必ずご確認ください。', ctxUsage: 'コンテキスト',
        ...
        cpEditCard: 'カードを編集',
        cpFieldName: '名前', cpFieldDesc: '一行説明', cpFieldDescPh: 'カードに表示されます。15 文字以内',
        ...
        // —— チャット/グローバル chrome ——
        appTitle: BRAND_NAME, winMin: '最小化', winMax: '最大化', winClose: '閉じる',
        ...
        voiceEditMode: '現在の入力を編集', voiceStructuredPostprocessing: 'リストに整理中…',
        voiceEditPostprocessing: '現在の入力を編集しています…', voiceEditPreviewReady: '音声編集を確認してください', voiceEditNoChange: '音声編集で変更はありません',
        ...
        // 鲜小助レビューカード。品/悟 の字はブランド表記として保持、周囲の文言は言語に追従。
        pvLoadingPin: '鲜小助 品 · レビュー', pvLoadingWu: '鲜小助 悟 · 発散補完',
...
dictJa.uiChatView = {
  ...
  placeholderDesignPpt:'生成したい PPT のテーマや修正内容を記述してください',
...
dictJa.uiSettingsView = {
  ...
  memorySummary: total => `鲜小助 は好み、業務コンテキスト、最近の事項を記憶し、後続の会話でコンテキストを引き継ぎやすくします。有効なメモリ ${total} 件を記録済み。`,
  memoryOffDesc: '有効にすると、鲜小助 は呼び名、安定した好み、業務コンテキスト、最近の事項を記憶し、繰り返しの説明を減らせます。',
  ...
  memoryOffNotice: 'メモリを有効にすると、鲜小助 は会話でこれらの情報を使用し、新しい長期メモリと最近のメモリを自動で整理します。',
  ...
  memoryRecentHint: '最近のメモリは進行中または完了したばかりの事項を記録し、鲜小助 がコンテキストを引き継ぐのに役立ちます。時間とともに自動で薄れますが、手動でアーカイブ・削除もできます。',
...
visualizer: { title:'データ分析の可視化', subtitle:'Chart.js ダッシュボード / グラフ分析 / HTML 可視化', desc:'構造化データ・表の集計・ビジネス指標を、鲜小助 のホスト体験に合った HTML 可視化ダッシュボードに変換します。デフォルトで Chart.js、アクセシブルな canvas、カスタム凡例、フラット配色を使用し、.html 成果物カードとして提供します。', latency:'ローカル' },
'package-author': { title:'プラグインパッケージ標準化', subtitle:'スキル/MCP/関数をアップロード可能な標準プラグインに整理', desc:'散在するスキル（SKILL.md）、MCP サービス、またはそれらの組み合わせを、鲜小助 ストアがインポートできる標準プラグインに整理します：plugin.json、mcp/manifest.json、SKILL.md の frontmatter を補い、アイコンを生成し、命名とレイアウトを検証して、ディレクトリまたは zip を出力します。', latency:'ローカル' },
