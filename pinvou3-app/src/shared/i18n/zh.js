...
export const dictZh = {
  ...browserZh,
        ...
        uiProjects: { convertToProject:'转为项目', projectNamePlaceholder:'项目名称', renameProject:'重命名项目', deleteProject:'删除项目', deleteConfirmLabel:'确认删除项目？', deleteProjectHint:'对话不会被删除，将回到未分组', moveToProject:'移动到项目…', moveOut:'移出项目', moveToUngrouped:'移到未分组', temporaryBadge:'临时工作区', folderUnavailable:'文件夹不可用', rebindFolder:'重新绑定', addFolderTitle:'将文件夹添加到项目？', addFolderBody:folder=>`项目将包含该会话的当前工作目录（${folder}），之后在其中新建的会话会自动进入该项目。`, searchPlaceholder:'搜索项目', noProjects:'暂无项目', ungrouped:'未分组', currentProject:'当前项目', movedNotice:'已移动对话', movedNoticeWithFolder:folder=>`已移动对话，并加入文件夹 ${folder}`, addFolderConfirm:'添加并移动', moveOnly:'仅移动', noMatchProject:'没有匹配的项目', opFailed:'项目操作失败', rebindTitle:'重新绑定项目文件夹', rebindConfirm:'移动并重绑定', rebindSessionsHint:n=>`${n} 个会话将改用新目录`, rebindSuccess:n=>`已重新绑定 ${n} 个会话`, rebindOldExistsWarn:'原文件夹仍存在，切换后两处将指向不同目录，确认继续？' },
