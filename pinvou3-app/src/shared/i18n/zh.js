...
const codexZh = {
  ...
  branchSwitchCarry:'携带更改切换', branchSwitchCarryHint:'更改随工作区携带到新分支；与目标分支冲突时切换失败，更改保持不变',
  branchSwitchStash:'Stash 后切换', branchSwitchStashHint:'先暂存更改（git stash -u），切换后自动恢复；恢复冲突时 stash 会保留',
  branchSwitchCommit:'提交后切换', branchSwitchCommitHint:'将全部更改（含未跟踪文件）提交到当前分支后再切换',
  branchSwitchCommitPlaceholder:'提交信息（必填）',
  branchSwitchCommitPrompt:(branch)=>`将全部未提交更改提交到当前分支，然后切换到「${branch}」：`,
  branchSwitchBusyError:'Agent 正在运行，请等待当前任务完成后再切换分支',
  ...
  rewindChip:n=>n>0?`回退到第 ${n} 轮`:'回退到第一轮之前',
  rewindChipConversationOnly:n=>n>0?`仅回退对话到第 ${n} 轮`:'仅清空对话',
  rewindDialogTitle:'确认回退',
  rewindChangesToUndo:'将撤销的代码变更', rewindConversationLabel:'对话',
  rewindConversationTarget:n=>n>0?`对话将截断到第 ${n} 轮末尾，之后的对话会被移除`:'全部对话将被清空',
  rewindConversationOnlyNote:'该轮没有可用的代码快照，本次仅回退对话，代码保持不变。',
  rewindPreRestoreNote:'当前代码状态会先自动保存为回滚点，反悔时可再次回退恢复。',
  rewindNoChanges:'该节点之后没有文件变更',
  rewindAdded:count=>`新增 ${count}`, rewindModified:count=>`修改 ${count}`, rewindDeleted:count=>`删除 ${count}`, rewindOther:count=>`其他 ${count}`,
  rewindStatus:{added:'新增',modified:'修改',deleted:'删除',renamed:'重命名',copied:'复制',gitlink:'嵌套仓库(不回退)',other:'其他'},
  ...
  compactStart:'正在压缩上下文', compactDone:'上下文压缩完成', compactFail:'上下文压缩失败', compactCancel:'已取消上下文压缩',
  nativeToolGateDecision:(tool,decision,reason,risk,agent)=>`权限闸门${decision === 'allowed' ? '允许' : decision === 'denied' ? '拒绝' : '无法审查并拒绝'}了 ${tool || '工具'}${agent ? `（子智能体 ${agent}）` : ''}${risk ? ` · 风险 ${risk}` : ''}${reason ? ` · ${reason}` : ''}`,
...
const acpProvidersZh = {
  ...
  export:'导出', import:'导入',
  exportWarningTitle:'导出文件包含明文密钥', exportWarningDesc:'导出文件包含 API Key 明文，请妥善保管，不要分享给他人。点击文本框全选后手动复制。', selectAll:'全选',
...
export const dictZh = {
 ...
 sudo: '高级执行权限 (Sudo)', 
        ...
        uiComputerUse: {
          ...
          platformUnsupportedHint:'当前平台没有电脑使用后端，无法开启此功能。',
          settingsStoppedHint:'电脑使用已停止。关闭后再重新开启，即可恢复助手控制。',
          bannerTitle:'助手正在控制你的电脑',
          bannerNote:'助手可以查看屏幕并操作鼠标和键盘，你可以随时停止。',
          bannerStop:'停止控制',
          grantTitle:'助手请求控制你的电脑',
          grantDesc:'允许后，助手可以在本次会话中查看你的屏幕并操作鼠标和键盘。请确认你信任当前任务；高危操作仍会逐一征求确认。',
          grantAllow:'允许本次会话控制',
          grantDeny:'拒绝',
          confirmTitle:'确认本次操作',
          ...
          confirmKeyChordMasked:(chord, count)=>chord?`按下组合键 ${chord} + ${count} 个掩码字符`:`按下 ${count} 个掩码字符`,
        ...
        uiMultiAgent: {
          ...
          runningAgentsCount:count=>`${count} 个运行中`,
          runningAgentsCollapse:'收起运行中的智能体', runningAgentsExpand:'展开运行中的智能体',
          ...
          webActionHint:'蜂群会话在 Web 端只读：此操作请在桌面端完成。',
        ...
        uiProjects: { convertToProject:'转为项目', projectNamePlaceholder:'项目名称', renameProject:'重命名项目', deleteProject:'删除项目', deleteConfirmLabel:'确认删除项目？', deleteProjectHint:'对话不会被删除，将回到未分组', moveToProject:'移动到项目…', moveToUngrouped:'移到未分组', alreadyUngrouped:'已在未分组', folderUnavailable:'文件夹不可用', rebindFolder:'重新绑定', moveConfirmTitle:'移动对话到此项目？', moveConfirmBody:(project, folder)=>`对话将移入「${project}」。其所在文件夹 ${folder} 不会加入项目，仅该对话归档。`, moveConfirm:'移动', searchPlaceholder:'搜索项目', noProjects:'暂无项目', currentProject:'当前项目', movedNotice:'已移动对话', movedNoticeWithFolder:folder=>`已移动对话；文件夹 ${folder} 已加入项目（本对话的可访问范围不变）`, noMatchProject:'没有匹配的项目', opFailed:'项目操作失败' , manageFolders:'管理文件夹', newSessionHere:'新建会话', rebindTitle:'重新绑定项目文件夹', rebindConfirm:'移动并重绑定', rebindSessionsHint:()=>'该文件夹下的所有会话都会改用新目录', rebindSuccess:n=>`已重新绑定 ${n} 个会话`, rebindUpToDate:'没有需要重绑定的会话', rebindOldExistsWarn:'原文件夹仍存在，切换后两处将指向不同目录，确认继续？', rebindPartial:(ok, failed)=>`已重新绑定 ${ok} 个会话，${failed} 个失败，可对剩余会话重试`, rebindBusyAfter:n=>`${n} 个会话在重绑定期间开始了新回合，如目录表现异常请让其空闲后重试一次` , rebindRetryRemaining:'重试剩余会话', rebindFailedSessions:'失败会话', rebindInProgress:'另一个目录重绑定正在进行中' , rebindBusyHint:'这些会话仍在回合中，等待其空闲后重试', rebindToRoot:'目标不能是文件系统根目录，请改选普通文件夹', rebindToNested:'新目录不能位于旧目录内部', rebindToUnusable:'目标文件夹不可用，请重新选择', rebindRootsConflict:'新目录会使项目自身的文件夹互相嵌套，请选择项目现有文件夹之外的目录', rebindRootsExpand:'展开全部失效文件夹', rebindRootsCollapse:'只保留第一个失效文件夹' },
        uiKeychain: { accessibleFolders:n=>`可访问的 ${n} 个文件夹`, accessibleFoldersRecorded:n=>`已记录 ${n} 个文件夹（当前仅主目录生效）`, primaryBadge:'主', unknownDirectory:'未知目录', alignAction:'对齐到项目', alignActionDesc:'把本对话的可访问文件夹更新为项目当前的全部文件夹（下个回合生效）', alignActionDescRecorded:'更新本对话记录的文件夹集合（多根暂不送达第三方智能体，仅主目录生效）', alignDone:'已对齐到项目', alignNoChange:'已是最新，无需对齐', alignNoProject:'该会话不在含文件夹的项目中，无可对齐内容', alignBusy:'当前回合结束后重试', alignWriteSkipped:'磁盘保持原样（写入未落盘）；活动会话未被打断，可重试对齐', alignPushDeferred:'已对齐到项目；当前运行中的引擎仍用旧文件夹集合，将在引擎重启后生效', alignFailed:'对齐失败' },
        uiManageFolders: { title:'管理文件夹', renameHint:'点击重命名项目', emptyRoots:'此项目暂无文件夹（纯标签项目）', primaryBadge:'主', setPrimaryHint:'设为主文件夹', removeAction:'移除', removeConfirm:'移除', removeConfirmBody:'在飞对话将继续访问该文件夹直到结束，且该文件夹会保留在其钥匙串中、跨重启生效，直到会话被删除；未来的新对话不再获得。显式归入的会话保持不变；自动归组的会话移到未分组。', removePrimaryBlocked:'这是主文件夹：请先把其它文件夹设为主，再移除它。', addAction:'添加文件夹…', addDuplicate:'该文件夹已在项目中', addNested:'该文件夹位于项目已有文件夹内部（或包含它），同一项目内不允许嵌套', addNoticeRestricted:'添加后，此项目的新对话将可访问该文件夹', addNoticeVisibility:'添加后，模型将知道该文件夹与项目新对话相关', addNoticeRestrictedRecorded:'添加后，此项目的新对话将记录该文件夹（多根暂不送达第三方智能体，仅主目录生效）', addNoticeVisibilityRecorded:'添加后，该文件夹将随项目新对话被记录（当前仅主目录对智能体生效）', exclusionSection:'不再自动建项目的文件夹', neverMaterializeAction:'不再自动建项目', neverMaterializeHint:'把此文件夹加入排除列表：之后不再为它自动物化项目（本项目与现有会话不受影响）', revokeExclusion:'撤销', exclusionDone:'已更新排除列表' },
        uiWorkspacePicker: { title:'选择工作区', searchPlaceholder:'搜索项目或路径', empty:'暂无项目，可浏览文件夹或开始临时会话', noMatch:'没有匹配的项目', multiRootSummary:(n, primary)=>`${n} 个文件夹 · 主: ${primary}`, primaryRootBadge:'默认', noticeRestricted:n=>`将可访问这 ${n} 个文件夹`, noticeVisibility:n=>`模型将知道这 ${n} 个文件夹与本对话相关`, noticeRestrictedRecorded:n=>`将记录这 ${n} 个文件夹（当前仅主目录生效）`, noticeVisibilityRecorded:n=>`这 ${n} 个文件夹将随会话记录（当前仅主目录对智能体生效）`, temporary:'临时会话', temporaryDesc:'不绑定目录', browse:'浏览其他文件夹…', browseDesc:'以所选文件夹开始新对话', excludedTitle:'此文件夹已设为不自动建项目', excludedBody:name=>`「${name}」在排除列表中，不再自动物化为项目；更宽的项目仍可能按目录把其下的会话自动归组。仍可以它为目录开始纯文件夹对话。`, excludedProceed:'仍以此文件夹开始', unknownDirectory:'未知目录' },
 ...
 cpDraftSaved: '已存入', cpDraftSavedTitle: '已有同名自制卡',
  ...
  voiceInputFailed: '语音输入失败',
        ...
        voiceIntroTitle: '语音输入可以这样用',
        voiceIntroClose: '关闭语音快捷键说明',
        voiceIntroShortcutLabel: '快捷键',
        voiceIntroComboLabel: '有文本时',
        voiceIntroModeLabel: '模式',
        voiceIntroDictationMode: '普通语音输入',
        voiceIntroDictationSteps: ['按 Alt 开始录音', '再按 Alt 结束录音', '文字填入输入框，可编辑后发送'],
        voiceIntroTaskMode: '语音编辑',
        voiceIntroTaskSteps: ['输入框已有文本时按 Alt 开始录音', '再按 Alt 结束录音', '根据语音要求编辑当前文本，确认后可发送（需开启智能整理；关闭时该手势保持听写）'],
        voiceIntroTaskExampleLabel: '示例编辑',
        voiceIntroTaskExample: '“把这段改成三条要点，语气更正式。”',
        voiceIntroGotIt: '暂不开启，继续语音',
        voiceIntroDone: '知道了',
...
Object.assign(dictZh.uiToolDetails.tools, {
  ...
  qcc:{ title:'企查查', subtitle:'企业工商数据授权查询', latency:'云端', desc:'接入企查查智能体数据平台 qcc-company 远程 MCP。点「连接」默认通过企查查账号 OAuth 授权；如有企查查 API Key 也可选填，密钥只写入本机系统凭据。', configTitle:'企查查 API Key（可选）', configDescription:'API Key 只保存在本机系统凭据，不写入 mcp.json。留空则直接使用 OAuth 授权。', configFields:[{key:'QCC_API_KEY', label:'企查查 API Key', helpText:'可选。默认使用 OAuth 授权；如需以 API Key 方式接入再填写。', placeholder:'粘贴企查查 API Key（可留空）'}], welcomeQueries:['查一下华为的工商信息','腾讯的工商登记信息','比亚迪有哪些对外投资','阿里巴巴的股东结构'] },
...
dictZh.uiChatView = {
  ...
  placeholderScenePpt:'描述你要生成的 PPT 主题或修改要求',
...
dictZh.uiSettingsView = {
  ...
  memoryOrganize: 'AI 整理记忆',
  memoryOrganizeDesc: '合并重复、清理过时条目，让长期记忆保持简洁准确',
...
dictZh.uiArtifacts = {
  ...
  diLineHeight:'行高', diLetterSpacing:'字距', diBgSize:'背景尺寸', diBgSizePlaceholder:'cover / contain', diBgPosition:'背景位置',
...
desktopHint:'请先在桌面端安装 Obsidian 并创建笔记库，然后在这里重新检测。',
  ...
  importingSkill:'正在导入插件包…', validatingSkillPackage:'校验并解压中', importFailedWith:e=>`导入失败：${e}`,
