...
export const dictZh = {
 ...
 sudo: '高级执行权限 (Sudo)', 
        ...
        uiKeychain: { accessibleFolders:n=>`可访问的 ${n} 个文件夹`, primaryBadge:'主', unknownDirectory:'未知目录', alignAction:'对齐到项目', alignActionDesc:'把本对话的可访问文件夹更新为项目当前的全部文件夹（下个回合生效）', alignDone:'已对齐到项目', alignNoChange:'已是最新，无需对齐', alignNoProject:'该会话不在含文件夹的项目中，无可对齐内容', alignBusy:'当前回合结束后重试', alignWriteSkipped:'磁盘已是目标状态；未打断活动会话', alignFailed:'对齐失败' },
        ...
        uiWorkspacePicker: { title:'选择工作区', searchPlaceholder:'搜索项目或路径', empty:'暂无项目，可浏览文件夹或开始临时会话', noMatch:'没有匹配的项目', multiRootSummary:(n, primary)=>`${n} 个文件夹 · 主: ${primary}`, primaryRootBadge:'默认', noticeRestricted:n=>`将可访问这 ${n} 个文件夹`, noticeVisibility:n=>`模型将知道这 ${n} 个文件夹与本对话相关`, temporary:'临时会话', temporaryDesc:'不绑定目录', browse:'浏览其他文件夹…', browseDesc:'以所选文件夹开始新对话', excludedTitle:'此文件夹已设为不自动建项目', excludedBody:name=>`「${name}」在排除列表中，不会自动归入项目。仍可以它为目录开始纯文件夹对话。`, excludedProceed:'仍以此文件夹开始', unknownDirectory:'未知目录' },
