...
Object.assign(dictZh.uiAttachments, {
  ...
  localEngineInstallPrompt:'本地识图引擎尚未安装，是否立即安装？也可以到 设置→模型→本地识图 安装或卸载。',
  localEngineInstallConfirm:'安装并继续', localEngineInstallCancel:'暂不安装',
  localEngineNotRunningPrompt:'本地识图引擎未运行，且自动启动已设为「从不」。',
  localEngineGoSettings:'去设置页启动', localEngineSendFallback:'本次回落云端视觉模型发送',
...
dictZh.uiSettingsDetail.llamaEngine = {
  title:'本地多模态引擎',
  introWhatLabel:'是什么', introWhat:'在本机运行开源视觉模型（Qwen3-VL），图片分析完全在本地完成，不上传云端。',
  introWhenLabel:'何时需要', introWhen:'当前对话模型不支持图片输入，或不希望把图片发送给云端服务商时。',
  introHowLabel:'如何启用', introHow:'在下方下载引擎与模型并启动服务；保持「自动作为图片识别兜底」开启，收到图片时会自动调用本地引擎分析。',
  introPerfLabel:'性能', introPerf:'识图速度取决于电脑性能，建议配备高性能 GPU 或 CPU 的电脑使用；首次加载模型需要较长等待。',
  setupSection:'安装与模型', behaviorSection:'行为', engineReady:'引擎已安装', engineMissing:'引擎未安装',
  modelReady:'模型已就绪', modelMissing:'模型未下载', running:'运行中', starting:'启动中…', stopped:'已停止',
  runningWithPort:port=>`运行中（127.0.0.1:${port}）`,
  downloadingEngine:'正在下载引擎', downloadingModel:'正在下载模型', cancelDownload:'取消',
  engineLabel:'引擎', modelLabel:'视觉模型', deviceLabel:'设备', serviceLabel:'服务',
  installEngine:'下载引擎（约 40MB）', installModel:'下载', gpu:'GPU（Vulkan / Metal）', cpu:'CPU',
  deviceAuto:'自动', deviceAutoGpu:'已检测到可用 GPU', deviceAutoCpu:'未检测到可用 GPU，将使用 CPU',
  recommended:'推荐', default:'默认', start:'启动引擎', stop:'停止引擎', deleteModel:'删除', deleteEngine:'卸载',
  deleteRequiresStop:'引擎运行中，请先停止引擎再删除',
  deleteEngineConfirm:tag=>`将卸载本地引擎${tag ? `（v${tag}）` : ''}，模型文件保留，重装引擎后即可使用。确定卸载？`,
  restartApply:'重启并应用', restartNeeded:'模型切换将在引擎重启后生效',
  deleteModelConfirm:name=>`将删除「${name}」的本地模型文件，需要时可重新下载。确定删除？`,
  viewLogs:'查看日志', privacyNote:'仅监听本机 127.0.0.1，模型与图片数据不离开电脑。',
  newSessionHint:'引擎启动后，新开的对话才能使用本地视觉能力。',
  visionFallbackLabel:'自动作为图片识别兜底',
  visionFallbackDesc:'开启后，未配置视觉模型的纯文本模型收到图片时，自动用本地引擎描述；显式配置了视觉模型或「本地识图引擎」的模型按各自配置处理，不被接管。',
  localEngineOption:'本地识图引擎', autoStartLabel:'自动启动引擎', autoStartFirstImage:'第一次发送图片时',
  autoStartLaunch:'开启 pinvou 时', autoStartNever:'从不自动启动',
  autoStartFirstImageDesc:'平时零占用；第一次发图时启动，首次识图需等待模型加载',
  autoStartLaunchDesc:'启动应用时后台预启动引擎，识图即用；引擎常驻内存',
  autoStartNeverDesc:'不自动启动，可在本页手动启动，发图时也可临时选择',
