...
const ToolOutput = ({ item, t }) => {
    ...
    const ToolCard = ({ item, t, variant = 'legacy' }) => {
        ...
        <div className={cardBoxCls('border-[#0B57D0]/20 dark:border-[#A8C7FA]/30')}>
          ...
          {active ? (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button className={cardBtnCls('primary')} onClick={() => bridge.interaction.acceptPlan(item.id, item.planMarkdown, undefined, item.planId)}>{t.planGo}</button>
              <button className={cardBtnCls()} onClick={() => onPrefill && onPrefill(t.planRevisePrefill)}>{t.planEdit}</button>
              <button className={cardBtnCls()} onClick={() => bridge.interaction.discardPlan(item.id, item.planId)}>{t.planDrop}</button>
            </div>
          ) : (
      ...
      return (
        <div className={cardBoxCls('border-[#E37400]/20 dark:border-[#FDD663]/30')}>
          ...
          {done ? (
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <button className={cardBtnCls()} onClick={() => bridge.interaction.planStuckReplan(item.id)}>{t.stuckReplan}</button>
              <button className={cardBtnCls('primary')} onClick={() => bridge.interaction.planStuckGo(item.id)}>⚡ {t.stuckGo}</button>
            </div>
          )}
        </div>
      );
