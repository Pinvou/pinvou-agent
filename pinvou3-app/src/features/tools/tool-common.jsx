...
import { BookOpen, Building2, ChevronDown, ChevronLeft, ChevronRight, CloudSun, Code, Cpu, FileText, FolderOpen, Globe, Hexagon, IconGrid, IconList, Layout, LineChart, Mail, MessageCircle, Navigation, Package, Palette, Presentation, Search, Send, Server, TrendingDown, TrendingUp, User, Video, Wrench, XIcon, Zap } from '../../components/icons.jsx';
...
const AcFmtIcon = FileTypeIcon;
    ...
    const tsToolsData = [
      ...
      { id: 23, backendId: 'file-master', title: '文件管理大师', subtitle: '本机找文件 / 磁盘占用分析 / 回收站安全删除与还原', category: 'office', type: 'MCP Server', version: 'v1.7.0', latency: '本地', desc: '说“帮我找一下那个文件 / C 盘满了”：找文件按文件名/目录名秒搜常用目录并兜底主目录（AppData 下应用目录也能命中）；磁盘清理只读扫描 C 盘常见积聚地并逐层下钻，按 🟢🟡🔴 三级风险说清“这是什么数据”，你点名的文件后台移入系统回收站（内置系统目录白名单，绝不物理删除）并自动记录删除日志，删错了可按日志一键还原，还能一键清空回收站释放空间。纯本地零依赖、数据不出机。安装后同名「文件管理大师」技能卡一并生效。⚠️ 危险操作提示：清空回收站与物理清除不可恢复；清理前请逐项核对。本工具通过白名单与双重确认保障安全，但不对误删或清理造成的损失承担责任。', icon: FolderOpen, color: 'bg-gradient-to-b from-slate-500 to-slate-700', installed: false, authRequired: false, welcomeQueries: ['帮我找一下上周的报表文件', 'C盘满了，看看什么占空间', '清理一下浏览器缓存'] },
    ];
    ...
    const tsSkillsData = [
      ...
      { id: 's8', backendId: 'file-master', title: '文件管理大师', subtitle: '本机找文件 / C 盘占用三级风险分析 / 回收站安全删除与还原', category: 'skill', type: 'Skill', version: '—', latency: '本地', desc: '一个技能管好本机文件：找文件按文件名/目录名秒搜常用目录并兜底主目录（AppData 下应用目录也能命中），配合搜索阶梯与中文 Windows 路径先验提高一次命中率；磁盘清理只读扫描系统盘与非系统盘并逐层下钻，按 🟢🟡🔴 三级风险说清“这是什么数据”，你点名的项后台移入系统回收站（可恢复，绝不物理删除），删错了可按日志一键还原，还能一键清空回收站释放空间。随工具商店的「文件管理大师」工具挂载生效。', icon: FolderOpen, color: 'bg-gradient-to-b from-slate-500 to-slate-700', installed: false, authRequired: false },
    ];
