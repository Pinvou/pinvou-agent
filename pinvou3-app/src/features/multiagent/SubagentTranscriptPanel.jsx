...
import { ArrowLeft, X } from '../../components/icons.jsx';
 ...
 *
 * 两级结构（CodexWorkspacePanel 同款）：列表（本会话全部子智能体 + 状态点/
 * 受阻标注）→ 详情（该实例的完整思考、工具与结果，共享对话时间线渲染）。
...
export function SubagentTranscriptPanel({ sessionId, initialAgentId, t, theme, onClose }) {
    ...
    >
      ...
      <div className="h-14 shrink-0 px-3 flex items-center gap-2 border-b border-black/[0.05] dark:border-white/[0.06]">
        {selectedAgentId ? (
          ...
          <div className="min-w-0 flex-1 text-[13px] font-semibold">{copy.agentsListTitle}</div>
        )}
        <button
          type="button"
          onClick={onClose}
          ...
          {Array.isArray(agents) && agents.map((entry) => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center gap-2.5 rounded-[12px] px-2.5 py-2 text-left hover:bg-black/[0.04] dark:hover:bg-white/[0.06]"
              >
                <AppIcon
