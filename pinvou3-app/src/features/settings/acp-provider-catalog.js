...
// base_url 为准。
//
// 模型名单按 2026-09-11 各厂商官方接入文档核对（platform.claude.com /
...
// ProviderFormModal：选择预设不自动填 model）。名单只增不删：中转站可能
// 仍代理旧型号，且 tests/acp_providers_ui_smoke.js 依赖既有条目。
...
// 截至 2026-09-11 各厂商官方文档在列（新档位加在各自厂商分组最前）：
export const ACP_MODEL_PRESETS = [
  // Anthropic（platform.claude.com，fable-5-1 为当前最高旗舰）
  'claude-fable-5-1', 'claude-fable-5', 'claude-opus-5', 'claude-sonnet-5', 'claude-haiku-4-5',
  // OpenAI（developers.openai.com，gpt-6-astra 为当前旗舰；gpt-5.3-codex 为
  // 当前 Codex 编码模型，Responses 专用）
  'gpt-6-astra', 'gpt-5.3-codex', 'gpt-5.6-sol', 'gpt-5.6-terra', 'gpt-5.6-luna', 'gpt-5.5', 'gpt-5.4', 'gpt-5.2',
  // DeepSeek（deepseek-flash = V4.1-Flash 为当前主力；v4-pro 自 2026-09-14 起
  // 被官方路由到 V4.1-Flash 计费；deepseek-chat/reasoner 旧别名已停用）
  'deepseek-flash', 'deepseek-v4-flash', 'deepseek-v4-pro',
  // Moonshot 开放平台（platform.kimi.com；kimi-k2.5 与 moonshot-v1 系列已于
  // 2026-08-31 全面下线；kimi-k3 为旗舰推荐，kimi-k2.7-code 面向代码场景）
  'kimi-k3', 'kimi-k2.7-code', 'kimi-k2.6', 'kimi-k2.7-code-highspeed',
  // 智谱 GLM（docs.bigmodel.cn，glm-5.3 为当前旗舰）
  'glm-5.3', 'glm-5.3-flash', 'glm-5.2', 'glm-5.1', 'glm-5', 'glm-4.7', 'glm-4.6', 'glm-4.7-flash',
  // Qwen（阿里云百炼，qwen3.8 系为当前旗舰/快速主力）
  ...
  // xAI（grok-4.6 为官方编码/Agent 推荐旗舰，grok-4.5 上代）
  'grok-4.6', 'grok-4.5', 'grok-4.3', 'grok-4.20-reasoning', 'grok-build-0.1',
];
