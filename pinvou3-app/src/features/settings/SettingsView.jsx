...
const SCard = React.forwardRef(({ title, titleAdornment, children, id, style }, ref) => (
    ...
    const ModelChip = ({ t, bs, onGotoSettings }) => {
      const [open, setOpen] = useState(false);
      ...
      return (
        <div className="relative px-2 mb-2">
          <button onClick={() => { if (!busy && canSwitchModels) setOpen(o => !o); }} disabled={busy || !canSwitchModels}
            ...
            className={`inline-flex items-center gap-1.5 pl-3 pr-2 py-1 rounded-full text-[12px] font-medium transition-colors disabled:opacity-50 bg-[#EAEDF1] text-[#1F1F1F] hover:bg-[#E0E3E7] dark:bg-[#2A2B2D] dark:text-[#E3E3E3] dark:hover:bg-[#333537]`}>
          ...
          {open && canSwitchModels && (
            <div>
              ...
              <div className={`absolute bottom-full left-2 mb-1 z-50 min-w-[240px] max-h-[340px] overflow-y-auto rounded-xl border shadow-lg py-1 bg-white border-[#E0E3E7] dark:bg-[#1E1F20] dark:border-[#333537]`}>
                {savedModels.map(m => (
                  <button key={m.id} onClick={() => pick(m.id)}
                    className={`w-full flex items-center gap-2 px-3 py-2 text-left transition-colors hover:bg-[#F0F4F9] dark:hover:bg-[#2A2B2D]`}>
                    <span className={`shrink-0 w-1.5 h-1.5 rounded-full ${m.id === effectiveId ? 'bg-[#34A853]' : 'bg-transparent'}`}></span>
                    <span className="flex-1 min-w-0">
                      <span className={`block text-[13px] truncate text-[#1F1F1F] dark:text-[#E3E3E3]`}>{selectorMainLabel(m, t)}</span>
                      <span className={`block text-[11px] truncate text-[#5F6368] dark:text-[#9AA0A6]`}>{selectorSubLabel(m, t)}</span>
                    </span>
                    {m.id === activeModelId && <span className={`shrink-0 text-[10px] px-1.5 py-0.5 rounded bg-[#E8EAED] text-[#5F6368] dark:bg-[#37393B] dark:text-[#9AA0A6]`}>{t.modelActiveTag}</span>}
                  </button>
                ))}
                {canManageModels && (
                  <div className={`border-t mt-1 pt-1 border-[#E8EAED] dark:border-[#333537]`}>
                    <button onClick={() => { setOpen(false); if (onGotoSettings) onGotoSettings(); }}
                      className={`w-full px-3 py-1.5 text-left text-[12px] text-[#5F6368] hover:bg-[#F0F4F9] dark:text-[#9AA0A6] dark:hover:bg-[#2A2B2D]`}>
                      {t.manageModels}
                    </button>
                  </div>
                )}
