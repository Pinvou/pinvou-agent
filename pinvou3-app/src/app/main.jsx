...
function hasPendingCollaborationTasks(bs) {
  ...
  return incoming.length > 0 || local.some(task => task && task.status !== 'completed');
    ...
    const App = () => {
      ...
      const bs = useBridgeState(['platform', 'sessions', 'chat', 'voice', 'knowledge', 'scheduled', 'monitor', 'settings', 'models', 'vllm', 'interaction', 'personas', 'workflow', 'memory', 'remoteControl', 'collaboration', 'updater', 'dependencies']);
      useLayoutEffect(() => {
