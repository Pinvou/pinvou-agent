...
- **main**: prompt 减肥 A 类(base.md 散文压缩 20.3K→11.1K,渲染总 prompt 38.4K→29.1K,-24%);前置阶段 N override 迁移已并入;推 `Pinvou/pinvou3`(owner 直推 bypass PR 保护)+ backup `h3c-hexin/pinvou3`
...
**🟡 中等**
- **prompt 减肥 C 类**(A 类 base.md 已做,渲染 38.4K→29.1K):剩余大头在 submodule 无 override——Personality(calm.md ~1.3K)/ Compaction 模板(compact.md ~3K)/ Mode·Approval(~3.2K)。要减得「加 override hook(同阶段 N,通用可提 PR,不增 fork drift)」或「改 fork」。建议优先 Compaction+Personality 加 hook 后压缩;instructions.md(5K,app 层)亦可二次精简。memory `prompt-slimming-task` 有地形+forkguard 红线
